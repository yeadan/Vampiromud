/**************************************************************************\
 *                                                                        *
 *     OasisEDITOR II for Smaug 1.40 written by Evan Cortens(Tagith)         *
 *                                                                        *
 *   Based on OasisEDITOR for CircleMUD3.0bpl9 written by Harvey Gilpin      *
 *                                                                        *
 **************************************************************************
 *                                                                        *
...
/*------------------------------------------------------------------------*/
void limpiar_editor( DESCRIPTOR_DATA *d )
{
    if ( d->editor )
    {
	if ( d->character )
	{
	    ...
	    act( AT_ACTION, "$n stops using EDITOR.", d->character, NULL, NULL, TO_CANSEE );
	}
...
void do_ooedit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Editar que objeto?\n\r", ch );
	return;
    }
    if ( (obj = get_obj_world( ch, arg ) ) == NULL )
    {
	send_to_char( "No hay ningun objeto asi en VampiroMud.\n\r", ch );
	return;
    }
	...
	if ( d->connected == CON_OEDIT )
	    if ( d->editor && EDITOR_VNUM(d) == obj->pIndexData->vnum )
	    {
		ch_printf( ch, "Este objeto esta siendo editado actualmente por %s.\n\r", d->character->name );
		return;
	    }
    ...
    EDITOR_VNUM(d) = obj->pIndexData->vnum;
    ...
    act( AT_ACTION, "$n comienza a usar el Editor de Areas.", ch, NULL, NULL, TO_CANSEE );
    return;
}
...
void do_ocopy( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !arg1 || !arg2 )
    {
	send_to_char( "Uso: ocopiar <original> <nuevo>\n\r", ch );
        return;
    }
    ovnum = atoi( arg1 );
    cvnum = atoi( arg2 );
...
#endif
    {
        ...
        {
            send_to_char( "Debes de tener un area asignada para copiar.\n\r", ch );
            return;
        }
        if ( cvnum < pArea->low_o_vnum
        ||   cvnum > pArea->hi_o_vnum )
	{
            send_to_char( "Ese numero no esta en tu rango.\n\r", ch );
            return;
        }
    }
    ...
    {
        send_to_char( "Ese objeto ya existe.\n\r", ch );
        return;
    }

    if ( (orig = get_obj_index( ovnum ) ) == NULL )
    {
	send_to_char( "Como puedes compiar algo que no existe?\n\r", ch );
	return;
    }
...
void oedit_disp_progs( DESCRIPTOR_DATA *d )
{
    ...
    ch_printf_color( d->character, "&gA&w) Añadir u nuevo programa\n\r" );
    ch_printf_color( d->character, "&gR&w) Eliminar un programa existente\n\r" );
    ch_printf_color( d->character, "&gQ&w) Salir\n\r" );
    ch_printf_color( d->character, "\n\rEleccion: " );

    EDITOR_MODE(d) = OEDIT_MPROGS;
}
...
void oedit_disp_container_flags_menu( DESCRIPTOR_DATA *d )
{
    ...
    sprintf( buf, "Flags de Contenedor: &c%s&w\n\r", flag_string( obj->value[1], container_flags ) );
    send_to_char_color( buf, ch );

    send_to_char( "Introducir flag, 0 para salir : ", ch );
}
...
void oedit_disp_extradesc_menu( DESCRIPTOR_DATA *d )
{
    ...
    EDITOR_MODE(d) = OEDIT_EXTRADESC_MENU;
}
...
void oedit_disp_extra_choice( DESCRIPTOR_DATA *d )
{
    ...
    EDITOR_MODE(d) = OEDIT_EXTRADESC_CHOICE;
}
...
void oedit_disp_prompt_apply_menu( DESCRIPTOR_DATA *d )
{
    ...
    EDITOR_MODE(d) = OEDIT_AFFECT_MENU;
...
void oedit_disp_affect_menu( DESCRIPTOR_DATA *d )
{
    ...
    EDITOR_MODE(d) = OEDIT_AFFECT_LOCATION;
...
void do_oedit_reset( CHAR_DATA *ch, char *argument )
{
    ...
    case SUB_OBJ_EXTRA:
        ...
        /* EDITOR_DESC(ch->desc) = ch->spare_ptr; */
	...
	ch->spare_ptr = ed;
        ...
        EDITOR_MODE(ch->desc) = OEDIT_EXTRADESC_CHOICE;
    ...
    case SUB_OBJ_LONG:
        ...
        ch->desc->connected = CON_OEDIT;
        ch->substate = SUB_NONE;
        EDITOR_MODE( ch->desc ) = OEDIT_MAIN_MENU;
 ...
 * to it based on which EDITOR mode you are in at the time
 */
void oedit_parse( DESCRIPTOR_DATA *d, char *arg )
{
        ...
        switch ( UPPER(arg[0]) )
	{
        case 'Q':
            /* send_to_char( "Do you wish to save this object internally?: ", d->character );
             EDITOR_MODE(d) = OEDIT_CONFIRM_SAVESTRING; */
            limpiar_editor( d );
            return;
        case '1':
            send_to_char("Enter namelist : ", d->character);
            EDITOR_MODE(d) = OEDIT_EDIT_NAMELIST;
            break;
        case '2':
            send_to_char("Enter short desc : ", d->character);
            EDITOR_MODE(d) = OEDIT_SHORTDESC;
            break;
        case '3':
            send_to_char("Enter long desc :-\r\n| ", d->character);
            EDITOR_MODE(d) = OEDIT_LONGDESC;
            break;
        case '4':
            /* lets not */
            send_to_char("Enter action desc :-\r\n", d->character);
            EDITOR_MODE(d) = OEDIT_ACTDESC;
            break;
        case '5':
            oedit_disp_type_menu(d);
            EDITOR_MODE(d) = OEDIT_TYPE;
            break;
        case '6':
            oedit_disp_extra_menu(d);
            EDITOR_MODE(d) = OEDIT_EXTRAS;
            break;
        case '7':
            oedit_disp_wear_menu(d);
            EDITOR_MODE(d) = OEDIT_WEAR;
            break;
        case '8':
            send_to_char("Enter weight : ", d->character);
            EDITOR_MODE(d) = OEDIT_WEIGHT;
            break;
        case '9':
            send_to_char("Enter cost : ", d->character);
            EDITOR_MODE(d) = OEDIT_COST;
            break;
        case 'A':
            send_to_char("Enter cost per day : ", d->character);
            EDITOR_MODE(d) = OEDIT_COSTPERDAY;
            break;
        case 'B':
            send_to_char("Enter timer : ", d->character);
            EDITOR_MODE(d) = OEDIT_TIMER;
            break;
        case 'C':
            send_to_char("Enter level : ", d->character);
            EDITOR_MODE(d) = OEDIT_LEVEL;
            break;
        case 'D':
	...
	editor_log( d, "Changed name to %s", obj->name );
        break;
	...
	editor_log( d, "Changed short to %s", obj->short_descr );
        break;
	...
	editor_log( d, "Changed longdesc to %s", obj->description );
        break;
	...
	editor_log( d, "Changed actiondesc to %s", obj->action_desc );
        break;
	...
	editor_log( d, "Changed object type to %s", o_types[number] );
        break;
    ...
    case OEDIT_WEIGHT:
        number = atoi(arg);
        obj->weight = number;
	editor_log( d, "Changed weight to %d", obj->weight );
        if ( IS_OBJ_STAT(obj, ITEM_PROTOTYPE) )
    ...
    case OEDIT_COST:
        number = atoi(arg);
        obj->cost = number;
	editor_log( d, "Changed cost to %d", obj->cost );
        if ( IS_OBJ_STAT(obj, ITEM_PROTOTYPE) )
	...
	editor_log( d, "Changed rent to %d", obj->pIndexData->rent );
        break;

    case OEDIT_TIMER:
        number = atoi(arg);
        obj->timer = number;
	editor_log( d, "Changed timer to %d", obj->timer );
        break;

    case OEDIT_LEVEL:
        number = atoi(arg);
        obj->level = URANGE( 0, number, MAX_LEVEL );
	editor_log( d, "Changed object level to %d", obj->level );
        break;
	...
	editor_log( d, "Changed layers to %d", obj->pIndexData->layers );
	oedit_disp_layer_menu(d);
        return;
    ...
    case OEDIT_VALUE_1:
        ...
        editor_log( d, "Changed v0 to %d", obj->value[0] );
        return;
	...
	editor_log( d, "Changed v1 to %d", obj->value[1] );
        return;
	...
	case ITEM_SCROLL:
        ...
        editor_log( d, "Changed v2 to %d", obj->value[2] );
        if ( IS_OBJ_STAT( obj, ITEM_PROTOTYPE) )
    ...
    case OEDIT_VALUE_4:
        ...
        editor_log( d, "Changed v3 to %d", obj->value[3] );
        if ( IS_OBJ_STAT( obj, ITEM_PROTOTYPE) )
    ...
    case OEDIT_VALUE_5:
        ...
        editor_log( d, "Changed v4 to %d", obj->value[4] );
        if ( IS_OBJ_STAT( obj, ITEM_PROTOTYPE) )
    ...
    case OEDIT_VALUE_6:
        ...
        editor_log( d, "Changed v5 to %d", obj->value[5] );
        if ( IS_OBJ_STAT( obj, ITEM_PROTOTYPE) )
    ...
    case OEDIT_AFFECT_MENU:
	    ...
	    else
	    {
                ...
                EDITOR_MODE(d) = OEDIT_AFFECT_REMOVE;
            }
            return;
	...
	if ( is_number( arg ) )
	{
        ...
        paf->location = number;
	EDITOR_MODE(d) = OEDIT_AFFECT_MODIFIER;
	...
	default:
        ...
        /* Link it in */
	if ( !value || EDITOR_VAL(d) == TRUE )
	{
            paf->modifier = value;
            editor_log( d, "Modified affect to: %s by %d", a_types[paf->location], value );
	    EDITOR_VAL(d) = FALSE;
	    oedit_disp_prompt_apply_menu(d);
	    return;
	}
	...
	xCLEAR_BITS( npaf->bitvector );
        ...
        ++top_affect;
        editor_log( d, "Added new affect: %s by %d", a_types[npaf->location], npaf->modifier );
    ...
    case OEDIT_AFFECT_REMOVE:
        number = atoi( arg );
        remove_affect_from_obj( obj, number );
        editor_log( d, "Removed affect #%d", number );
        oedit_disp_prompt_apply_menu(d);
        return;
	...
	/* if ( SetOExtra( obj, arg ) || SetOExtraProto( obj->pIndexData, arg ) )
	{
        ...
        editor_log( d, "Changed exdesc %s to %s", ed->keyword, arg );
        STRFREE( ed->keyword );
    ...
    case OEDIT_EXTRADESC_CHOICE:
        ...
        case 0:
            ...
            return;
        case 1:
            EDITOR_MODE(d) = OEDIT_EXTRADESC_KEY;
            send_to_char( "Enter keywords, speperated by spaces: ", d->character );
            return;
        case 2:
            EDITOR_MODE(d) = OEDIT_EXTRADESC_DESCRIPTION;
    ...
    case OEDIT_EXTRADESC_DELETE:
        ...
        editor_log( d, "Deleted exdesc %s", ed->keyword );
        if ( IS_OBJ_STAT( obj, ITEM_PROTOTYPE ) )
    ...
    case OEDIT_EXTRADESC_MENU:
	    ...
	    top_ed++;
            ...
            editor_log( d, "Added new exdesc" );

            oedit_disp_extra_choice(d);
            return;

        case 'R':
	    EDITOR_MODE(d) = OEDIT_EXTRADESC_DELETE;
	...
	case 'R':
	    ...
	    EDITOR_MODE(d) = OEDIT_MPROGS_DELETE;
	    return;
	case 'Q':
	    break;
	default:
	...
	{
	case 'A':
	    EDITOR_MODE(d) = OEDIT_MPROGS_TYPE;
	    oedit_disp_prog_types(d);
	    return;
	case 'B':
	    EDITOR_MODE(d) = OEDIT_MPROGS_ARG;
	...
	editor_log( d, "Changed prog %s to %s",
    ...
    EDITOR_CHANGE(d) = TRUE; /*. Has changed flag .*/
    oedit_disp_menu(d);
}

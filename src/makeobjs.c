...
void make_corpse( CHAR_DATA *ch, CHAR_DATA *killer )
{
    ...
    {
    if(ch->pcdata->clan->morgue != 0)
    location = get_room_index ( ch->pcdata->clan->morgue );
    else
    location = get_room_index ( ROOM_VNUM_MORGUE );
    }
...
/*	Using corpse cost to cheat, since corpses not sellable */
    ...
    sprintf( buf, "cuerpo corpse %s", name );
    STRFREE( corpse->name );
...
/*KAYSER*/
    ...
    /* Cuando quiera restringir el uso del deposito bajar esto */
...
void do_crear_taquilla( CHAR_DATA *ch, char *argument, OBJ_DATA *taquilla )
{
    char buf[MAX_STRING_LENGTH];
    char *nombre;
    CHAR_DATA *victim;
    ...
    if ( argument[0] == '\0' )
    {
        send_to_char( "&YUso: taquilla <jugador>.\n\r", ch );
        return;
    }
    ...
    nombre = victim->name;
    taquilla		= create_object(get_obj_index(OBJ_VNUM_TAQUILLA), 0);

    xSET_BIT( taquilla->extra_flags, ITEM_TAQUILLA );

    sprintf( buf, "%s", nombre );
    STRFREE( taquilla->name );
    taquilla->name = STRALLOC( buf );

    sprintf( buf, taquilla->short_descr, nombre );
    STRFREE( taquilla->short_descr );
    taquilla->short_descr = STRALLOC( buf );
    taquilla->value[0] = 1000;

    ch_printf( ch, "De la nada creas una taquilla con la marca de %s.\n\r", nombre );
    ch_printf( victim, "%s hace unos gestos con las manos y crea una taquilla.\n\r", ch->name );

    obj_to_room( taquilla, victim->in_room );

    write_taquillas( victim, nombre, NULL );

...
CMDTYPE	   *command_hash[MAX_COMANDOS];	/* hash table for cmd_table */
...
bool check_pos( CHAR_DATA *ch, sh_int position )
{
    ...
    if( ch->position == POS_PREDECAP )
    {
    	send_to_char( "Estas herido de muerte, solo te queda esperar...\n\r", ch );
	return FALSE;
    }
	...
	{
	case POS_DEAD:
	    send_to_char( "Algo dificil cuando estas MUERTO...\n\r", ch );
	    break;
	...
	case POS_FIGHTING:
	      ...
	      send_to_char( "Todavia estas luchando!\n\r", ch);
             }
	    break;
    	case POS_DEFENSIVE:
            if(position<=POS_EVASIVE){
	      send_to_char( "Tu estilo de lucha es demasiado simple para que puedas hacer eso!\n\r", ch);
            } else {
	    send_to_char( "No puedes!Aun estas luchando!\n\r", ch);
            }
	    break;
    	case POS_AGGRESSIVE:
            if(position<=POS_EVASIVE){
...
void interpret( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char( "Estas congelado!\n\r", ch );
	    return;
	}

	if( !IS_NPC(ch) && ch->position == POS_PREDECAP )
	{
		if(str_prefix( command, "quien"))
		send_to_char( "Estas herido de muerte, solo te queda esperar...\n\r", ch );
		return;
	}
	...
	for ( cmd = command_hash[LOWER(command[0])%126]; cmd; cmd = cmd->next )
        ...
        /*
         * Para activar a desactivar comandos
         * Mejora del codigo SMAUG por COLIKOTRON Code Team
         */
         if (cmd)
         if ((cmd->estado != 0 && cmd->estado != 2))
         {
                send_to_char( "Comando DESACTIVADO temporalmente.\n\r", ch);
                return;
         }
         ...
         if (cmd->estado == 2)
         {
                send_to_char( "Comando OBSOLETO.\n\r", ch );
                return;
         }

         /*
          * Checkeo de dsciplinas y niveles
          * Vampiromud2.0b
          */
        if(cmd)
        if(cmd->disciplina > 0)
        {
          if(IS_NPC(ch))
          {
            ...
            if(ch->pcdata->habilidades[cmd->disciplina] < cmd->disci_level && !IS_IMMORTAL( ch ) )
            {
                ch_printf(ch, "&wNo conoces ese nivel de &g%s&w.\n\rAyuda '&g%s&w' para saber mas sobre ello.\n\r",
                capitalize(disciplina[cmd->disciplina]),
...
/*
     	    ...
     	    act( AT_GREY, "$n deja de estar afk.", ch, NULL, NULL, TO_ROOM );

*/

     	    act( AT_GREY, "$n deja de estar afk.", ch, NULL, NULL, TO_CANSEE );
...
*/
    ...
    sprintf( lastplayercmd, "%s ha usado %s", ch->name, logline );
...
/*	if ( ch->desc && ch->desc->original )
...
#endif
	{
		...
		return;
		  ...
		  if ( !IS_SET( pexit->exit_info, EX_SECRET ) )
		    act( AT_PLAIN, "$d esta cerrado.", ch, NULL, pexit->keyword, TO_CHAR );
		  else
		...
		return;
            ...
            percent = number_range( 1, 3 );
            if( percent == 1)
	    send_to_char( "Huh?\n\r", ch );
            if( percent == 2)
	    send_to_char( "No me digas.\n\r", ch );
            if( percent == 3)
	    send_to_char( "Anda ia.\n\r", ch );
	}
	return;
    }
	...
	send_to_char("No parece que puedas hacerlo ahora.\n\r", ch );
...

{
    ...
    case POS_INCAP:
    case POS_MORTAL:
	send_to_char( "Estas demasiado herido para hacer eso.\n\r", ch );
    ...
    case POS_STUNNED:
	send_to_char( "Estas demasiado aturdido.\n\r", ch );
	return TRUE;
...

    {
...

    	{
...

    		{
    			...
    			ch_printf(victim, "Intentas ignorar a %s,"

    				" pero no parece que puedas.\n\r", ch->name);
	...
	&&   !HAS_PROG(victim->pIndexData, ACT_PROG) )
	{
		...
		if ( IS_NEUTRAL(ch) )
		{
		    act( AT_ACTION, "$n lame a $N.",  victim, NULL, ch, TO_NOTVICT );
		    act( AT_ACTION, "Lames a $N.",  victim, NULL, ch, TO_CHAR    );
		    act( AT_ACTION, "$n te lame.", victim, NULL, ch, TO_VICT    );
		}
		else
		{
		    act( AT_ACTION, "$n actua como si $N no existiera.",  victim, NULL, ch, TO_NOTVICT );
		    act( AT_ACTION, "Simplemente ignoras a $N.",  victim, NULL, ch, TO_CHAR    );
		    act( AT_ACTION, "$n parece que te ignora.", victim, NULL, ch, TO_VICT    );
		}
		break;
...

{
  ...
  if ( IS_AFFECTED (ch, AFF_POSSESS) && IS_SET( cmd->flags, CMD_FLAG_POSSESS ))
        ...
        sprintf ( cmd_flag_buf, "No puedes %s mientras estes poseyendo a alguien!\n\r",
        ...
        sprintf ( cmd_flag_buf, "No puedes %s mientras uses el polymorph!\n\r",

...
SPELL_FUN *spell_function( char *name )
{
    ...
    if ( !str_cmp( name, "spell_comunion_espiritual" ))             return spell_comunion_espiritual;
    ...
    if ( !str_cmp( name, "spell_draining_breath" ))     return spell_draining_breath;
    ...
    if ( !str_cmp( name, "spell_habilidad" ))       return spell_habilidad;
    ...
    if ( !str_cmp( name, "spell_toque_espiritual" ))	     return spell_toque_espiritual;
...
DO_FUN *skill_function( char *name )
{
    ...
    {
    case 'a':
   if ( !str_cmp( name, "do_abdicar" ))            return do_abdicar;
   if ( !str_cmp( name, "do_apartados" ))            return do_apartados;
   if ( !str_cmp( name, "do_abrazar" ))            return do_abrazar;
   if ( !str_cmp( name, "do_accept" ))             return do_accept;
   if ( !str_cmp( name, "do_adiestrar" ))             return do_adiestrar;
   if ( !str_cmp( name, "do_adquirir" ))             return do_adquirir;
   if ( !str_cmp( name, "do_ahall" ))              return do_ahall;
   if ( !str_cmp( name, "do_arena" ))              return do_arena;
   if ( !str_cmp( name, "do_aprender" ))              return do_aprender;
   if ( !str_cmp( name, "do_asimilar" ))              return do_asimilar;
   ...
   if ( !str_cmp( name, "do_aassign" ))      return do_aassign;
	...
	if ( !str_cmp( name, "do_areas" ))		return do_areas;
                 if ( !str_cmp( name, "do_ascender" ))		return do_ascender;
   ...
   if ( !str_cmp( name, "do_board" ))    return do_board;
	...
	break;
    case 'c':
   if ( !str_cmp( name, "do_cancionserena" ))          return do_cancionserena;
   if ( !str_cmp( name, "do_celeridad" ))          return do_celeridad;
   if ( !str_cmp( name, "do_celeridad" ))          return do_celeridad;
   if ( !str_cmp( name, "do_capasombria" ))        return do_capasombria;
   if ( !str_cmp( name, "do_coma" ))               return do_coma;
   if ( !str_cmp( name, "do_conquistas" ))               return do_conquistas;
   if ( !str_cmp( name, "do_conjurar" ))               return do_conjurar;
   if ( !str_cmp( name, "do_concentrar" ))               return do_concentrar;
   if ( !str_cmp( name, "do_creardisciplina" ))               return do_creardisciplina;
   if ( !str_cmp( name, "do_crearnivel" ))               return do_crearnivel;
   if ( !str_cmp( name, "do_crearesfera" ))               return do_crearesfera;
   if ( !str_cmp( name, "do_critico" ))               return do_critico;
   if ( !str_cmp( name, "do_camuflaje" ))          return do_camuflaje;
   ...
   if ( !str_cmp( name, "do_chaos" ))              return do_chaos;
	if ( !str_cmp( name, "do_copyover" ))           return do_copyover;
                 if ( !str_cmp( name, "do_caldero" ))      return do_caldero;
	...
	if ( !str_cmp( name, "do_counciltalk" ))	return do_counciltalk;
        if ( !str_cmp( name, "do_crear_taquilla" ))		return do_crear_taquilla;
	if ( !str_cmp( name, "do_credits" ))		return do_credits;
        if ( !str_cmp( name, "do_creo" ))		return do_creo; /* Creo ignem Taumaturgia nivel 1 */
	if ( !str_cmp( name, "do_cset" ))		return do_cset;
	break;
    case 'd':
                   if ( !str_cmp( name, "do_decapitar" ))        return do_decapitar;
                   if ( !str_cmp( name, "do_debilidad" ))        return do_debilidad;
                   if ( !str_cmp( name, "do_declarar" ))        return do_declarar;
                   if ( !str_cmp( name, "do_decline" ))            return do_decline;
	if ( !str_cmp( name, "do_degradar" ))		return do_degradar;
	...
	if ( !str_cmp( name, "do_deny" ))		return do_deny;
        if ( !str_cmp( name, "do_dset" ))		return do_dset;
        if ( !str_cmp( name, "do_dver" ))		return do_dver;
	...
	if ( !str_cmp( name, "do_devote" ))		return do_devote;
        if ( !str_cmp( name, "do_diablerizar" ))		return do_diablerizar;
	if ( !str_cmp( name, "do_dig" ))		return do_dig;
	if ( !str_cmp( name, "do_disarm" )) 		return do_disarm;
                 if ( !str_cmp( name, "do_disciplinas" ))          return do_disciplinas;
	...
	if ( !str_cmp( name, "do_dominio" ))      return do_dominio;
	...
	if ( !str_cmp( name, "do_eat" ))		return do_eat;
        if ( !str_cmp( name, "do_eset" ))		return do_eset;
        if ( !str_cmp( name, "do_ever" ))		return do_ever;
        if ( !str_cmp( name, "do_esferas" ))		return do_esferas;
        if ( !str_cmp( name, "do_email" ))		return do_email;
	if ( !str_cmp( name, "do_embrujar" ))              return do_embrujar;
	...
	if ( !str_cmp( name, "do_emote" ))		return do_emote;
        if ( !str_cmp( name, "do_empalar" ))		return do_empalar;
	if ( !str_cmp( name, "do_empty" ))		return do_empty;
	if ( !str_cmp( name, "do_enfermedad" ))         return do_enfermedad;
	...
	break;
    case 'f':
	if ( !str_cmp( name, "do_fascinacion" ))        return do_fascinacion;
	if ( !str_cmp( name, "do_feed" ))		return do_feed;
        if ( !str_cmp( name, "do_finger" ))		return do_finger;
        if ( !str_cmp( name, "do_fortaleza" ))		return do_fortaleza;
	...
	if ( !str_cmp( name, "do_forceclose" ))		return do_forceclose;
                 if ( !str_cmp( name, "do_forma_incorporea" ))	return do_forma_incorporea;
	 ...
	 */
	if ( !str_cmp( name, "do_famtest" ))	return do_famtest;
	if ( !str_cmp( name, "do_fcrear" ))		return do_fcrear;
	if ( !str_cmp( name, "do_feditar" ))	return do_feditar;
	if ( !str_cmp( name, "do_fadmitir" ))	return do_fadmitir;
	if ( !str_cmp( name, "do_fexpulsar" ))	return do_fexpulsar;
	if ( !str_cmp( name, "do_familia" ))	return do_familia;
	if ( !str_cmp( name, "do_fvotar" ))		return do_fvotar;
	if ( !str_cmp( name, "do_finvocar" ))	return do_finvocar;
	/*
	 * Vampiromud v3.0 -- FIN Familias -- by: SaNgUi
	 */
	break;
    case 'g':
	if ( !str_cmp( name, "do_gaso" ))		return do_gaso;
        if ( !str_cmp( name, "do_garrasdelabestia" ))		return do_garrasdelabestia;
   if ( !str_cmp( name, "do_generacionup" ))    return do_generacionup;  /* SiGo */
	...
	if ( !str_cmp( name, "do_guardatodo" ))		return do_guardatodo;
	...
	if ( !str_cmp( name, "do_hiscore" ))		return do_hiscore; /* cronel hiscores */
	if ( !str_cmp( name, "do_hiscoset" ))		return do_hiscoset; /* cronel hiscores */
	...
	if ( !str_cmp( name, "do_hset" ))		return do_hset;
        /*if ( !str_cmp( name, "do_haff" ))		return do_haff;*/
	break;
    case 'i':
#ifdef USE_IMC
	...
	if ( !str_cmp( name, "do_induct" ))		return do_induct;
        if ( !str_cmp( name, "do_ingreso" ))            return do_ingreso;
	if ( !str_cmp( name, "do_infoarea" ))		return do_infoarea;
	...
	break;
    case 'l':
        /*if ( !str_cmp( name, "do_llamassubmundo" ))		return do_llamassubmundo;*/
        if ( !str_cmp( name, "do_llamadasalvaje" ))		return do_llamadasalvaje;
	...
	if ( !str_cmp( name, "do_mascara" ))            return do_mascara;
	...
	if ( !str_cmp( name, "do_mesmerismo" )) 	return do_mesmerismo;
    	if ( !str_cmp( name, "do_memoria_del_juerguista" )) return do_memoria_del_juerguista;
	if ( !str_cmp( name, "do_miembros" )) 		return do_miembros;
	...
	if ( !str_cmp( name, "do_mdelete" ))		return do_mdelete;
        if ( !str_cmp( name, "do_mejorar" ))		return do_mejorar;
        if ( !str_cmp( name, "do_mensaje" ))		return do_mensaje;
	if ( !str_cmp( name, "do_memory" ))		return do_memory;
   	if ( !str_cmp( name, "do_misericordia" ))    	return do_misericordia;
   	if ( !str_cmp( name, "do_mirada" ))             return do_mirada;
	...
	if ( !str_cmp( name, "do_mpmload" ))		return do_mpmload;
        if ( !str_cmp( name, "do_mcopy" ))		return do_mcopy;
	...
	if ( !str_cmp( name, "do_name" ))		return do_name;
        if ( !str_cmp( name, "do_nset" ))		return do_nset;
        if ( !str_cmp( name, "do_nver" ))		return do_nver;
	...
	if ( !str_cmp( name, "do_noaffects"))		return do_noaffects;
	...
	if ( !str_cmp( name, "do_noteroom" ))		return do_noteroom;
        if ( !str_cmp( name, "do_nota" ))		return do_nota;
  	if ( !str_cmp( name, "do_nuisance" ))		return do_nuisance;
	break;
    case 'o':
        if ( !str_cmp( name, "do_oleadademente" ))	return do_oleadademente;
...
/*	if ( !str_cmp( name, "do_opentourney" ))	return do_opentourney; */
	...
	break;
    case 'p':
        if ( !str_cmp( name, "do_potencia" ))		return do_potencia;
	if ( !str_cmp( name, "do_pagelen" ))		return do_pagelen;
	if ( !str_cmp( name, "do_pasion" ))		return do_pasion;
        if ( !str_cmp( name, "do_privacy" ))		return do_privacy;
	if ( !str_cmp( name, "do_pager" ))		return do_pager;
        if ( !str_cmp( name, "do_pETRIFICARSE" ))	return do_petrificarse;
	...
	if ( !str_cmp( name, "do_peace" ))		return do_peace;
        if ( !str_cmp( name, "do_pertenecer" ))		return do_pertenecer;
	...
	if ( !str_cmp( name, "do_pxset" ))              return do_pxset;
	break;
    case 'q':
   ...
   if ( !str_cmp( name, "do_regreso" ))    return do_regreso;
                   if ( !str_cmp( name, "do_reintegro" ))                                return do_reintegro;
	if ( !str_cmp( name, "do_remove" ))		return do_remove;
	if ( !str_cmp( name, "do_remains" ))		return do_remains;
        if ( !str_cmp( name, "do_renacer" ))            return do_renacer;
	...
	if ( !str_cmp( name, "do_sacrifice" ))		return do_sacrifice;
                   if ( !str_cmp( name, "do_saldo" ))                                      return do_saldo;
	...
	if ( !str_cmp( name, "do_silence" ))		return do_silence;
        if ( !str_cmp( name, "do_silenciodelamuerte" ))		return do_silenciodelamuerte;
	...
	if ( !str_cmp( name, "do_south" ))		return do_south;
        if ( !str_cmp( name, "do_sentidosaguzados" ))		return do_sentidosaguzados;
   ...
   if ( !str_cmp( name, "do_study" ))     return do_study;
	if ( !str_cmp( name, "do_stun" ))		return do_stun;
        if ( !str_cmp( name, "do_susurrosamables" ))		return do_susurrosamables;
	...
	if ( !str_cmp( name, "do_topten" ))		return do_topten;
        if ( !str_cmp( name, "do_tormento" ))             return do_tormento;
	if ( !str_cmp( name, "do_testigodelastinieblas" ))		return do_testigodelastinieblas;
	if ( !str_cmp( name, "do_tangana" ))		return do_tangana;
	if ( !str_cmp( name, "do_tell" ))		return do_tell;
	if ( !str_cmp( name, "do_think" ))		return do_think;
	if ( !str_cmp( name, "do_time" ))		return do_time;
        if ( !str_cmp( name, "do_tiempo" ))		return do_tiempo;
   ...
   if ( !str_cmp( name, "do_traffic" ))      return do_traffic;
	if ( !str_cmp( name, "do_transfer" ))		return do_transfer;
                   if ( !str_cmp( name, "do_trucos_del_almendruco" ))		return do_trucos_del_almendruco;
	if ( !str_cmp( name, "do_trust" ))		return do_trust;
	if ( !str_cmp( name, "do_typo" ))		return do_typo;
        /*if ( !str_cmp( name, "do_toqueespiritual" ))		return do_toqueespiritual;*/
	break;
    case 'u':
	...
	if ( !str_cmp( name, "do_version" ))		return do_version;
        if ( !str_cmp( name, "do_vficha" ))		return do_vficha;
	if ( !str_cmp( name, "do_victories" ))		return do_victories;
	if ( !str_cmp( name, "do_visible" ))		return do_visible;
        /*if ( !str_cmp( name, "do_vaff" ))		return do_vaff;*/
        if ( !str_cmp( name, "do_visiondelalma" ))	return do_visiondelalma;
...
char *spell_name( SPELL_FUN *spell )
{
    ...
    if ( spell == spell_comunion_espiritual )     return "spell_comunion_espiritual";
    ...
    if ( spell == spell_draining_breath )      return "spell_draining_breath";
    ...
    if ( spell == spell_habilidad )          return "spell_habilidad";
    ...
    if ( spell == spell_toque_espiritual )	    return "spell_toque_espiritual";
...
char *skill_name( DO_FUN *skill )
{
    ...
    if ( skill == do_abdicar )		return "do_abdicar";
    if ( skill == do_apartados )	return "do_apartados";
    if ( skill == do_abrazar )		return "do_abrazar";
    if ( skill == do_aassign )		return "do_aassign";
    if ( skill == do_asimilar )		return "do_asimilar";
    if ( skill == do_advance )		return "do_advance";
    if ( skill == do_adiestrar )                                return "do_adiestrar";
    if ( skill == do_adquirir )		return "do_adquirir";
    ...
    if ( skill == do_pertenecer )                       return "do_pertenecer";
    ...
    if ( skill == do_potencia )		return "do_potencia";
    if ( skill == do_celeridad )	return "do_celeridad";
    if ( skill == do_fascinacion )      return "do_fascinacion";
    if ( skill == do_fortaleza )	return "do_fortaleza";

	/*
	 * Vampiromud v3.0 -- Familias -- by: SaNgUi
	 */
	if ( skill == do_famtest )		return "do_famtest";
	if ( skill == do_fcrear )		return "do_fcrear";
	if ( skill == do_feditar )		return "do_feditar";
	if ( skill == do_fadmitir )		return "do_fadmitir";
	if ( skill == do_fexpulsar )	return "do_fexpulsar";
	if ( skill == do_familia )		return "do_familia";
	if ( skill == do_fvotar )		return "do_fvotar";
	if ( skill == do_finvocar )		return "do_finvocar";
	/*
	 * Vampiromud v3.0 -- FIN Familias -- by: SaNgUi
	 */

    if ( skill == do_capasombria )	return "do_capasombria";
    if ( skill == do_coma )		return "do_coma";
    if ( skill == do_conquistas )		return "do_conquistas";
    if ( skill == do_conjurar )		return "do_conjurar";
    if ( skill == do_concentrar )		return "do_concentrar";
    if ( skill == do_creardisciplina )		return "do_creardisciplina";
    if ( skill == do_crearnivel )		return "do_crearnivel";
    if ( skill == do_crearesfera )		return "do_crearesfera";
    if ( skill == do_critico )		return "do_critico";
    if ( skill == do_oleadademente )	return "do_oleadademente";
    if ( skill == do_llamadasalvaje )	return "do_llamadasalvaje";
    if ( skill == do_sentidosaguzados )	return "do_sentidosaguzados";
    ...
    if ( skill == do_pasion )		return "do_pasion";
    ...
    if ( skill == do_crear_taquilla )   return "do_crear_taquilla";
    if ( skill == do_bury )		return "do_bury";
    if ( skill == do_buy )		return "do_buy";
    if ( skill == do_caldero )      return "do_caldero";
    ...
    if ( skill == do_declarar )         return "do_declarar";
    ...
    if ( skill == do_ascender )		return "do_ascender";
    ...
    if ( skill == do_decapitar )		return "do_decapitar";
    if ( skill == do_debilidad )		return "do_debilidad";
    ...
    if ( skill == do_dset )		return "do_dset";
    if ( skill == do_dver)		return "do_dver";
    ...
    if ( skill == do_diablerizar )		return "do_diablerizar";
    if ( skill == do_dig )		return "do_dig";
    if ( skill == do_disarm )		return "do_disarm";
    if ( skill == do_disciplinas )     return "do_disciplinas";
    if ( skill == do_disconnect )	return "do_disconnect";
    if ( skill == do_dominio )		return "do_dominio";
    if ( skill == do_dismiss )          return "do_dismiss";
    if ( skill == do_degradar )		return "do_degradar";
    ...
    if ( skill == do_eset )		return "do_eset";
    if ( skill == do_ever )		return "do_ever";
    if ( skill == do_esferas )		return "do_esferas";
    if ( skill == do_embrujar )            return "do_embrujar";
    ...
    if ( skill == do_empalar )		return "do_empalar";
    if ( skill == do_empty )		return "do_empty";
    if ( skill == do_enfermedad )            return "do_enfermedad";
    ...
    if ( skill == do_aprender )            return "do_aprender";
    ...
    if ( skill == do_forma_incorporea )	return "do_forma_incorporea";
    ...
    if ( skill == do_garrasdelabestia )		return "do_garrasdelabestia";
    if ( skill == do_get )		return "do_get";
    if ( skill == do_generacionup )    return "do_generacionup";
    ...
    if ( skill == do_guardatodo )	return "do_guardatodo";
    ...
    if ( skill == do_cancionserena )		return "do_cancionserena";
    if ( skill == do_hide )		return "do_hide";
    if ( skill == do_hitall )		return "do_hitall";
    if ( skill == do_hiscore )		return "do_hiscore"; /* cronel hiscores */
    if ( skill == do_hiscoset )		return "do_hiscoset"; /* cronel hiscores */
    ...
    if ( skill == do_ingreso )                  return "do_ingreso";
    ...
    if ( skill == do_visiondelalma )       return "do_visiondelalma";
    if ( skill == do_vficha )	return "do_vficha";
    ...
    if ( skill == do_miembros )		return "do_miembros";
    if ( skill == do_mascara )          return "do_mascara";
    if ( skill == do_massign )		return "do_massign";
    if ( skill == do_mcreate )		return "do_mcreate";
    if ( skill == do_mdelete )		return "do_mdelete";
    if ( skill == do_mejorar )		return "do_mejorar";
    if ( skill == do_mensaje )		return "do_mensaje";
    if ( skill == do_mirada )           return "do_mirada";
    if ( skill == do_memory )		return "do_memory";
    if ( skill == do_mesmerismo )   	return "do_mesmerismo";
    if ( skill == do_memoria_del_juerguista) return "do_memoria_del_juerguista";
    if ( skill == do_mfind )		return "do_mfind";
    if ( skill == do_minvoke )		return "do_minvoke";
    if ( skill == do_mistwalk )		return "do_mistwalk";
    if ( skill == do_misericordia )     return "do_misericordia";
    ...
    if ( skill == do_mcopy )		return "do_mcopy";
    ...
    if ( skill == do_nset )		return "do_nset";
    if ( skill == do_nver )		return "do_nver";
    ...
    if ( skill == do_noaffects )        return "do_noaffects";
    ...
    if ( skill == do_nota )		return "do_nota";
...
/*  if ( skill == do_opentourney )	return "do_opentourney"; */
    ...
    if ( skill == do_petrificarse )		return "do_petrificarse";
    ...
    if ( skill == do_pxset )            return "do_pxset";
    ...
    /* Disciplinas de Vampiromud */
    if ( skill == do_creo )             return "do_creo"; /* Creo ignem Taumaturgia nivel 1 */
#ifdef USE_IMC
    ...
    if ( skill == do_regreso )    return "do_regreso";
    if ( skill == do_reintegro )        return "do_reintegro";
    if ( skill == do_remove )		return "do_remove";
    if ( skill == do_remains )		return "do_remains";
    if ( skill == do_renacer )          return "do_renacer";
...
#endif
    if ( skill == do_saldo     )              return "do_saldo";
    ...
    if ( skill == do_silenciodelamuerte )		return "do_silenciodelamuerte";
    ...
    if ( skill == do_susurrosamables )      return "do_susurrosamables";
    ...
    if ( skill == do_tormento )           return "do_tormento";
    if ( skill == do_testigodelastinieblas )		return "do_testigodelastinieblas";
    if ( skill == do_tangana )		return "do_tangana";    
    if ( skill == do_tell )		return "do_tell";
    if ( skill == do_think )		return "do_think";
    if ( skill == do_time )		return "do_time";
    if ( skill == do_tiempo )		return "do_tiempo";
    ...
    if ( skill == do_trucos_del_almendruco )		return "do_trucos_del_almendruco";
...
bool load_class_file( char *fname )
{
	...
	case 'C':
	    ...
	    break;
                 ...
                 case 'D':
                     KEY( "DisciClass1", class->disci_class1,    fread_number( fp ) );
                     KEY( "DisciClass2", class->disci_class2,    fread_number( fp ) );
                     KEY( "DisciClass3", class->disci_class3,    fread_number( fp ) );
                     break;
...
void write_class_file( int cl )
{
    ...
    fprintf( fpout, "DisciClass1         %d\n",    class->disci_class1 ); /* Asignacion de disciplinas primarias por clase */
    fprintf( fpout, "DisciClass2        %d\n",    class->disci_class2 ); /* Asignacion de disciplinas primarias por clase */
    fprintf( fpout, "DisciClass3         %d\n",    class->disci_class3 ); /* Asignacion de disciplinas primarias por clase */
...
void fwrite_skill( FILE *fpout, SKILLTYPE *skill )
{
    ...
    if ( skill->para_vampiros)
        fprintf( fpout, "PaVampis        %d\n",	skill->para_vampiros );
        fprintf( fpout, "Remort          %d\n", skill->remort );
    if ( skill->difficulty )
    ...
    if ( skill->disciplina  )
	fprintf( fpout, "Disciplina     %d~\n",	skill->disciplina );
    if ( skill->nv_disciplina )
	fprintf( fpout, "Nivel     %d~\n",	skill->nv_disciplina );
...
void save_commands()
{
	...
	for ( command = command_hash[x]; command; command = command->next )
	{
	    ...
	    fprintf( fpout, "Code        %s\n",	 skill_name(command->do_fun) );
                     fprintf( fpout, "Desactivado    %d\n",          command->estado );
                     fprintf( fpout, "Disciplina     %d\n",          command->disciplina );
                     fprintf( fpout, "Disciplinalevel        %d\n",          command->disci_level );
...
SKILLTYPE *fread_skill( FILE *fp )
{
	...
	case 'D':
       ...
       KEY( "Disciplina", skill->disciplina,    fread_number( fp ) );
	    break;
	...
	case 'N':
            ...
            KEY( "Nivel",  skill->nv_disciplina,  fread_number( fp ) );
	    break;

	case 'P':
	    KEY( "Participants",skill->participants,	fread_number( fp ) );
            KEY( "PaVampis",skill->para_vampiros,	fread_number( fp ) );
	    break;

	case 'R':
	    KEY( "Remort",      skill->remort,          fread_number( fp ) );
...
void fread_command( FILE *fp )
{
	...
	case 'C':
	    ...
	    break;
                 ...
                 case 'D':
                      KEY( "Desactivado",       command->estado,        fread_number( fp ) );
                      KEY( "Disciplina",        command->disciplina,    fread_number( fp ) );
                      KEY( "Disciplinalevel",   command->disci_level, fread_number( fp ) );
                      break;

                            ...
                            ROOM_INDEX_DATA *pRoom )
{
  ...
  {
    send_to_char( "Reset a que habitacion?\n\r", ch );
    return NULL;
  }
  if ( arg[0] == '\0' )
    pRoom = ch->in_room;
  ...
  if ( !pRoom )
  {
    send_to_char( "La Room no existe.\n\r", ch );
    return NULL;
  }
  return pRoom;
}
			...
			ROOM_INDEX_DATA *pRoom, char *name)
{
  ...
  if ( !*name )
  {
    ...
    if ( !reset )
      send_to_char( "Reset de objeto no encontrado.\n\r", ch );
    return reset;
  }
  else
  {
			...
			ROOM_INDEX_DATA *pRoom, char *name)
{
  ...
  if ( !*name )
  {
    ...
    if ( !reset )
      send_to_char( "Reset de mob no encontrado.\n\r", ch );
    return reset;
  }
  else
  {
		 ...
		 ROOM_INDEX_DATA *aRoom )
{
  ...
  if ( !aRoom && !str_cmp(arg, "area") )
  {
    ...
    {
      send_to_char( "No tienes ningun reset definido.\n\r", ch );
      return;
    }
    num = pArea->nplayer;
    pArea->nplayer = 0;
...
void reset_area( AREA_DATA *pArea )
{
  ...
  /*Quito esto estoy hasta los wevos. Kayser, enero 2004*/
  /*
  if ( !pArea->first_reset )
  {
  ...
  */
  ...
  for ( pReset = pArea->first_reset; pReset; pReset = pReset->next )
  {
	...
	bug ( buf, 0 );
      ...
      if( ( IS_OBJ_STAT(pObjIndex, ITEM_RARE ) && ( pObjIndex->count >= RARE_LEVEL ) )
	  ||( IS_OBJ_STAT(pObjIndex,ITEM_UNIQUE) && (pObjIndex->count >= 1) ) )
      {
	obj = NULL;
	lastobj = NULL;
	break;
      ...
      if(IS_OBJ_STAT(pObjIndex, ITEM_CASUAL))
      {
        probabilidad = number_range(0, 1000);
        if (probabilidad >= pObjIndex->value[5])
        {
          obj = NULL;
          lastobj = NULL;
          break;
        }
        sprintf( buf, "Objeto casual creado -- %s -- %s", pObjIndex->short_descr, mob->short_descr);
        bug (buf, 0);
      }
...
/*      if ( pArea->nplayer > 0 ||
      ...
      if ( pArea->nplayer > 0
	  ||( count_obj_list(pObjIndex, pRoomIndex->first_content) > 0 )
	  ||( IS_OBJ_STAT(pObjIndex,ITEM_RARE) && ( pObjIndex->count >= RARE_LEVEL))
	  ||( IS_OBJ_STAT(pObjIndex,ITEM_UNIQUE) && (pObjIndex->count >= 1) ) )
      {
        obj = NULL;
        lastobj = NULL;

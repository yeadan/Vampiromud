...
QUEST_DATA    * 	questmaster;	/* questmaster */
...
sh_int			gsn_curse;
sh_int			gsn_invis;
sh_int                  gsn_presencia_invisible;
sh_int                  gsn_celeridad; /* atake extra y wait_state */
sh_int                  gsn_potencia; /* plus de dam relacionado con el nivel del jugador ( maximo mortal 800)
plus de hitroll y damroll segun generacion y nivel de disciplina */
...
void    load_propietario     args( ( AREA_DATA *tarea, FILE *fp ) );
...
void    load_recursos   args( ( AREA_DATA *tarea, FILE *fp ) );
void	load_objects	args( ( AREA_DATA *tarea, FILE *fp ) );
void 	load_projects   args( ( void ) );
void    load_revision   args( ( AREA_DATA *tarea, FILE *fp ) );
void    load_puntos     args( ( AREA_DATA *tarea, FILE *fp ) );
void    load_conquistable    args( ( AREA_DATA *tarea, FILE *fp ) );
...
void    fread_questmaster     args( ( void ) ); /* Para el que cargue al Questmaster */
...
void boot_db( bool fCopyOver )
{
   ...
   sysdata.mud_name = str_dup("(Vampiro Mundo de Tinieblas)");
    }

    log_string("Cargando sociales");
    load_socials();

    log_string("Cargando tabla de skills");
    load_skill_table();
    sort_skill_table();
   ...
   if ( !gsn_first_spell && skill_table[x]->type == SKILL_SPELL )
    ...
    log_string("Cargando Clanes y Clases");
    load_classes();
    ...
    log_string("Cargando Lenguas");
    load_tongues();

    log_string("Creando Wizlist");
    make_wizlist();

    log_string( "Building wizinfo" );
    build_wizinfo( TRUE );

    log_string("Initializando request pipe");
    init_request_pipe();
	...
	log_string("Fijando clima y tiempo");
	...
	*/
	...
	ASSIGN_GSN( gsn_backstab,	"backstab" );
        ASSIGN_GSN( gsn_empalar,      "empalar" );
        ASSIGN_GSN( gsn_tangana,       "tangana" );
   	ASSIGN_GSN( gsn_circle,    "circle" );
        ASSIGN_GSN( gsn_celeridad,    "celeridad" );
        ASSIGN_GSN( gsn_potencia,    "potencia" );
	ASSIGN_GSN( gsn_tumble,		"evadir" );
	...
	ASSIGN_GSN( gsn_habilidad,      "habilidad" );
	...
	/*ASSIGN_GSN( gsn_aid,		"aid" );*/
        ASSIGN_GSN( gsn_aid,            "asistir" );
	/*ASSIGN_GSN( gsn_track,		"track" );*/
        ASSIGN_GSN( gsn_track,          "cazar" );
	...
	ASSIGN_GSN( gsn_presencia_invisible,          "presencia invisible" );
        ASSIGN_GSN( gsn_capa_sombria,                "capa sombria" );
	ASSIGN_GSN( gsn_mass_invis,	"mass invis" );
	ASSIGN_GSN( gsn_poison,		"poison" );
	ASSIGN_GSN( gsn_study,     "estudiar" );
	...
	log_string("Leyendo en el fichero de areas...");
    ...
    */
	...
	initialize_economy( );
   log_string( "Reseteando Areas" );
	area_update( );
   log_string( "Cargando Dioses" );
	load_buildlist( );
   log_string( "Cargando Correo" );
	load_todas( );
    //save_board( );

   log_string( "Cargando familias" );
	load_clans( );
   log_string( "Cargando Consejos" );
	load_deity( );
   log_string( "Cargando deidades" );
	load_councils( );
        ...
        load_reserved( );
      /*  scan_equipment(); */              /* objetos raros unicos (quest) SiGo */
...
/* Morphs MUST be loaded after class and race tables are set up --Shaddai */
        ...
        MOBtrigger = TRUE;
               /* cronel hiscore */
       log_string( "Cargando Tabla de Ranking" );
       load_hiscores( );
       /* Inicio de los Quest */
    CREATE( questmaster, QUEST_DATA, 1 );
    fread_questmaster();
	log_string("Despertando al Questmaster");
        log_string( "Cargando taquillas");
        load_taquillas();
        load_disciplinas();
        load_nivel();
        load_esfera();
 ...
 */
void load_propietario( AREA_DATA *tarea, FILE *fp )
{
    if ( !tarea )
    {
	bug( "Load_propietario: no #AREA seen yet." );
	if ( fBootDb )
	{
	  shutdown_mud( "No #AREA" );
	  exit( 1 );
    ...
    if ( tarea->propietario )
      STRFREE( tarea->propietario );
    tarea->propietario   = fread_string( fp );
    return;
}
...
void load_revision( AREA_DATA *tarea, FILE *fp )
{
    if ( !tarea )
    {
	bug( "Load_revision: no #AREA." );
	if ( fBootDb )
	{
	  shutdown_mud( "No #AREA" );
	  exit( 1 );
    ...
    tarea->revisada	= fread_number( fp );
    return;
}

void load_puntos( AREA_DATA *tarea, FILE *fp )
{
        int aux = 0;
        if( !tarea )
        {
                bug( "Load_puntos: no #AREA." );
                if( fBootDb )
                {
                        shutdown_mud( "No #AREA." );
                        exit( 1 );
              ...
              for( aux = 0; aux < MAX_FAMILIAS; aux++ )
         tarea->puntos[aux]     = fread_number( fp );

         return;
}

void load_conquistable( AREA_DATA *tarea, FILE *fp )
{
    if ( !tarea )
    {
	bug( "Load_conquistable: no #AREA." );
	if ( fBootDb )
	{
	  shutdown_mud( "No #AREA" );
	  exit( 1 );
    ...
    tarea->conquistable	= fread_number( fp );
    return;
}
...
void load_recursos(AREA_DATA *tarea, FILE *fp)
{
  int x1, x2, x3, x4, x5;
  char *ln;
  ...
  if( !tarea )
  {
    bug( "Load_recursos: no hay #AREA por ningun lao" );
    shutdown_mud( "No #AREA (Load_Recursos)" );
    exit(1);
   }
  ...
  for( ; ; )
	...
	break;
      ...
      tarea->recursos->madera = x1;
      tarea->recursos->viveres = x5;
      tarea->recursos->agua = x4;
      tarea->recursos->combustible = x3;
      tarea->recursos->mineral = x2; 
    }
  return;
}
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
    ...
    mob->mpscriptpos		= 0;
    if( xIS_SET(pMobIndex->act, ACT_AUTOMOB))
...
OBJ_DATA *create_object( OBJ_INDEX_DATA *pObjIndex, int level )
{
   ...
   /* exit( 1 ); */
   bug("el objeto cojonero ese no se ha creado:Null null tull tull, agarrame del Dul.");
   return;
    }
...
bool str_cmp( const char *astr, const char *bstr )
{
    if ( !astr )
    {
	bug( "Comparacion de strings: No existe primer string." );
	if ( bstr )
	  fprintf( stderr, "Comparacion de strings: primero: (null)  segundo: %s\n", bstr );
	return TRUE;
    }

    if ( !bstr )
    {
	bug("el primer string era %s",astr);	
	bug( "Comparacion de strings: No existe segundo string." );
	if ( astr )
	  fprintf( stderr, "Comparacion de strings: primero: %s  segundo: (null)\n", astr );
	return TRUE;
    }
...
void bug( const char *str, ... )
{
	...
	sprintf( buf, "\x1b[0;37m[*****]\x1b[0;31m ARCHIVO: \x1b[1;37m%s \x1b[0;34m LINEA:\x1b[1;37m %d\x1b[0m\x1b", strArea, iLine );
	log_string( buf );
    ...
    strcpy( buf, "\x1b[0;37m[*****]\x1b[0;31m BUG \x1b[0m\x1b : " );
    {
	va_list param;
...
void log_string_plus( const char *str, sh_int log_type, sh_int level )
{
    ...
    fprintf( stderr, "\x1b[0;37m[\x1b[0;32m %s \x1b[0m\x1b :: %s\n", strtime, str );
...
void make_wizlist( )
{
  ...
  sprintf(buf, "\r"
                         " _tOSSSSOS#R#R  RRB@\r"
                       "e#e#SSO~CCCS##RRRB@#R#SR\r"
                      "CS##'CSOtCOSS##RBBB@s7'#SR@\r"
                   "('S##CCSO  SSSRRB@@@@  #SSS#RS#@\r"
                  "~tR@O##S(t(CSSRBB@@@@@  sSR#SSS#S@\r"
                "S~S@   SSSCOSSRRB@@@BB@  @SSSSR OS@#@\r"
               "C #B tOOt' S#RBBB@@@S##RSB        S#@t#@\r"
              "COt##OOC~~SSBB@@@@@BR##R           SSC'#B\r"
              "StSSSOSRB@@@@@@@                 CSROSBORR\r"
             "eOS@@RB@@@@@@ B                #SSSRSRB  SR@\r"
            "COOBB@#@B                         SSR     #S@\r"
           "QOe@@B                                     RS@\r"
           "S#R@@                                       S\r"
              "R#R@                     #CO@@R@@        S@\r"
            "S    # #                eR  S#BRR#SS##t~'OB@\r"
            "S RS                    eS   SRBRRR#SSS#SSB@\r"
            "C#R  Q      Oe    R@    @###            Q#BB\r"
            "##B #SB  S  OS@   eSR     ##S#@@       OtS@\r"
           "s #RB S@  S  eS@   ###R    ##RS#R@      OtRB\r"
             "##SR#B  S  ##R@    ##R@     #R#S#S#S#O B@\r"
               "RRS#@#SR   #R     #S#R@       RSR#SB@@\r"
                "####RSR   S#      #SR#@         #B@\r"
                  "###SRR  ##R@    ###SR#R@@@@O SRB\r"
                   "#RR#RRR  ##@    ##S#R#QeSSS# R\r"
                       "RRRRRRR####e'~eB@SSB@@\r"
                             "R RRBBBR\n\r"
...
/*  towizfile( " Masters of the Realms of Despair!" );*/
  ...
  for ( wiz = first_wiz; wiz; wiz = wiz->next )
  {
	...
	buf[0] = '\0';
      ...
      switch(ilevel)
      {
   case MAX_LEVEL -  0: towizfile( "     &BAmo De La ReaLiDaD&w" );  break;
   case MAX_LEVEL -  1: towizfile( "      &BDioS PSiCoTRoPiCo&w" );     break;
   case MAX_LEVEL -  2: towizfile( "      &BDiOs NaZi&w" );      break;
   case MAX_LEVEL -  3: towizfile( "        &BCoNoCeDoReS De La &yHeReJiA&w" );      break;
   case MAX_LEVEL -  4: towizfile( " Administradores" ); break;
   case MAX_LEVEL -  5: towizfile( " Infinitos" );  break;
   case MAX_LEVEL -  6: towizfile( " Creadores" ); break;
   case MAX_LEVEL -  7: towizfile( " Dioses" );      break;
   case MAX_LEVEL -  8: towizfile( " Dioses Principiantes" );  break;
   case MAX_LEVEL -  9: towizfile( " Imortales" );    break;
   case MAX_LEVEL - 10: towizfile( " SemiDioses" );    break;
   case MAX_LEVEL - 11: towizfile( " Sabios" );      break;
   case MAX_LEVEL - 12: towizfile( " Creadores" );     break;
   case MAX_LEVEL - 13: towizfile( " Acolitos" );     break;
   case MAX_LEVEL - 14: towizfile( " Angeles" );    break;
   case MAX_LEVEL - 15: towizfile( " Avatares" );      break;
   case MAX_LEVEL - 16: towizfile( " Invitados" );    break;
   default:      towizfile( " Colaboradores" );      break;
      }
    }
    if ( strlen( buf ) + strlen( wiz->name ) > 90 )
    {
	towizfile( buf );
	buf[0] = '\0';
    }
...
void load_area_file( AREA_DATA *tarea, char *filename )
{
	...
	else if ( !str_cmp( word, "AREA"     ) )
	{
        ...
        else if ( !str_cmp( word, "CONQUISTABLE" ) ) load_conquistable (tarea, fpArea);
        else if ( !str_cmp( word, "PROPIETARIO" ) ) load_propietario ( tarea, fpArea );
        else if ( !str_cmp( word, "PUNTOS" ) ) load_puntos ( tarea, fpArea ); /* Puntos de cada familia en el area */
        else if ( !str_cmp( word, "REVISADA" ) ) load_revision (tarea, fpArea);
        else if ( !str_cmp( word, "AUTHOR"   ) ) load_author  (tarea, fpArea);
	else if ( !str_cmp( word, "FLAGS"    ) ) load_flags   (tarea, fpArea);
       	else if ( !str_cmp( word, "RECURSOS" )) load_recursos(tarea, fpArea);
		 ...
		 char *loadst, char *notloadst )
{
	...
	pager_printf(ch, "%-15s| Rooms: %5d - %-5d"
		     " Objs: %5d - %-5d Mobs: %5d - %-5d%s Total Rooms:%d\n\r",
		(pArea->filename ? pArea->filename : "(invalid)"),
		pArea->low_r_vnum, pArea->hi_r_vnum,
		pArea->low_o_vnum, pArea->hi_o_vnum,
		pArea->low_m_vnum, pArea->hi_m_vnum,
		IS_SET(pArea->status, AREA_LOADED) ? loadst : notloadst, pArea->room_total );
	count++;
    }
...
void do_check_vnums( CHAR_DATA *ch, char *argument )
{
    ...
    if (low_range < 1 || low_range > 9999999 )
    {
      send_to_char("Invalid argument for bottom of range.\n\r", ch);
      return;
    }

    if (high_range < 1 || high_range > 9999999 )
    {

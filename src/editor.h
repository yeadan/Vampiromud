...
/*. Utils exported from editor.c .*/
void strip_string( char * );
void cleanup_editor( DESCRIPTOR_DATA *d );
...
struct editor_save_info
{
    int zone;
    char type;
    struct editor_save_info *next;
};

/*. Descriptor access macros .*/
#define EDITOR_MODE(d) 	((d)->editor->mode) 	/* Parse input mode	*/
#define EDITOR_NUM(d) 	((d)->editor->number)	/* Room/Obj VNUM 	*/
#define EDITOR_VNUM(d)	EDITOR_NUM(d)
#define EDITOR_VAL(d) 	((d)->editor->value)  	/* Scratch variable	*/
#define EDITOR_OBJ(d)	(obj)
#define EDITOR_DESC(d) 	((d)->editor->desc)	/* Extra description	*/
#define EDITOR_AFF(d)	((d)->editor->paf)		/* Affect data 		*/
#define EDITOR_CHANGE(d)	((d)->editor->changed)	/* Changed flag		*/
#define EDITOR_EXIT(d)     ((d)->editor->xit)		/* An Exit		*/

#ifdef OLD_CIRCLE_STYLE
# define EDITOR_ROOM(d)    ((d)->editor->room)        /* Room structure       */
# define EDITOR_OBJ(d)     ((d)->editor->obj)         /* Object structure     */
# define EDITOR_MOB(d)     ((d)->editor->mob)         /* Mob structure        */
# define EDITOR_SHOP(d)    ((d)->editor->shop)        /* Shop structure       */
# define EDITOR_EXIT(d)	(EDITOR_ROOM(d)->dir_option[EDITOR_VAL(d)])
#endif

/*. Add/Remove save list types	.*/
#define EDITOR_SAVE_ROOM			0
#define EDITOR_SAVE_OBJ			1
#define EDITOR_SAVE_ZONE			2
#define EDITOR_SAVE_MOB			3
#define EDITOR_SAVE_SHOP			4

...
CHAR_DATA *find_keeper( CHAR_DATA *ch )
{
    ...
    {
	do_say( keeper, "Los asesinos no son bienvenidos aqui!" );
	sprintf( buf, "El ASESINO %s esta en mi tienda!\n\r", ch->name );
	do_shout( keeper, buf );
	return NULL;
    }
    ...
    {
	do_say( keeper, "Los ladrones no son bienvenidos aqui!" );
	sprintf( buf, "El LADRON %s esta en mi tienda!\n\r", ch->name );
	do_shout( keeper, buf );
	return NULL;
    }
    ...
    {
	if ( whof == ch )
	    send_to_char( "No creo que sea buena idea...\n\r", ch );
	else
	    do_say( keeper, "Estoy demasiado ocupado!" );
        return NULL;
    }

    if ( who_fighting(ch) )
    {
	ch_printf( ch, "%s esta demasiado asustado para vender ahora.\n\r", PERS(keeper, ch) );
	return NULL;
    }
	...
	{
	    do_say( keeper, "Cerrado, vuelve mas tarde." );
	    return NULL;
	}
    }
    else
	...
	{
	    do_say( keeper, "Cerrado, vuelve manyana." );
	    return NULL;
	}
    }
    ...
    {
        send_to_char( "Como te va a vender algo estando dormido?\n\r", ch );
        return NULL;
    }

    if ( keeper->position < POS_SLEEPING )
    {
        send_to_char( "No parece que pueda oirte...\n\r", ch );
        return NULL;
    }
    ...
    {
	do_say( keeper, "No comercio con gente que no puedo ver." );
	return NULL;
    }
    ...
    {
	if ( speakswell > 60 )
	    sprintf( buf, "%s, podrias repetirme eso? No lo he acabado de pillar.", ch->name );
	else
	if ( speakswell > 50 )
	    sprintf( buf, "%s, podrias hablar un poco mas claro?", ch->name );
	else
	if ( speakswell > 40 )
	    sprintf( buf, "%s, perdona pero... Que es lo que querias?", ch->name );
	else
	    sprintf( buf, "%s, si es que te llamas asi, no consigo entender una palabra de lo que dices.", ch->name );
	do_tell( keeper, buf );
	return NULL;
    }
...
CHAR_DATA *find_fixer( CHAR_DATA *ch )
{
    ...
    */
    ...
    {
	ch_printf( ch, "%s esta demasiado asustado para atenderte ahora.\n\r", PERS(keeper, ch) );
	return NULL;
    }
    ...
    if ( keeper->position == POS_SLEEPING )
    {
        send_to_char( "Como te va a atender estando dormido?\n\r", ch );
        return NULL;
    }
...
int get_repaircost( CHAR_DATA *keeper, OBJ_DATA *obj )
{
	...
	{
	    cost = (int) (obj->cost * rShop->profit_fix / 500);
	    found = TRUE;
	    break;
	}
    }
...
void do_buy( CHAR_DATA *ch, char *argument )
{
	...
	if ( !pRoomIndexNext )
	{
	    ...
	    send_to_char( "No puedes comprar eso aqui.\n\r", ch );
	    return;
	}
	...
	{
	    send_to_char( "Ya has comprado una mascota este nivel.\n\r", ch );
	    return;
	}

	if ( ch->gold < 10 * pet->level * pet->level )
	{
	    send_to_char( "No puedes permitirtelo.\n\r", ch );
	    return;
	}

	if ( ch->level < pet->level )
	{
	    send_to_char( "No estas preparado para domesticar eso!\n\r", ch );
	    return;
	}
...
*/
	...
	sprintf( buf, "%sLleva un collarin que pone 'Pertenezco a %s'.\n\r",
	...
	if ( is_number( arg ) )
	{
	    ...
	    if ( noi > mnoi )
	    {
		act( AT_TELL, "$n te dice telepaticamente 'No te puedo vender mas objetos de momento once.'", keeper, NULL, ch, TO_VICT );
		ch->reply = keeper;
		return;
	    }
	}
	...
	{
	    act( AT_TELL, "$n te dice telepaticamente 'No vendo eso - prueba 'LISTA'.'",
	...
	if ( !IS_OBJ_STAT( obj, ITEM_INVENTORY ) && ( noi > 1 ) )
	{
	    ...
	    act( AT_TELL, "$n te doce telepaticamente 'No tengo suficientes en stock"
	     " para poder vender mas de uno cada vez.'", keeper, NULL, ch, TO_VICT );
	    ch->reply = keeper;
	    return;
	}
	...
	if ( ch->gold < cost )
	{
	    act( AT_TELL, "$n te dice telepaticamente 'No te puedes permitir comprar $p.'",
	...
	{
	    act( AT_TELL, "$n te dice telepaticamente 'No puedes usar $p todavia.'",
		keeper, obj, ch, TO_VICT );
	    ch->reply = keeper;
	    return;
	}
	...
	{
	    act( AT_TELL, "$n te dice telepaticamente 'Es solo un prototipo! No puedo vendertelo...'",
		keeper, NULL, ch, TO_VICT );
      	    ch->reply = keeper;
	    return;
	}
	...
	{
	    send_to_char( "No puedes llevar mas objetos.\n\r", ch );
	    return;
	}
	...
	if ( noi == 1 )
	{
	    act( AT_ACTION, "$n compra $p.", ch, obj, NULL, TO_ROOM );
    	    act( AT_ACTION, "Compras $p.", ch, obj, NULL, TO_CHAR );
	}
        else
	{
	    sprintf( arg, "$n compra %d $p%s.", noi,
		( obj->short_descr[strlen(obj->short_descr)-1] == 's'
		? "" : "s" ) );
	    act( AT_ACTION, arg, ch, obj, NULL, TO_ROOM );
	    sprintf( arg, "Compras %d $p%s.", noi,
		( obj->short_descr[strlen(obj->short_descr)-1] == 's'
		? "" : "s" ) );
	    act( AT_ACTION, arg, ch, obj, NULL, TO_CHAR );
	    act( AT_ACTION, "$N lo pone en una bolsa y te lo da.",
	...
	if ( keeper->gold > maxgold )
	{
	    ...
	    act( AT_ACTION, "$n pone algo de oro en manos de los dioses.", keeper, NULL, NULL, TO_ROOM );
	}
    ...
    if ( IS_SET(ch->in_room->room_flags, ROOM_PET_SHOP) )
    {
                ...
                pet = pet->next_in_room )
        {
            if ( xIS_SET(pet->act, ACT_PET) && IS_NPC(pet) )
            {
                ...
                {
                    found = TRUE;
                    send_to_pager( "Mascotas a la venta:\n\r", ch );
                }
        ...
        if ( !found )
            send_to_char( "Lo siento, se nos han acabado las mascotas por ahora.\n\r", ch );
        return;
    }
    else
    {
...
void do_sell( CHAR_DATA *ch, char *argument )
{
    ...
    {
	act( AT_TELL, "$n te dice telepaticamente 'No tienes eso.'",
		keeper, NULL, ch, TO_VICT );
	ch->reply = keeper;
	return;
    }
    ...
    if ( obj->timer > 0 )
    {
	act( AT_TELL, "$n te dice telepaticamente, '$p se deteriora demasiado rapido...'", keeper, obj, ch, TO_VICT );
	return;
    }

    if ( ( cost = get_cost( ch, keeper, obj, FALSE ) ) <= 0 )
    {
	act( AT_ACTION, "$n mira desinteresado $p.", keeper, obj, ch, TO_VICT );
	return;
    }

    if ( cost >= keeper->gold )
    {
	act( AT_TELL, "$n te dice telepaticamente, 'No me puedo permitir $p...'", keeper, obj, ch, TO_VICT );
	return;
    }

    separate_obj( obj );
    act( AT_ACTION, "$n vende $p.", ch, obj, NULL, TO_ROOM );
    sprintf( buf, "Vendes $p por %d monedas de oro%s.",
	cost, cost == 1 ? "" : "" );
...
void do_value( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "$n te dice telepaticamente 'Te dare %d monedas de oro por $p.'", cost );
                 ...
                 char *arg, int maxgold, char *fixstr, char*fixstr2 )
{
   ...
   if ( !can_drop_obj( ch, obj ) )
       ch_printf( ch, "No puedes deshacerte de %s.\n\r", obj->name );
   else if ( ( cost = get_repaircost( keeper, obj ) ) < 0 )
   {
       if (cost != -2)
       act( AT_TELL, "$n te dice telepaticamente, 'Lo siento, No puedo hacer nada por $p.'",
            keeper, obj, ch, TO_VICT );
       else
	  act( AT_TELL, "$n te dice telepaticamente, 'No parece que $p necesite ser reparado'", keeper, obj, ch, TO_VICT );
   }
   ...
   {
      sprintf( buf,
       "$N te dice telepaticamente, 'Te costara %d moneda%s de oro %s %s...'", cost,
        cost == 1 ? "" : "s", fixstr, obj->name );
      act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
      act( AT_TELL, "$N te dice telepaticamente, 'No puedes permitirtelo.'", ch,
              NULL, keeper, TO_CHAR );
   }
   else
   {
      sprintf( buf, "$n da $p a $N, quien rapidamente lo %s.", fixstr2 );
      act( AT_ACTION, buf, ch, obj, keeper, TO_ROOM );
      sprintf( buf, "Le entregas %d moneda%s de oro a $N, %s $p.",
      ...
      if ( keeper->gold > maxgold )
      {
          boost_economy( keeper->in_room->area, keeper->gold - maxgold/2 );
          keeper->gold = maxgold/2;
          act( AT_ACTION, "$n envia algo de dinero a los dioses (que pasa la play 2 es muy cara :P).", keeper,
		NULL, NULL, TO_ROOM );
      }
      ...
      {
          default:
            send_to_char( "Por alguna razon piensas que no es una buena idea...\n\r", ch);
            break;
          case ITEM_ARMOR:
...
void do_repair( CHAR_DATA *ch, char *argument )
{
    ...
    maxgold = keeper->level * keeper->level * 10000;
    ...
    if ( !strcmp( argument, "todo" ) || !strcmp( argument, "all" ) )
    {
...
void appraise_all( CHAR_DATA *ch, CHAR_DATA *keeper, char *fixstr )
{
    ...
    for ( obj = ch->first_carrying; obj != NULL ; obj = obj->next_content )
    {
        ...
        ||   obj->item_type == ITEM_STAFF ) )
            ...
            if ( ( cost = get_repaircost( keeper, obj ) ) < 0 )
            {
               if (cost != -2)
               act( AT_TELL,
                    "$n te dice telepaticamente, 'Lo siento, no puedo hacer nada con $p.'",
                    keeper, obj, ch, TO_VICT );
               else
               act( AT_TELL, "$n te dice telepaticamente, '$p parece estar bien para mi!'",
                    keeper, obj, ch, TO_VICT );
            }
            ...
            {
            sprintf( buf,
            "$N te dice telepaticamente, 'Te costara %d moneda%s de oro %s %s'",
    ...
    if ( total > 0 )
    {
       send_to_char ("\n\r", ch);
       sprintf( buf,
          "$N te dice telepaticamente, 'Te costara %d moneda%s de oro en total.'",
          total, cost == 1 ? "" : "s" );
       act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
       strcpy( pbuf,
       "$N te dice telepaticamente, 'Recuerda que hay un recargo del 10% por repararlo todo :).'");
       act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
    }
}
...
void do_appraise( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Evaluar reparar que?\n\r", ch );
	return;
    }
    ...
    if ( !strcmp( arg, "all") || !strcmp( arg, "todo") )
    {
    ...
    {
      if (cost != -2)
	act( AT_TELL, "$n te dice telepaticamente, 'Lo siento, no puedo hacer nada con $p.'", keeper, obj, ch, TO_VICT );
      else
	act( AT_TELL, "$n te dice telepaticamente, '$p parece bien para mi!'", keeper, obj, ch, TO_VICT );
      return;
    }

    sprintf( buf,
       "$N te dice telepaticamente, 'Te costara %d moneda%s de oro %s eso...'", cost,
       cost == 1 ? "" : "s", fixstr );
    act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
    if ( cost > ch->gold )
      act( AT_TELL, "$N te dice telepaticamente, 'No puedes permitirtelo.'", ch,

...
const struct map_info_type map_info[] =
{
    { SECT_BLOCKED,      "&p",   "~&w", "",   "blokeado" },
    { SECT_UNSEEN,       "&w&z", " &w", "",   "desconocido" },
    { SECT_HERE,         "&w&W", "*&w", "",   "Este eres tu!" },
    { SECT_INSIDE,       "^z&W", " &w", "",   "dentro" },
    { SECT_CITY,         "^w&W", " &w", "",   "ciudad" },
    { SECT_FIELD,        "^G&g", " &w", "",   "campo" },
    { SECT_FOREST,       "^g&G", " &w", "&W", "bosque" },
    { SECT_HILLS,        "^Y&O", " &w", "&W", "colinas" },
    { SECT_MOUNTAIN,     "^r&Y", " &w", "",   "montanyas" },
    { SECT_WATER_SWIM,   "^B&b", " &w", "",   "shallow water" },
    { SECT_WATER_NOSWIM, "^b&B", " &w", "",   "deep running water" },
    { SECT_AIR,          "^c&C", " &w", "",   "aire" },
    { SECT_DESERT,       "^O&Y", " &w", "",   "desierto" },
    { SECT_UNDERWATER,   "^p&C", " &w", "",   "bajo el agua" },
    { SECT_DUNNO,        "&P",   " &w", "",   "dunas" },
    { SECT_OCEANFLOOR,   "^w&z", " &w", "",   "oceano" },
    { SECT_UNDERGROUND,  "^w&z", " &w", "",   "bajo tierra" },
...
void ShowMap( CHAR_DATA *ch, int min, int max, int size, int center )
{
    ...
    send_to_char( borderbuf, ch );
    send_to_char( "\n\r", ch );
    send_to_char( "Si no entiedes na, mapa leyenda.\n\r", ch );
    return;
}
...
void do_mapper( CHAR_DATA *ch, char *argument )
{
    ...
    if( str_prefix( arg1, "leyenda ayuda" ) )
    {
        ...
        size = ( is_number(  arg1 ) ) ? atoi (arg1) : 21;
	if ( size >= 22 )
	  size = 21;
    ...
    if ( size != 7 )
    {
        size = IS_IMMORTAL( ch ) ? size :
            get_curr_int( ch )/2 +
            ( ( !IS_NPC( ch ) ) ? ch->pcdata->learned[gsn_scan]/25 : 0 );
        if ( size % 2  == 0 )
            size +=1;

...
int exp_level( CHAR_DATA *ch, sh_int level )
{
   ...
   {
   if( ES_REMORT(ch))
   return (( 2000 + ( ch->pcdata->renacido * 1000 )) * lvl + get_exp_base(ch));
   else
   return ( ( 2000 * lvl + get_exp_base(ch) ));
   }
   else
   return ( ( 2000 * lvl + get_exp_base(ch) ));
}
...
/*
 * Encuentra a un mob en Mundo de Tinieblas por un VNUM
 */
...
/*
 * Encuentra un objeto en Mundo de Tinieblas por un VNUM
 */
...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd )
{
	...
	if ( !IS_NPC(ch) )
		...
		URANGE( 0, ch->pcdata->condition[COND_BLOODTHIRST] + mod, ch->level+10 + (13 / ch->generacion) * 40 );
	break;
...
/* spell cast upon wear/removal of an object	-Thoric */
	...
	{
	    depth++;
	    act( AT_ACTION, "$p pesa demasiado para seguir blandiendolo.",
		ch, wield, NULL, TO_CHAR );
	    act( AT_ACTION, "$n deja de blandir $p.", ch, wield, NULL, TO_ROOM );
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
	...
	if ( loading_char != ch )
	{
	   act( AT_MAGIC, "$p te da un calambrazo y se cae al suelo.", ch, obj, NULL, TO_CHAR );
	   act( AT_MAGIC, "$n es rechazado por $p, cayendose al suelo.",  ch, obj, NULL, TO_ROOM );
	}
	if ( obj->carried_by )
	   obj_from_char( obj );
...
CHAR_DATA *get_char_world( CHAR_DATA *ch, char *argument )
{
	...
	/* Añadido por SiGo para que no haga CRASH con la gente en invis o ke el jugador no puede ver
         * en el momento de ser abrazado
	 * SiGo email to sigo@vampiromud.com 24/07/2002 */
	else
...
bool ms_find_obj( CHAR_DATA *ch )
{
	...
	{
	    default:
	    case  1: t="Solo un segundo...\n\r";					break;
	    case  2: t="No puedes encontrarlo...\n\r";					break;
...
OBJ_DATA *find_obj( CHAR_DATA *ch, char *argument, bool carryonly )
{
	...
	if ( !obj )
		...
		"No veo nada parecido a eso en $p." :
		"No veo nada parecido a eso en $p.",
...
/* Deadlies link-dead over 2 ticks aren't seen by mortals -- Blodkai */
    ...
    /* Jugadores con Ofuscacion Capa sombria */

    if( xIS_SET(victim->afectado_por, DAF_CAPA_SOMBRIA))
    {
    	if(!IS_VAMPIRE(ch))
       	{
       		if ( IS_AFFECTED(ch, AFF_TRUESIGHT) )
	...
	{
     		if((!xIS_SET(ch->afectado_por, DAF_SENTIDOS_AGUZADOS) && ch != victim))
        	{
        		if(!IS_IMMORTAL(ch))
        			return FALSE;
        	}
...
bool can_see_obj( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    if (IS_VAMPIRE(ch) && xIS_SET(ch->afectado_por, DAF_SENTIDOS_AGUZADOS))
		return TRUE;
...
char *affect_loc_name( int location )
{
    ...
    case APPLY_STR:		return "la fuerza";
    case APPLY_DEX:		return "la destreza";
    case APPLY_INT:		return "la inteligencia";
    case APPLY_WIS:		return "la sabiduria";
    case APPLY_CON:		return "la constitucion";
    case APPLY_CHA:		return "el carisma";
    case APPLY_LCK:		return "la suerte";
    case APPLY_SEX:		return "el sexo";
    case APPLY_CLASS:		return "la clase";
    case APPLY_LEVEL:		return "el nivel";
    case APPLY_AGE:		return "la edad";
    case APPLY_MANA:		return "el mana";
    case APPLY_HIT:		return "la vida";
    case APPLY_MOVE:		return "el movimiento";
    case APPLY_GOLD:		return "el oro";
    case APPLY_EXP:		return "la experiencia";
    case APPLY_AC:		return "la armadura";
    case APPLY_HITROLL:		return "el hitroll";
    case APPLY_DAMROLL:		return "el damroll";
    case APPLY_SAVING_POISON:	return "save vs veneno";
    case APPLY_SAVING_ROD:	return "save vs rod";
    case APPLY_SAVING_PARA:	return "save vs paralisis";
    case APPLY_SAVING_BREATH:	return "save vs alientos";
    case APPLY_SAVING_SPELL:	return "save vs conjuros";
    case APPLY_HEIGHT:		return "la altura";
    case APPLY_WEIGHT:		return "el peso";
    case APPLY_AFFECT:		return "algo";
    case APPLY_RESISTANT:	return "tu resistencia";
    case APPLY_IMMUNE:		return "tu inmunidad";
    case APPLY_SUSCEPTIBLE:	return "tu susceptibilidad";
    case APPLY_BACKSTAB:	return "backstab";
    case APPLY_PICK:		return "forzar cerraduras";
    case APPLY_TRACK:		return "cazar";
    case APPLY_STEAL:		return "robar";
    case APPLY_SNEAK:		return "el sigilo";
    case APPLY_HIDE:		return "ocultarse";
    case APPLY_PALM:		return "palm";
    case APPLY_DETRAP:		return "detrap";
    case APPLY_DODGE:		return "esquivar";
    case APPLY_PEEK:		return "peek";
    case APPLY_SCAN:		return "otear";
    case APPLY_GOUGE:		return "gouge";
    case APPLY_SEARCH:		return "search";
    case APPLY_MOUNT:		return "mount";
    case APPLY_DISARM:		return "desarmar";
    ...
    case APPLY_WEAPONSPELL:	return "conjuro de arma";
    case APPLY_WEARSPELL:	return "conjuro de equipo";
    case APPLY_REMOVESPELL:	return "remove spell";
    case APPLY_MENTALSTATE:	return "el estado mental";
    case APPLY_EMOTION:		return "el estado emocional";
    case APPLY_STRIPSN:		return "dispel";
    case APPLY_REMOVE:		return "remove";
    case APPLY_DIG:		return "dig";
    case APPLY_FULL:		return "el hambre";
    case APPLY_THIRST:		return "la sed";
    case APPLY_DRUNK:		return "la embriaguez";
    case APPLY_BLOOD:		return "la sangre";
...
char *affect_bit_name( EXT_BV *vector )
{
    ...
    buf[0] = '\0';
    if ( xIS_SET(*vector, AFF_BLIND        )) strcat( buf, " ceguera"         );
    if ( xIS_SET(*vector, AFF_INVISIBLE    )) strcat( buf, " invisible"     );
    if ( xIS_SET(*vector, AFF_DETECT_EVIL  )) strcat( buf, " detectar_mal"   );
    if ( xIS_SET(*vector, AFF_DETECT_INVIS )) strcat( buf, " detectar_invisible"  );
    if ( xIS_SET(*vector, AFF_DETECT_MAGIC )) strcat( buf, " detectar_magia"  );
    if ( xIS_SET(*vector, AFF_DETECT_HIDDEN)) strcat( buf, " detectar_oculto" );
    ...
    if ( xIS_SET(*vector, AFF_POISON       )) strcat( buf, " veneno"        );
...
char *extra_bit_name( EXT_BV *extra_flags )
{
    ...
    if ( xIS_SET(*extra_flags, ITEM_QUEST)   ) strcat( buf, " quest"     );
...
/* asignacion automatica de disciplinas del jugador */

void asignar_disciplinas (CHAR_DATA *ch)
{
    int i;

     for (i = 0 ; i < MAX_DISCIPLINAS ; i++)
	ch->pcdata->habilidades[i] = -2;

    if (IS_VAMPIRE(ch) )
    {
	if( ch->pcdata->habilidades[DISCIPLINA_ANIMALISMO] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_ANIMALISMO] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_AUSPEX] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_AUSPEX] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_BARDO] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_BARDO] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_CELERIDAD] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_CELERIDAD] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_CONOCIMIENTO] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_CONOCIMIENTO] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_DAIMOINON] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_DAIMOINON] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_DEMENTACION] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_DEMENTACION] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_DOMINACION] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_DOMINACION] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_EXTINCION] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_EXTINCION] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_KINETISMO] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_KINETISMO] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_FORTALEZA] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_FORTALEZA] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_MELOPOMENE] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_MELOPOMENE] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_MORTIS] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_MORTIS] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_MYTHECERIA] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_MYTHECERIA] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_NIGROMANCIA] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_NIGROMANCIA] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_NIHILISTICA] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_NIHILISTICA] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_OBEAH] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_OBEAH] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_OBTENEBRACION] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_OBTENEBRACION] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_OFUSCACION] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_OFUSCACION] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_PRESENCIA] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_PRESENCIA] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_POTENCIA] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_POTENCIA] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_PROTEAN] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_PROTEAN] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_QUIMERISMO] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_QUIMERISMO] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_SANGUINUS] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_SANGUINUS] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_SPIRITUS] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_SPIRITUS] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_TANATOSIS] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_TANATOSIS] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_TEMPORIS] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_TEMPORIS] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_TAUMATURGIA] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_TAUMATURGIA] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_DARKTARMA] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_DARKTARMA] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_VICISITUD] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_VICISITUD] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_VISCERATIKA] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_VISCERATIKA] = 0;
                 if( ch->pcdata->habilidades[DISCIPLINA_SERPENTIS] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_SERPENTIS] = 0;
	if( ch->pcdata->habilidades[DISCIPLINA_COMUN] < 0 )
            ch->pcdata->habilidades[DISCIPLINA_COMUN] = 0;
    }

    ch->pcdata->habilidades[class_table[ch->class]->disci_class1] +=2; /* Disciplina de clan */
    ch->pcdata->habilidades[class_table[ch->class]->disci_class2] +=1; /* Disciplina secundaria */
    ch->pcdata->habilidades[class_table[ch->class]->disci_class3] +=1; /* Disciplina terciaria */
    ch->pcdata->habilidades[34] +=1; /* Disciplina comun de alimentacion */
...
ch_ret spring_trap( CHAR_DATA *ch, OBJ_DATA *obj )
{
	 ...
	 txt = "surrounded by a mysterious aura";		break;
      ...
      sprintf( buf, "Estas %s!", txt );
      act( AT_HITME, buf, ch, NULL, NULL, TO_CHAR );
      sprintf( buf, "$n esta %s.", txt );
...
void showaffect( CHAR_DATA *ch, AFFECT_DATA *paf )
{
	...
	if ( paf->location != APPLY_NONE && paf->modifier != 0 )
	{
	    ...
	    {
	      default:
		sprintf( buf, "Afecta %s en %d.\n\r",
		  affect_loc_name( paf->location ), paf->modifier );
		break;
	      case APPLY_AFFECT:
		sprintf( buf, "Afecta %s en",
...
int get_gen_worth( CHAR_DATA *ch )
{
    int gen;
    if ( ch->generacion == MAX_GENERACION )
    {
    gen = ch->generacion + 10 /2 ;

    return ch->gen;
    }
    if ( ch->generacion == GENERACION_DECIMOSEGUNDA )
    {
    gen = ch->generacion + 33 /2;

    return ch->gen;
    }
    if ( ch->generacion == GENERACION_DECIMOPRIMERA )
    {
    gen = ch->generacion + 58 /2;

    return ch->gen;
    }
    if ( ch->generacion == GENERACION_DECIMA )
    {
    gen = ch->generacion + 82 /2;

    return ch->gen;
    }
    if ( ch->generacion == GENERACION_NOVENA )
    {
    gen = ch->generacion + 106 /2;

    return ch->gen;
    }
    if ( ch->generacion == GENERACION_OCTAVA )
    {
    gen = ch->generacion + 130 /2;

    return ch->gen;
    }
    if ( ch->generacion == GENERACION_SEPTIMA )
    {
    gen = ch->generacion + 154 /2;

    return ch->gen;
    }
    if ( ch->generacion == GENERACION_SEXTA )
    {
    gen = ch->generacion + 178 /2;

    return ch->gen;
    }
    if ( ch->generacion == GENERACION_QUINTA )
    {
    gen = ch->generacion + 202 /2;

    return ch->gen;
    }
    if ( ch->generacion == GENERACION_MATUSALEN )
    {
    gen = ch->generacion + 10000;

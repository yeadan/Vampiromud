...
void do_authorize( CHAR_DATA *ch, char *argument )
{
  ...
  if ( arg1[0] == '\0' )
     {
     send_to_char( "Uso:  autorizar <jugador> <yes|name|immsim|mobsim|swear|plain|unpronu|no/deny>\n\r", ch );
     send_to_char( "Pendientes de autorizacion:\n\r", ch );
     send_to_char( " Elijiendo nombre del personaje\n\r", ch );
     send_to_char( "---------------------------------------------\n\r", ch );
     for ( d = first_descriptor; d; d = d->next )
         if ( (victim = d->character) != NULL && IS_WAITING_FOR_AUTH(victim) )
            ch_printf( ch, " %s@%s nuevo %s %s (%s)...\n\r",
               victim->name,
       victim->desc->host,
               race_table[victim->race]->race_name,
               class_table[victim->class]->who_name,
               IS_PKILL( victim ) ? "Matable" : "Pacifico" );
    return;
    }
  ...
  if ( arg2[0]=='\0' || !str_cmp( arg2,"accept" ) || !str_cmp( arg2,"yes" ))
     {
        ...
        sprintf( buf, "%s: autorizado", victim->name);
        ...
        "\n\r&GLos ADMINISTRAODRES del MUD han aceptado el nombre %s.\n\r"
        "Estas autorizado a entrar en Vampiro Edad Oscura agradece a los"
        "DIOSES su misericordia para contigo.\n\r", victim->name);
     ...
     else if ( !str_cmp( arg2, "immsim" ) || !str_cmp( arg2, "i" ) )
     {
       ...
       sprintf( buf, "%s: nombre denegado, similar al de un inmortal del juego", victim->name);
       to_channel( buf, CHANNEL_AUTH, "Auth", LEVEL_NEOPHYTE );

       send_to_char_color(
        "&REl nombre que has elejido es similar al de un INMORTAL actual. \n\r"
        "Por favor elije otro nombre con el comando nombre.\n\r", victim);
     ...
     {
       victim->pcdata->auth_state = 2;
       sprintf( buf, "%s: nombre denegado - similar al nombre de un mob", victim->name);
   to_channel( buf, CHANNEL_AUTH, "Auth", LEVEL_NEOPHYTE );

       send_to_char_color(
        "&REl nombre que has elejido es similar al de alguno de los\n\r"
        "monstruos del juego, a la larga crearia problemas por lo tanto\n\r"
        "nos vemos incapaces de autorizarlo. Por favor elije otro nombre\n\r"
        "usando el comando nombre.\n\r", victim);
       ch_printf( ch, "You requested %s change names.\n\r", victim->name);
       return;
       }
       ...
       {
         victim->pcdata->auth_state = 2;
         sprintf( buf, "%s: nombre denegado - palabra clave", victim->name);
         to_channel( buf, CHANNEL_AUTH, "Auth", LEVEL_NEOPHYTE );

         send_to_char_color(
          "&RNo podemos autorizar tu nombre por ser como palabras clave, en algun tipo.\n\r"
          "de lenguaje del juego.Por favor, elije otro nombre usando el comando nombre.\n\r",victim);
         ch_printf( ch, "You requested %s change names.\n\r", victim->name);
         return;
       }
...
void do_bamfin( CHAR_DATA *ch, char *argument )
{
   ...
   send_to_char_color( "&YBamfin fijado.\n\r", ch );
    }
    return;
}
...
void do_bamfout( CHAR_DATA *ch, char *argument )
{
   ...
   send_to_char_color( "&YBamfout fijado.\n\r", ch );
    }
    return;
}
...
void do_fquit( CHAR_DATA *ch, char *argument )
{
  ...
  if ( victim->level != 1 )
  {
     send_to_char( "No es nivel uno!\n\r", ch );
     return;
  }
  set_char_color( AT_IMMORT, victim );
  send_to_char( "Sientes ganas de hacer SALIR...\n\r", victim );
  if ( victim->fighting )
...
void do_pardon( CHAR_DATA *ch, char *argument )
{
    ...
    {
   send_to_char( "Sintaxis: perdonar <jugador> <pktotal|killer|thief|attacker>.\n\r", ch );
	return;
    }
   ...
   send_to_char( "No en jugadores.\n\r", ch );
    ...
    if ( !str_cmp( arg2, "pktotal" ) )
    {
    	if ( xIS_SET(victim->act, PLR_PKTOTAL) )
    	{
    		ch->cnt_pktotal =0;
    		xREMOVE_BIT( victim->act, PLR_PKTOTAL );
    		ch_printf( ch, "Flag de PKTOTAL eliminado a %s.\n\r", victim->name );
    		set_char_color( AT_IMMORT, victim );
    		send_to_char( "Ya no eres un PKTOTAL.\n\r", victim );
    	}
    	return;
    }
	...
	if ( xIS_SET(victim->act, PLR_ATTACKER) )
	{
       ...
       ch_printf( ch, "Flag de atacante eliminado a %s.\n\r", victim->name );
	    set_char_color( AT_IMMORT, victim );
       send_to_char( "Ya no eres un ATACANTE.\n\r", victim );
	}
	return;
    }
...
void do_transfer( CHAR_DATA *ch, char *argument )
{
    ...
    argument = one_argument( argument, arg2 );
    if ( arg1[0] == '\0' ) {
   send_to_char( "Transferir a quien (y a donde)?\n\r", ch );
	return;
    }
    if ( !str_cmp( arg1, "todos" ) && get_trust(ch) >= LEVEL_GREATER)
    {
	for ( d = first_descriptor; d; d = d->next )
	{
	...
	{
       send_to_char( "No existe esa localizacion o ese Vnum.\n\r", ch );
	    return;
	}
	if ( room_is_private( location ) && get_trust( ch ) < sysdata.level_override_private )
	{
       send_to_char( "Lo siento esa habitacion tiene flag private ahora mismo.\n\r", ch );
	    return;
	}
    }
    if ( ( victim = get_char_world( ch, arg1 ) ) == NULL )
   ...
   send_to_char( "No esta aqui.\n\r", ch );
    ...
    {
   send_to_char( "Aun no esta autorizado para que le hagas un Trasfer!\n\r", ch);
	return;
    }
    if ( !victim->in_room )
    {
   send_to_char( "No tiene una localizacion fisica!\n\r", ch );
	return;
    }
    ...
    {
       pager_printf( ch,
          "Lo siento. %s no deberia de ser molestado.\n\r", victim->name);
       pager_printf( victim,
          "Tu DND flag ha hecho fallar un transfer de %s.\n\r", ch->name);
       return;
    }
    /* end of modification                                         */
    ...
    if ( victim->fighting )
	stop_fighting( victim, TRUE );
    act( AT_MAGIC, "$n desaparece en una explosion de sangre.", victim, NULL, NULL, TO_ROOM );
    victim->retran = victim->in_room->vnum;
    char_from_room( victim );
    char_to_room( victim, location );
    act( AT_MAGIC, "$n llega desde una explosion de sangre.", victim, NULL, NULL, TO_ROOM );
    if ( ch != victim )
    ...
    &&   !in_hard_range( victim, location->area ) )
      act( AT_DANGER, "Precaucion:  el nivel del jugador no concuerda con el nivel del area.", ch, NULL, NULL, TO_CHAR );
}
...
void do_retran( CHAR_DATA *ch, char *argument )
{
	...
	if ( arg[0] == '\0' )
	{
      send_to_char("Retransferir a quien?\n\r", ch );
		return;
	}
	if ( !(victim = get_char_world(ch, arg)) )
	{
...
void do_ostat( CHAR_DATA *ch, char *argument )
{
	...
	ch_printf_color( ch, "&cAfecta &w%s &cen &w%d. (extra)\n\r",
	    affect_loc_name( paf->location ), paf->modifier );
    for ( paf = obj->pIndexData->first_affect; paf; paf = paf->next )
	ch_printf_color( ch, "&cAfecta &w%s &cen &w%d.\n\r",
	    affect_loc_name( paf->location ), paf->modifier );
    return;
}
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
	...
	victim->sex == SEX_FEMALE  ? "mujer" : "neutro",
    ...
    if ( ( IS_VAMPIRE(victim) && !IS_NPC(victim) ) )
    {
      sprintf( bdbuf, "%d/%d", victim->pcdata->condition[COND_BLOODTHIRST],
        40 + victim->level + (13 / victim->generacion) * 10 );
    ...
    if( xIS_SET(ch->act, PLR_ALTERADO) )
        pager_printf( ch, "&cAlterado:  &RPermanecera alterado durante &w%d&R minutos.\n\r", victim->pcdata->alterado );
    if ( IS_NPC( victim ) )
	...
	pager_printf_color( ch, "&cPcflags    : &w%s\n\r", flag_string( victim->pcdata->flags, pc_flags ) );
        if ( !xIS_EMPTY(victim->esferas))
        pager_printf( ch, "&cEsferas    : &w%s\n\r", ext_flag_string(&victim->esferas, esferas ) );
        if( IS_VAMPIRE(victim))
	        if( !xIS_EMPTY(victim->afectado_por))
        		pager_printf( ch, "&cAffect de disciplina : &w%s\n\r",ext_flag_string( &victim->afectado_por, dis_affect ) );
    ...
    sprintf(texto, "No es un automob");
    if(victim->dificultad == 1)
    sprintf(texto, "Es Facil");
    if(victim->dificultad == 2)
    sprintf(texto, "Es Normal");
    if(victim->dificultad == 3)
    sprintf(texto, "Es Duro");
    if(victim->dificultad == 4)
    sprintf(texto, "Es Dificil");
    if(victim->dificultad == 5)
    sprintf(texto, "Es Complicado");
    if(victim->dificultad == 6)
    sprintf(texto, "Es Para ir en grupo");
    if(victim->dificultad >= 7)
    sprintf(texto, "Es Casi inmortal");
    ch_printf( ch, "&cDificultad: (&w%d&c)&w%s\n\r", victim->dificultad, texto );
    }
...
void do_gwhere( CHAR_DATA *ch, char *argument )
{
    ...
    if ( low < 1 || high < low || low > high || high > 250 )
    {
...
void do_reboot( CHAR_DATA *ch, char *argument )
{
    ...
    if ( auction->item )
	do_auction( ch, "stop");
    sprintf( buf, "Reboot por %s.", ch->name );
    do_echo( ch, buf );
...
void do_switch( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Switchearte dentro de quien?\n\r", ch );
	return;
    }
    if ( !ch->desc )
	return;
    if ( ch->desc->original )
    {
	send_to_char( "Ahora sigues switcheado.\n\r", ch );
	return;
    }
    if ( ( victim = get_char_world( ch, arg ) ) == NULL )
    {
    ...
    if ( victim->desc )
    {
	send_to_char( "Personaje en juego.\n\r", ch );
	return;
    }
    if ( !IS_NPC(victim) && ch->level < LEVEL_GREATER )
    {
	send_to_char( "No puedes switchear a un jugador!\n\r", ch );
	return;
    }
    if ( victim->switched )
    {
       send_to_char( "No puedes switchear a un jugador ya switcheado!\n\r",ch);
       return;
    }
...
void do_return( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "Vuelves a tu cuerpo.\n\r", ch );
...
void do_minvoke( CHAR_DATA *ch, char *argument )
{
	...
	if ( vnum == -1 )
	{
	    send_to_char( "No existe ese mob.\n\r", ch );
	    return;
	}
    }
    else
	...
	if ( IS_NPC(ch) )
	{
	  send_to_char( "Ke dices?\n\r", ch );
	  return;
	}
	if ( !ch->pcdata || !(pArea=ch->pcdata->area) )
	{
	  send_to_char( "Necesitas tener un area asignada para invocar a ese mob.\n\r", ch );
	  return;
	}
...
void do_advance( CHAR_DATA *ch, char *argument )
{
  ...
  /*You can demote yourself but not someone else at your own trust.-- Narn*/
    ...
    if ( ( level = atoi( arg2 ) ) < 1 || level > MAX_LEVEL ) {
        send_to_char( "Level range is 1 to 250.\n\r", ch );
        return;
    }
    ...
    for ( iLevel = victim->level ; iLevel < level; iLevel++ )
    {
        if (level < LEVEL_IMMORTAL)
          send_to_char( "Subes un nivel!!\n\r", victim );
        victim->level += 1;
        victim->pcdata->max_level += 1; 
        advance_level( victim );
    }
...
void do_trust( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "Level must be 0 (reset) or 1 to 250.\n\r", ch );
	return;
    }
    if ( level > get_trust( ch ) ) {
...
void do_restore( CHAR_DATA *ch, char *argument )
{
	...
	CHAR_DATA *vch_next;
        ...
        if ( get_trust( ch ) < LEVEL_SUB_IMPLEM )
        {
 	    ...
 	    return;
          ...
          else
          {
            ...
            {
              send_to_char( "Lo siento aun no puedes hacer otro restore.\n\r", ch );
              do_restoretime( ch, "" );
              return;
            }
          }
        ...
        save_char_obj( ch );
        send_to_char( "Empezando 'restore all' ...\n\r", ch);
	for ( vch = first_char; vch; vch = vch_next )
	{
		...
		vch->pcdata->condition[COND_BLOODTHIRST] = (40 + vch->level + (13 / vch->generacion) * 10);
		update_pos (vch);
    ...
    victim->move = victim->max_move;
    if ( victim->pcdata )
      victim->pcdata->condition[COND_BLOODTHIRST] = (10 + victim->level + (13 / victim->generacion) * 40);
    update_pos( victim );
    if ( ch != victim )
...
/* KAYSER para comando de imm nuevo */


void do_noaffects ( CHAR_DATA *ch, char *argument)
{
	char arg[MAX_INPUT_LENGTH];
	CHAR_DATA *victim;
	long critical = 0;
	...
	one_argument( argument, arg );
 	if ( arg[0] == '\0' )
 	{
		send_to_char( "Quitar los affects a quien?\n\r", ch);
		return;
	}
	if ( ( victim = get_char_world( ch, arg ) ) == NULL )
	{
	...
	/* Para quitar affects de disciplinas */
...
void do_noemote( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( victim ) )
    {
	send_to_char( "Solo en jugadores.\n\r", ch );
	return;
    }
    if ( get_trust( victim ) >= get_trust( ch ) )
    {
	...
	send_to_char( "Ya puedes expresar emociones.\n\r", victim );
	ch_printf( ch, "NOEMOTE quitado a %s.\n\r", victim->name );
    }
    else
    {
	xSET_BIT(victim->act, PLR_NO_EMOTE);
	send_to_char( "No puedes expresar emociones!\n\r", victim );
	ch_printf( ch, "NOEMOTE puesto a %s.\n\r", victim->name );
    }
    return;
}
...
void do_force( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' || argument[0] == '\0' )
    {
	send_to_char( "Forzar a kien a ke?\n\r", ch );
	return;
    }
	...
	CHAR_DATA *vch_next;
        ...
        if ( mobsonly )
        {
	  send_to_char( "Forzar a quien a que?\n\r", ch );
	  return;
        }
	...
	for ( vch = first_char; vch; vch = vch_next )
	{
	    ...
	    {
		act( AT_IMMORT, "Sientes la necesidad de $t.", ch, argument, vch, TO_VICT );
	...
	{
	    send_to_char( "Mejor sera ke no lo molestes!\n\r", ch );
	    return;
	}
    ...
    {
	send_to_char("No puedes decirle que haga eso!\n\r", ch );
	return;
    }
    act( AT_IMMORT, "Sientes la necesidad de $t.", ch, argument, victim, TO_VICT );
    interpret( victim, argument );
    }

    send_to_char( "Ya esta. :)\n\r", ch );
    return;
}
...
void do_invis( CHAR_DATA *ch, char *argument )
{
	...
	level = atoi( arg );
	if ( level < 2 || level > get_trust( ch ) +1 )
	{
	    send_to_char( "Invalid level.\n\r", ch );
	    return;
	}
	if (level > LEVEL_IMPLEMENTOR) 
		level = LEVEL_IMPLEMENTOR;
	if (!IS_NPC(ch))
        {
	  ch->pcdata->wizinvis = level;
	  ch_printf( ch, "Nivel de Wizinvis puesto a %d.\n\r", level );
        }

        if (IS_NPC(ch))
        {
          ch->mobinvis = level;
          ch_printf( ch, "Nivel de Mobinvis puesto a %d.\n\r", level );
        }
	return;
    }
	...
	if ( ch->pcdata->wizinvis < 2 )
	  ch->pcdata->wizinvis = LEVEL_IMPLEMENTOR;
    }
    if (IS_NPC(ch))
    {
	if ( ch->mobinvis < 2 )
	  ch->mobinvis = LEVEL_IMPLEMENTOR;
    }
...
void do_newpassword( CHAR_DATA *ch, char *argument )
{
    ...
    {
      send_to_char( "newpass <juador> <nuevopassword>.\n\r", ch );
      return;
    }

    if( victim == '\0' )
    {
	ch_printf(ch, "%s debe estar aqui para poder cambiarselo.\n\r", arg1 );
	return;
    }

    if ( IS_NPC( victim ) )
    {
	send_to_char( "No puedes cambiar el password de eso!\n\r",ch);
	return;
    }
	...
	{
	  send_to_char( "No puedes cambiarle el password!\n\r",ch);
	  return;
      }
    }
    else
	...
	{
	  ch_printf( ch, "%s no lo apreciaria :P\n\r", victim->name );
	  return;
	}
    }
    ...
    if ( strlen(arg2) < 5 )
    {
      send_to_char( "El nuevo password debe al menos de cinco caracteres.\n\r", ch );
      return;
    }
    ...
    for ( p = pwdnew; *p != '\0'; p++ )
    {
      if ( *p == '~' )
	{
	  send_to_char("Nuevo password no aceptable, (no uses ~) prueba otra vez.\n\r", ch );
	  return;
	}
    }
    ...
    save_char_obj( victim );
    ch_printf( ch, "&REl password de %s ha sido cambiado a: %s\n\r&w", victim->name, arg2 );
    sprintf( buf, "&R%s ha cambiado tu password a: %s\n\r&w", ch->name, arg2 );
...
/*
 * Funcion para el pload (loadup) KAYSER
 */
void do_loadup( CHAR_DATA *ch, char *argument )
{
    ...
    if ( name[0] == '\0' )
    {
	send_to_char( "Sintaxis: pload <jugador>\n\r", ch );
	return;
    }
	...
	if ( get_trust(d->character) >= get_trust( ch ) )
	{
	   do_say( d->character, "NI SE TE OCURRA molestarme otra vez!" );
	   send_to_char( "Pienso que seria mejor dejar a ese jugador en paz...\n\r", ch );
	   d->character->desc	= NULL;
	   do_quit( d->character, "" );
	   return;
	}
	...
	DISPOSE( d->outbuf );
	DISPOSE( d );
	ch_printf(ch, "Jugador %s cargado en la room %d.\n\r", capitalize( name ),old_room_vnum );
	sprintf(buf, "%s aparece de la nada, con los ojos en blanco.\n\r", capitalize( name ) );
        act( AT_IMMORT, buf, ch, NULL, NULL, TO_ROOM );
	send_to_char( "Done.\n\r", ch );
	return;
    }
    /* else no player file */
    send_to_char( "Ese jugador no existe.\n\r", ch );
    return;
}
...
( CHAR_DATA *ch, char *argument )
{
  ...
  if ( arg[0] == '\0' )
  {
      send_to_char( "Destruir la ficha de que jugador?\n\r", ch );
      return;
  }
  ...
  else
  {
    ...
    {
    	send_to_char("No puedes borrar a INMORTALES de esta forma.\n\r", ch);
    	return;
    }
  ...
  sprintf( buf2, "%s%s", BACKUP_DIR, name );
  ch_printf(ch, "Personaje %s BORRADO y copiado a %s.\n\r", name, buf2);
  rename(buf, buf2);  
  return;
}
...
void do_cedit( CHAR_DATA *ch, char *argument )
{
	...
	if ( get_trust(ch) > LEVEL_SUB_IMPLEM  )
	{
	    ...
	    send_to_char( "\n\rField being one of:\n\r", ch );
	    send_to_char( "  level position log code flags disci discilv activo\n\r", ch );
	}
	return;
    }
    ...
    /* Para activar o desactivar un comando on line sin aumentar su nivel */

    if ( !str_cmp( arg2, "activo" ) )
    {
	int activo = atoi( argument );

	if ( activo < 0 || activo > 2 )
	{
	    send_to_char( "0 comando activo, 1 inactivo, 2 obsoleto.\n\r", ch );
	    return;
	}
    ...
    /* Para decidir el nivel de disciplina del comando */

    if ( !str_cmp( arg2, "discilv" ) )
    {
	int level = atoi( argument );

	if ( (level < 0 || level > 15 ) )
	{
	    send_to_char( "Niveles de 0 a 10.\n\r", ch );
	    return;
	}
	command->disci_level = level;
	send_to_char( "Hecho.\n\r", ch );
	return;
    }

    /* Para decidir a que disciplina pertenece el comando */

    if ( !str_cmp( arg2, "disci" ) )
    {
        int disci = atoi( argument );

        if ( disci < 0 || disci > MAX_DISCIPLINAS )
        {
                sprintf( buf, "Disciplinas disponibles de 0 ( no pertenece a ninguna )  a %d.\n\rAyuda 'numdisci'.\n\r", MAX_DISCIPLINAS );
                send_to_char( buf, ch );
                return;
        }
        command->disciplina = disci;
        send_to_char( "Hecho.\n\r", ch );
        return;
     }
    if ( !str_cmp( arg2, "log" ) )
    ...
    if ( !str_cmp( arg2, "classes" ) )
    {
        for(i=0;i<MAX_CLASS;i++)
        {
           if( !str_cmp(argument,class_table[i]->who_name) )
           {
              ...
              bug("Bit de razas: %d", race->class_restriction);
              send_to_char( "Done.\n\r", ch );
              return;
           }
        }
...
void do_pxset( CHAR_DATA *ch, char *argument )
{
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  ...
  if ( arg[0] == '\0' || amount <= 0 )
  {
    send_to_char( "Syntax: pxset <jugador> <cantidad>\n\r", ch );
    send_to_char( "La cantidad debe ser un numero mayor que 0.\n\r", ch );
    return;
  }
  if ( ( victim = get_char_world( ch, arg ) ) == NULL )
  {
  ...
  if ( IS_NPC( victim ) )
  {
    send_to_char( "No se puede si hay mobs de por medio.\n\r", ch );
    return; 
  }
  ...
  if (victim->level < LEVEL_AVATAR)
  	...
  	victim->exp_acumulada += amount;
  
  ch_printf( victim, "Eres recompensado con %d puntos de experiencia.\n\r", amount );
  ch_printf( ch, "Recompensas a %s con %d puntos de experiencia.\n\r", victim->name, amount );
  return;
}
/*FIN KAYSER*/
void do_qpset( CHAR_DATA *ch, char *argument )
{
  ...
  {
    send_to_char( "Syntax: qpset <jugador> <dar/quitar> <cantidad>\n\r", ch );
    send_to_char( "La cantidad debe ser un numero mayor que 0.\n\r", ch );
    return;
  }
  if ( ( victim = get_char_world( ch, arg ) ) == NULL )
  ...
  set_char_color( AT_IMMORT, victim );
  if ( nifty_is_name_prefix( arg2, "dar" ) )
  {
    give = TRUE;
  ...
  else if ( nifty_is_name_prefix( arg2, "quitar" ) )
    give = FALSE;
  else
  {
    do_qpset( ch, "" );
  ...
  if ( give )
  {
    ...
    ch_printf( victim, "Eres recompensado con %d puntos de quest.\n\r", amount );
    ch_printf( ch, "Recompensas a %s con %d puntos de quest.\n\r",
	victim->name, amount );
  }
  else
  {
    ...
    {
      ch_printf( ch, "%s no tiene %d para dar.\n\r",
	  victim->name, amount );
      return;
    }
    else
    {
    victim->pcdata->quest_curr -= amount;
    ch_printf( victim, "Te han quitado %d puntos de quest.\n\r", amount );
    ch_printf( ch, "Quitas a %s %d puntos de quest.\n\r",
	victim->name, amount );
    }
  }
  return;
...
void do_qpstat( CHAR_DATA *ch, char *argument )
{
  ...
  if ( arg[0] == '\0' )
  {
    send_to_char( "Syntax:  qpstat <jugador>\n\r", ch );
    return;
  }
  if ( ( victim = get_char_world( ch, arg ) ) == NULL )
  {
  ...
  if ( IS_NPC( victim ) )
  {
    send_to_char( "Los mobs no tiene de eso.\n\r", ch );
    return;
  }
  ch_printf( ch, "%s tiene %d puntos de quest, de un total de %d puntos de quest conseguidos.\n\r",
        victim->name,
        victim->pcdata->quest_curr,
        victim->pcdata->quest_accum );
  return;
...
void do_khistory(CHAR_DATA *ch, char *argument)
{
	...
	if(arg[0] == '\0')
	{
		ch_printf(ch, "sintaxis: mhistorial <jugador>\n\r");
		return;
	}
	...
	set_char_color(AT_BLOOD, ch);
	ch_printf(ch, "Historial de muertes de %s:\n\r", vch->name);
			...
			vch->pcdata->killed[track].vnum; track++)
	{
		...
		ch_printf(ch, ")");
		set_char_color(AT_RED, ch);
		ch_printf(ch, "    - asesinado %d veces.\n\r",
...
void do_pcrename( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' || arg2[0] == '\0' )
    {
	send_to_char("Sintaxis: Renombrar <victima> <nuevo nombre>\n\r", ch );
	return;
    }

    if  (!check_parse_name( arg2, 1) )
    {
	send_to_char("Nombre ilegal.\n\r", ch );
	return;
    }
    ...
    if ( IS_NPC(victim ) )
    {
	send_to_char("No puedes renombrar mobs.\n\r", ch );
	return;
    }

    if ( get_trust(ch) < get_trust(victim) )
    {
	send_to_char("No creo que a el le gustase!\n\r", ch );
	return;
    }
    ...
    {
	send_to_char("Ese nombre ya existe.\n\r", ch );
	return;
    }
    ...
    if ( IS_IMMORTAL( victim ) )
	make_wizlist();
    send_to_char("Jugador renombrado.\n\r", ch );
    return;
}

/* esto servira para hacer la vida de los dioses mas ofuscada y duradera
  programado nuevamente por el Comando Fumao COLIKOTRON!!*/

void do_trucos_del_almendruco( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' /*|| arg2[0] == '\0'*/ )
    {
                send_to_char("Uso: truco <nombre truco>\n\r", ch );
                send_to_char( "Los trucos que hay por ahora son: empanar, desempanar, salida, entrada y polla.\n\r", ch );
                return;
    }

    if ( !str_cmp( arg1, "empanar" ) )
    {
      if ( arg2[0] == '\0'  )
      {
        /*buf = [MAX_STRING_LENGTH];*/
        sprintf(buf, "&G%s se empana cual &RSan Jacobo....&GDejo un cartel, &Wno molestar!\n\r", ch->name  );
        echo_to_all( AT_BLUE, buf, ECHOTAR_ALL );
        xSET_BIT(ch->act, PLR_WIZINVIS);
        xSET_BIT(ch->act, PLR_AFK);
        send_to_char( "Empanado activado.\n\r", ch );
        return;
      }
      if ( !str_cmp( arg2,  "mensaje" ) )
      {
        sprintf(buf, "&G%s se empana cual &RSan Jacobo....&GDejo un cartel, &Wno molestar!\n\r", ch->name );
        echo_to_all( AT_BLUE, buf, ECHOTAR_ALL );
        xSET_BIT(ch->act, PLR_AFK);
        send_to_char( "Empanar activo sin invis.\n\r", ch );
        return;
      }
      return;
       }
     if (!str_cmp( arg1, "desempanar" ) )
     {
       if ( arg2[0] == '\0' )
       {
         sprintf(buf, "&G%s ya esta bien doradito &Rse desempana... &Wnyam nyam.!\n\r", ch->name );
         echo_to_all( AT_BLUE, buf, ECHOTAR_ALL );
         xREMOVE_BIT(ch->act, PLR_WIZINVIS);
         xREMOVE_BIT(ch->act, PLR_AFK);
         send_to_char( "Empanao desactivao xD\n\r", ch );
         return;
        }
       return;
       }

       if ( !str_cmp( arg1, "salida" ) )
       {
        if ( arg2[0] == '\0' )
        {
          sprintf( buf, "&R[&G%s &Pha decidido volver al mundo real&R]\n\r", ch->name );
          echo_to_all( AT_BLUE, buf, ECHOTAR_ALL );
          xSET_BIT(ch->act, PLR_WIZINVIS);
          send_to_char( "Los has enganyado a todos eh?xD\n\r", ch );
          return;
          }
          return;
          }

       if ( !str_cmp( arg1, "polla" ) )
       {
       char buf[MAX_STRING_LENGTH];
sprintf( buf,"                                 .oOOOOooo..\n\r                           ..e oOOOOOOOOOOOOo.\n\r                        ..elll OOOOOOOOOOOOOOO\n\r                   ..ellllllll OOOOOOOOOOOOOO'\n\r+,,.           .elllllllllllllll OOOOOOOOOOO*'\n\r.+,,+,+`eeeeellllllllllllllllllll '*OOO**''`\n\r.,+,,.`llllllllllllllllllllllle*``\n\r.+++`+llllll++lllllllllle*'`\n\r+ ','`++llllllllllle*'`\n\r,.'+`+ +llllllle``       Todo esto es tuyo?\n\r''`+'' +eee@@el`\n\r,+'+''+%@@@@@@@.\n\r'+,'.+%%@@@@@@@*\n\r'+,.@@@@@@@@@@*\n\r++'+'+@@@@@@@@'\n\r'+'+  ~@@@@@~\n\r" );

echo_to_all(AT_WHITE,buf, ECHOTAR_ALL );
       return;
       }

       if ( !str_cmp( arg1, "entrada" ) )
       {
        if ( arg2[0] == '\0' )
        {
          sprintf( buf, "&R[&G%s &Pha dejado el mundo real para unirse a Vampiro&R]\n\r", ch->name );
          echo_to_all( AT_BLUE, buf, ECHOTAR_ALL );
          xREMOVE_BIT(ch->act, PLR_WIZINVIS);
          send_to_char( "Los has enganyado a todos eh?xD\n\r", ch );
          return;
...
/* Esto es un snipett de un notas ke enkontre por internet esta interesante
   hace que un inmortal vuelva a ser mortal en el nivel ke kiera durante un tiempo xD */
...
void do_mortality( CHAR_DATA *ch, char *argument )
{
    ...
    argument = one_argument( argument, arg );
    if ( arg[0] == '\0' && (!IS_IMMORTAL(ch) && ch->tmplevel < 51))
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
    if (IS_IMMORTAL(ch))
    {
     ...
     {
	send_to_char( "Venga hombre no me seas notas.\n\r", ch );
        return;
     }
     ...
     if ( xIS_SET(ch->act, PLR_WIZINCOG) )
       xREMOVE_BIT(ch->act, PLR_WIZINCOG);
     if ( xIS_SET(ch->act, PLR_WIZINVIS) )
       xREMOVE_BIT(ch->act, PLR_WIZINVIS);
     save_char_obj(ch);
     }
     ...
     act( AT_PLAIN, "$n revienta en mil pedazitos deja el lugar hecho un asco.", ch, NULL, NULL, TO_ROOM );
     act( AT_PLAIN, "Te revientas en mil pedazitos,  cada pedazo se une al otro y te conviertes en Heroe.", ch, NULL, NULL, TO_CHAR );

     if (!str_cmp(argument, "anunciar") || !str_cmp(arg, "anunciar"))
     {
      sprintf(buf, "%s acaba de rebajarse a mortal para canear a alguien supongo xD.", ch->name);
      echo_to_all(AT_IMMORT, buf, ECHOTAR_ALL);
     }
     return;
    }
    if (!IS_IMMORTAL(ch) && ch->tmplevel > LEVEL_HERO)
    {
    ...
    ch->exp = exp_level( ch, ch->level );
    save_char_obj(ch);
    act( AT_PLAIN, "$n se hace un pajote bestial lo mancha todo , DIOS QUE ASKO!!.", ch, NULL, NULL, TO_ROOM );
    act( AT_PLAIN, "Te haces un pajote y al orgasmearte recuperas tu estatus de INMORTAL,  ole tu sexo!.", ch, NULL, NULL, TO_CHAR );
     if (!str_cmp(argument, "anunciar") || !str_cmp(arg, "anunciar"))
     {
      sprintf(buf, "%s vuelve entre los dioses despues de kedarse relajad@ :P.", ch->name);
      echo_to_all(AT_IMMORT, buf, ECHOTAR_ALL);
     }
    return;
    }

...
bool spec_cast_adept( CHAR_DATA *ch )
{
    ...
    {
    case 0:
    act( AT_MAGIC, "$n pronuncia las palabras 'ciroht'.", ch, NULL, NULL, TO_ROOM );
	spell_smaug( skill_lookup( "armor" ), ch->level, ch, victim );
	return TRUE;

    case 1:
    act( AT_MAGIC, "$n pronuncia las palabras 'sunimod'.", ch, NULL, NULL, TO_ROOM );
	spell_smaug( skill_lookup( "bless" ), ch->level, ch, victim );
	return TRUE;

    case 2:
    act( AT_MAGIC, "$n pronuncia las palabras 'suah'.", ch, NULL, NULL, TO_ROOM );
	spell_cure_blindness( skill_lookup( "cure blindness" ),
	    ch->level, ch, victim );
	return TRUE;

    case 3:
    act( AT_MAGIC, "$n pronuncia las palabras 'nran'.", ch, NULL, NULL, TO_ROOM );
	spell_smaug( skill_lookup( "cure light" ),
	    ch->level, ch, victim );
	return TRUE;

    case 4:
    act( AT_MAGIC, "$n pronuncia las palabras 'nyrcs'.", ch, NULL, NULL, TO_ROOM );
	spell_cure_poison( skill_lookup( "cure poison" ),
	    ch->level, ch, victim );
	return TRUE;

    case 5:
    act( AT_MAGIC, "$n pronuncia las palabras 'gartla'.", ch, NULL, NULL, TO_ROOM );
	spell_smaug( skill_lookup( "refresh" ), ch->level, ch, victim );
	return TRUE;

    case 6:
    act( AT_MAGIC, "$n pronuncia las palabras 'naimad'.", ch, NULL, NULL, TO_ROOM );
        spell_smaug( skill_lookup( "cure serious" ), ch->level, ch, victim );
        return TRUE;

    case 7:
    act( AT_MAGIC, "$n pronuncia las palabras 'gorog'.", ch, NULL, NULL, TO_ROOM );
...
bool spec_executioner( CHAR_DATA *ch )
{
    ...
    {
	sprintf( buf, "%s es un %s!  As well as a COWARD!",
		victim->name, crime );
	do_yell( ch, buf );
	return TRUE;
    }

    sprintf( buf, "%s es un %s!  PROTECCION AL INOCENTE!  SAANGREEE!!!",
	victim->name, crime );
    do_shout( ch, buf );
...
bool spec_fido( CHAR_DATA *ch )
{
    ...
    act( AT_ACTION, "$n salvagemente devora el cuerpo.", ch, NULL, NULL, TO_ROOM );
...
bool spec_guard( CHAR_DATA *ch )
{
    ...
    if ( victim )
    {
	sprintf( buf, "%s es un %s!  PROTECCION AL INOCENTE!!  BANZAI!!",
		victim->name, crime );
	do_shout( ch, buf );
	multi_hit( ch, victim, TYPE_UNDEFINED );
	return TRUE;
    ...
    if ( ech )
    {
    act( AT_YELL, "$n grita 'PROTECCION AL INOCENTE!!  BANZAI!!",
...
bool spec_janitor( CHAR_DATA *ch )
{
	...
	&&  !trash->first_content) )
	{
	    act( AT_ACTION, "$n recoge algo del suelo.", ch, NULL, NULL, TO_ROOM );
	    obj_from_room( trash );
...
bool spec_poison( CHAR_DATA *ch )
{
   ...
   if ( ( victim = who_fighting( ch ) ) == NULL
    ...
    act( AT_HIT, "Muerdes a $N!",  ch, NULL, victim, TO_CHAR    );
    act( AT_ACTION, "$n muerde a $N!",  ch, NULL, victim, TO_NOTVICT );
    act( AT_POISON, "$n te muerde!", ch, NULL, victim, TO_VICT    );
...
bool spec_thief( CHAR_DATA *ch )
{
	...
	{
	    act( AT_ACTION, "Descubres las manos de $n en tu bolsillo!",

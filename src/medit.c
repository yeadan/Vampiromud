 ...
 ****************************************************************************
 *                      Modulo de Edicion de Mobs Veditor                   *
 ****************************************************************************
 *   Basado en OasisEDITOR para CircleMUD3.0bpl9 escrto por Harvey Gilpin   *
 ****************************************************************************/
...
char 		*editor_clan_list[50];
char 		*editor_deity_list[50];
char 		*editor_council_list[50];
int		editor_top_clan;
int		editor_top_guild;
int		editor_top_order;
int             editor_top_council;
int             editor_top_deity;

char *	const	position_names [] =
{
"muerto", "mortalmenteherido", "incapacitado", "atontado", "durmiendo",
"berserker", "descansando", "agresivo", "sentado", "luchando", "defensivo",
"evasivo", "plantado", "sobremontura", "shoved", "dragged"
};
...
void do_omedit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Editar que mob?\n\r", ch );
	return;
    }
    if ( (victim = get_char_world( ch, arg ) ) == NULL )
    {
	send_to_char( "Esta lelo he, no existe ese bisho.\n\r", ch );
	return;
    }
    ...
    {
	send_to_char( "No puedes modificar jugadores con Medit.\n\r", ch );
	return;
    }
	...
	if ( d->connected == CON_MEDIT )
	    if ( d->editor && EDITOR_VNUM(d) == victim->pIndexData->vnum )
	    {
                ch_printf( ch, "Este %s esta siendo editado por %s.\n\r",
                           IS_NPC(victim) ? "mob" : "jugador", d->character->name );
		return;
	    }

    if(IS_NPC(victim))
    xSET_BIT(victim->act, ACT_PROTOTYPE);
    ...
    if ( IS_NPC( victim ) )
    {
        EDITOR_VNUM(d) = victim->pIndexData->vnum;
        /* medit_setup( d, EDITOR_VNUM(d) ); */
    }
    else
	medit_setup_arrays();
    ...
    EDITOR_CHANGE(d) = FALSE;
    medit_disp_menu( d );

    act( AT_ACTION, "$n se empana y empieza a usar el Veditor de Areas.", ch, NULL, NULL, TO_ROOM );
    return;
}
...
void medit_setup_arrays( void )
{
	...
	{
	    editor_clan_list[count] = clan->name;
	    count++;
	}
    }
    editor_top_clan = count-1;
    ...
    editor_top_order = count-1;

    count = 0;
    for ( deity = first_deity; deity; deity = deity->next )
    {
	editor_deity_list[count] = QUICKLINK( deity->name );
	count++;
    }
    editor_top_deity = count;

    count = 0;
    for ( council = first_council; council; council = council->next )
    {
	editor_council_list[count] = council->name;
	count++;
    }
    editor_top_council = count;
}
...
void medit_disp_positions( DESCRIPTOR_DATA *d )
{
    ...
    ch_printf_color( ch, "\n\r                          VampiroMUD2.0b Area-On-Line-Editor\n\r\n\r" );
    send_to_char_color( " &g0&w) Muerto\n\r", d->character );
    send_to_char_color( " &g1&w) Mortalmente Herido\n\r", d->character );
    send_to_char_color( " &g2&w) Incapacitado\n\r", d->character );
    send_to_char_color( " &g3&w) Atontado\n\r", d->character );
    send_to_char_color( " &g4&w) Durmiendo\n\r", d->character );
    send_to_char_color( " &g5&w) Berserker\n\r", d->character );
    send_to_char_color( " &g6&w) Descansando\n\r", d->character );
    send_to_char_color( " &g7&w) Agresivo\n\r", d->character );
    send_to_char_color( " &g8&w) Sentado\n\r", d->character );
    send_to_char_color( " &g9&w) Luchando\n\r", d->character );
    send_to_char_color( "&g10&w) Defensivo\n\r", d->character );
    send_to_char_color( "&g11&w) Evasivo\n\r", d->character );
    send_to_char_color( "&g12&w) Plantado\n\r", d->character );
    /* for (i = 0; i < POS_MOUNTED; i++)
    {
	sprintf(buf, "&g%2d&w) %s\r\n", i, capitalize(position_names[i]));
	send_to_char_color(buf, d->character);
    } */
    send_to_char("Introduce el numero de la posicion : ", d->character);
}
...
void medit_disp_sex( DESCRIPTOR_DATA *d )
{
    ...
    write_to_buffer( d, "50\x1B[;H\x1B[2J", 0 );
    ch_printf_color( ch, "\n\r                          VampiroMUD2.0b Area-On-Line-Editor\n\r\n\r" );
    send_to_char_color( " &g0&w) Ninguno??\n\r", d->character );
    send_to_char_color( " &g1&w) Masho\n\r", d->character );
    send_to_char_color( " &g2&w) Hembra\n\r", d->character );
    send_to_char("\n\rQue va a ser?  ", d->character);
}
...
void medit_disp_spec( DESCRIPTOR_DATA *d )
{
    ...
    write_to_buffer( d, "50\x1B[;H\x1B[2J", 0 );
    ch_printf_color( ch, "\n\r                          VampiroMUD2.0b Area-On-Line-Editor\n\r\n\r" );
    ch_printf_color( ch, " &g 0&w) No tiene nada especial\n\r" );
    ...
    send_to_char( "Que funcion especial tendra: ", ch );
}
...
void medit_disp_ris( DESCRIPTOR_DATA *d )
{
    ...
    send_to_char( "Que flag (0 para salir): ", d->character );
}
...
void medit_disp_attack_menu( DESCRIPTOR_DATA *d )
{
    ...
    sprintf( buf, "Flags actuales: &c%s&w\n\rQue flag de ataque (0 para salir): ",
...
void medit_disp_defense_menu( DESCRIPTOR_DATA *d )
{
    ...
    sprintf( buf, "Flags actuales: &c%s&w\n\rQue flag de defensa (0 para salir): ",
...
void medit_disp_mob_flags( DESCRIPTOR_DATA *d )
{
    ...
    sprintf(buf, "\r\n"
	"Flags actuales : &c%s&w\r\n"
	"Que flags de mob (0 para salir) : ",
...
void medit_disp_aff_flags( DESCRIPTOR_DATA *d )
{
    ...
    if ( EDITOR_MODE(d) == OEDIT_AFFECT_MODIFIER )
    {
	buf[0] = '\0';
    ...
    send_to_char_color( "Que flags de affectado (0 para salir) : ", d->character );
}
...
void medit_disp_clans( DESCRIPTOR_DATA *d )
{
    ...
    write_to_buffer( d, "50\x1B[;H\x1B[2J", 0 );
    /* ch_printf( ch, "%d %d %d\n\r", editor_top_clan, editor_top_guild, editor_top_order ); */
    ch_printf_color( ch, "\n\r                          VampiroMUD2.0b Area-On-Line-Editor\n\r\n\r" );
    send_to_char( "Clanes:\n\r", d->character );
    for ( count = 0; count <= editor_top_clan; count++ )
    {
	ch_printf_color( ch, "&g%2d&w) %-20.20s\n\r", count+1, editor_clan_list[count] );
    }

    send_to_char( "\n\rGremios:\n\r", d->character );
    for ( count = editor_top_clan+1; count <= editor_top_guild; count++ )
    {
	ch_printf_color( ch, "&g%2d&w) %-20.20s\n\r", count+1, editor_clan_list[count] );
    }

    send_to_char( "\n\rOrdenes:\n\r", d->character );
    for ( count = editor_top_guild+1; count <= editor_top_order; count++ )
    {
	ch_printf_color( ch, "&g%2d&w) %-20.20s\n\r", count+1, editor_clan_list[count] );
    }
    send_to_char( "Eleccion (0 para ninguna): ", d->character );
}
...
void medit_disp_deities( DESCRIPTOR_DATA *d )
{
    ...
    ch_printf_color( d->character, "&g%2d&w) %-20.20s\n\r", 0, "Ninguna" );
    for ( count = 0; count < editor_top_deity; count++ )
    {
	ch_printf_color( d->character, "&g%2d&w) %-20.20s\n\r", count+1, editor_deity_list[count] );
    }
    send_to_char( "Eleccion: ", d->character );
}
...
void medit_disp_councils( DESCRIPTOR_DATA *d )
{
    ...
    write_to_buffer( d, "50\x1B[;H\x1B[2J", 0 );
    ch_printf_color( ch, "\n\r                          VampiroMUD2.0b Area-On-Line-Editor\n\r\n\r" );
    ch_printf( d->character, "%d\n\r", editor_top_council );
    ch_printf_color( d->character, "&g%2d&w) %-20.20s\n\r", 0, "Ninguno" );
    for ( count = 0; count < editor_top_council; count++ )
    {
	ch_printf_color( d->character, "&g%2d&w) %-20.20s\n\r", count+1, editor_council_list[count] );
    }
    send_to_char( "Eleccion: ", d->character );
}
...
void medit_disp_parts( DESCRIPTOR_DATA *d )
{
    ...
    ch_printf_color( d->character, "\n\rFlags actuales: %s\n\rIntroduzca un flag o 0 para salir: ",
...
void medit_disp_races( DESCRIPTOR_DATA *d )
{
    ...
    send_to_char( "\n\rRaza o Pertenencia: ", d->character );
}
...
void medit_disp_saving_menu( DESCRIPTOR_DATA *d )
{
    ...
    send_to_char( "\n\rQue save a modificar: ", d->character );

    EDITOR_MODE(d) = MEDIT_SAVE_MENU;
}
...
void medit_disp_npc_menu( DESCRIPTOR_DATA *d )
{
    ...
    ch_printf_color( ch, "\n\r                          VampiroMUD2.0b Area-On-Line-Editor\n\r\n\r" );
    ch_printf_color( ch, "&w-- Estas editando &g%s&w.\n\r", mob->short_descr[0] == '\0' ? "(ninguna descripcion fijada)" : mob->short_descr);
    ch_printf_color( ch, "-- Mob Vnum:  [&c%d&w]\n\r", mob->pIndexData->vnum );
    ch_printf_color( ch, "&g1&w) Sexo: &O%-7.7s         &g2&w) Nombre: &O%s\n\r", mob->sex == SEX_MALE ? "hombre" : mob->sex == SEX_FEMALE  ? "mujer" : "neutral", mob->name );
    ch_printf_color( ch, "&g3&w) DescrCorta: &O%s\n\r", mob->short_descr[0] == '\0' ? "(ninguna)" : mob->short_descr );
    ch_printf_color( ch, "&g4&w) DescLarga:-\n\r&O%s\n\r", mob->long_descr[0] == '\0' ? "(ninguna)" : mob->long_descr );
    ch_printf_color( ch, "&g5&w) Descripcion:-\n\r&O%-74.74s\n\r", mob->description );
    ch_printf_color( ch, "&g6&w) Clase: [&c%-11.11s&w], &g7&w) Raza:   [&c%-11.11s&w]\n\r",
	npc_class[mob->class], npc_race[mob->race] );
    ch_printf_color( ch, "&g8&w) Nivel:       [&c%5d&w], &g9&w) Alineamiento: [&c%5d&w], &gA&w) Fuerza:   [&c%5d&w]\n\r",
	mob->level, mob->alignment, get_curr_str(mob) );
    ch_printf_color( ch, "&gB&w) Inteligencia:[&c%5d&w], &gC&w) Sabiduria:    [&c%5d&w], &gD&w) Destreza: [&c%5d&w]\n\r",
	get_curr_int(mob), get_curr_wis(mob), get_curr_dex(mob) );
    ch_printf_color( ch, "&gE&w) Constitucion:[&c%5d&w], &gF&w) Carisma:      [&c%5d&w], &gG&w) Suerte:   [&c%5d&w]\n\r",
    ...
    ch_printf_color( ch, "&gN&w) Oro:      [&c%8d&w], &gO&w) Funcion Especial: &O%-22.22s\n\r",
        mob->gold, lookup_spec( mob->spec_fun ) );
    ch_printf_color( ch, "&gP&w) Saves Versus:\n\r" );
    ch_printf_color( ch, "&gR&w) Resistente  : &O%s\n\r", flag_string( mob->resistant, ris_flags ) );
    ch_printf_color( ch, "&gS&w) Inmune      : &O%s\n\r", flag_string( mob->immune, ris_flags ) );
    ch_printf_color( ch, "&gT&w) Susceptible : &O%s\n\r", flag_string( mob->susceptible, ris_flags ) );
    ch_printf_color( ch, "&gU&w) Posicion    : &O%s\n\r", position_names[ (int) mob->position ] );
    ch_printf_color( ch, "&gV&w) Ataques     : &c%s\n\r", ext_flag_string( &mob->attacks, attack_flags ) );
    ch_printf_color( ch, "&gW&w) Defensas    : &c%s\n\r", ext_flag_string( &mob->defenses, defense_flags ) );
    ch_printf_color( ch, "&gX&w) Partes cuerpo &c%s\n\r", flag_string(mob->xflags, part_flags) );
    ch_printf_color( ch, "&gY&w) Act Flags   : &c%s\n\r", ext_flag_string( &mob->act, act_flags ) );
    ch_printf_color( ch, "&gZ&w) Afectado por: &c%s\n\r", affect_bit_name( &mob->affected_by ) );
    ch_printf_color( ch, "&gQ&w) Salir\n\r" );
    ch_printf_color( ch, "Eleccion : " );
    ch_printf_color( ch, "\n\r\n\rColiKoTRoN Code Team VampiroMUD(c) GNU License" );

    EDITOR_MODE(d) = MEDIT_NPC_MAIN_MENU;
}
...
void medit_disp_pc_menu( DESCRIPTOR_DATA *d )
{
    ...
    ch_printf_color( ch, "\n\r                          VampiroMUD2.0b Area-On-Line-Editor\n\r\n\r" );
    ch_printf_color( ch, "&g1&w) Sexo: &O%-7.7s           &g2&w) Nombre: &O%s\n\r",
	victim->sex == SEX_MALE ? "hombre" : victim->sex == SEX_FEMALE  ? "mujer" : "neutral", victim->name );
    ch_printf_color( ch, "&g3&w) Descripcion:-\n\r&O%-74.74s\n\r", victim->description );
    ch_printf_color( ch, "&g4&w) Clase: [&c%-11.11s&w],  &g5&w) Pertenencia:   [&c%-11.11s&w]\n\r",
	class_table[victim->class]->who_name, race_table[victim->race]->race_name );
    ch_printf_color( ch, "&g6&w) Nivel:       [&c%5d&w],  &g7&w) Alineammiento:    [&c%5d&w],  &g8&w) Fuerza:  [&c%5d&w]\n\r",
  ...
  if ( get_trust(ch) >= 58 )
    ...
    EDITOR_MODE(d) = MEDIT_PC_MAIN_MENU;
}
...
void medit_parse( DESCRIPTOR_DATA *d, char *arg )
{
	...
	case 'Y':
	    ...
	    if ( IS_NPC(victim) )
	    {
		sprintf( log_buf, "EDITOR: %s edits mob %d(%s)", d->character->name, victim->pIndexData->vnum, victim->name );
		for ( tarea = first_asort; tarea; tarea = tarea->next )
		{
		    if ( EDITOR_VNUM(d) >= tarea->low_m_vnum
		    &&   EDITOR_VNUM(d) <= tarea->hi_m_vnum )
		    {
		...
		&&   IS_SET(tarea->status, AREA_LOADED ) )
		{
            ...
            {
                sprintf( log_buf, "EDITOR: %s edits %s", d->character->name, victim->name );
  ...
  case MEDIT_NPC_MAIN_MENU:
	...
	case 'Q':
	    ...
	    return;
	case '1':
	    EDITOR_MODE(d) = MEDIT_SEX;
	    medit_disp_sex(d);
	    return;
	case '2':
	    EDITOR_MODE(d) = MEDIT_NAME;
	    send_to_char( "\n\rEnter name: ", d->character );
	    return;
	case '3':
	    EDITOR_MODE(d) = MEDIT_S_DESC;
	    send_to_char( "\n\rEnter short description: ", d->character );
	    return;
	case '4':
	    EDITOR_MODE(d) = MEDIT_L_DESC;
	    send_to_char( "\n\rEnter long description: ", d->character );
	    return;
	case '5':
	    EDITOR_MODE(d) = MEDIT_D_DESC;
	...
	case 'P':
	    ...
	    return;
	case 'R':
	    EDITOR_MODE(d) = MEDIT_RESISTANT;
	    medit_disp_ris(d);
	    return;
	case 'S':
	    EDITOR_MODE(d) = MEDIT_IMMUNE;
	    medit_disp_ris(d);
	    return;
	case 'T':
	    EDITOR_MODE(d) = MEDIT_SUSCEPTIBLE;
	    medit_disp_ris(d);
	    return;
	case 'U':
	    EDITOR_MODE(d) = MEDIT_POS;
	    medit_disp_positions(d);
	    return;
	case 'V':
	    EDITOR_MODE(d) = MEDIT_ATTACK;
	    medit_disp_attack_menu(d);
	    return;
	case 'W':
	    EDITOR_MODE(d) = MEDIT_DEFENSE;
	    medit_disp_defense_menu(d);
	    return;
	case 'X':
	    EDITOR_MODE(d) = MEDIT_PARTS;
	    medit_disp_parts(d);
	    return;
	case 'Y':
	    EDITOR_MODE(d) = MEDIT_NPC_FLAGS;
	    medit_disp_mob_flags(d);
	    return;
	case 'Z':
	    EDITOR_MODE(d) = MEDIT_AFF_FLAGS;
  ...
  case MEDIT_PC_MAIN_MENU:
	...
	case 'Q':
		...
		EDITOR_MODE(d) = MEDIT_CONFIRM_SAVESTRING;
	    }
	    else*/
		limpiar_editor( d );
	    return;
	...
	case 'E':
	    ...
	    return;
	case 'F':
	    EDITOR_MODE(d) = MEDIT_HITPOINT;
	    send_to_char( "\n\rEnter hitpoints: ", d->character );
	    return;
	case 'G':
	    EDITOR_MODE(d) = MEDIT_MANA;
	    send_to_char( "\n\rEnter mana: ", d->character );
	    return;
	case 'H':
	    EDITOR_MODE(d) = MEDIT_MOVE;
	...
	case 'U':
	    ...
	    break;
	case 'V':
	    EDITOR_MODE(d) = MEDIT_PC_FLAGS;
	    medit_disp_plr_flags(d);
	    return;
	case 'W':
	    EDITOR_MODE(d) = MEDIT_PCDATA_FLAGS;
	...
	case 'X':
	    ...
	    return;
	case 'Y':
	    EDITOR_MODE(d) = MEDIT_DEITY;
	    medit_disp_deities(d);
	    return;
	case 'Z':
	    if ( get_trust(d->character) < LEVEL_GOD )
		break;
	    EDITOR_MODE(d) = MEDIT_CLAN;
	    medit_disp_clans(d);
	    return;
	case '=':
	    if ( get_trust(d->character) < 58 )
		break;
	    EDITOR_MODE(d) = MEDIT_COUNCIL;
	...
	if ( !IS_NPC(victim) && get_trust( d->character ) > 58 )
	{
	    ...
	    editor_log( d, "Changes name to %s", arg );
	    return;
	}
	STRFREE( victim->name );
	...
	editor_log( d, "Changed name to %s", arg );
	break;
	...
	editor_log( d, "Changed short desc to %s", arg );
        break;
	...
	editor_log( d, "Changed long desc to %s", arg );
	break;

    case MEDIT_D_DESC:
	/*. We should never get here .*/
	limpiar_editor( d );
	bug( "EDITOR: medit_parse(): Reached D_DESC case!", 0 );
	break;
	...
	if ( is_number( arg ) )
        ...
        while ( arg[0] != '\0' )
        {
            ...
            if ( is_number( arg1 ) )
            {
                ...
                {
                    send_to_char( "Ese flag no es correcto: ", d->character );
                    return;
                }
            }
            else
            ...
            &&   !is_name( "protoflag", d->character->pcdata->bestowments ) )
                send_to_char( "Tu no tienes el suficiente poder como para cambiar ese flag.\n\r", d->character );
            else if( IS_NPC(victim) && number == ACT_IS_NPC )
                send_to_char( "Es imposible cambiar este flag.\n\r", d->character );
            else if( IS_NPC(victim) && number == ACT_UALIZADO )
                send_to_char( "No se permite poner ese flag a los mobs.\n\r", d->character );
            else
            {
    ...
    case MEDIT_PC_FLAGS:
		...
		number -= 1; /* offset :P */
                xTOGGLE_BIT( victim->act, number );
		editor_log( d, "%s the flag %s", xIS_SET( victim->act, number ) ? "Added" : "Removed",
		    plr_flags[number] );
            }
        }
        else
    ...
    case MEDIT_PCDATA_FLAGS:
		...
		editor_log( d, "%s the pcflag %s", IS_SET( victim->pcdata->flags, 1 << number ) ? "Added" : "Removed",
		    pc_flags[number] );
            }
        }
        else
    ...
    case MEDIT_AFF_FLAGS:
		...
		editor_log( d, "%s the affect %s", xIS_SET( victim->affected_by, number ) ? "Added" : "Removed",
		    a_flags[number] );
            }
        }
        else
...
/*. Numerical responses .*/
    case MEDIT_HITPOINT:
	victim->max_hit = URANGE( 1, atoi(arg), 327000 );
	editor_log( d, "Changed hitpoints to %d", victim->max_hit );
	break;

    case MEDIT_MANA:
	victim->max_mana = URANGE( 1, atoi(arg), 300000 );
	editor_log( d, "Changed mana to %d", victim->max_mana );
	break;

    case MEDIT_MOVE:
	victim->max_move = URANGE( 1, atoi(arg), 300000 );
	editor_log( d, "Changed moves to %d", victim->max_move );
	break;

    case MEDIT_PRACTICE:
	victim->practice = URANGE( 1, atoi(arg), 3000 );
	editor_log( d, "Changed practives to %d", victim->practice );
	break;

    case MEDIT_PASSWORD:
	if ( get_trust(d->character) < 95 )
	    break;
	if ( strlen(arg) < 5 )
	{
	...
	if ( IS_SET(sysdata.save_flags, SV_PASSCHG) )
	    save_char_obj( victim );
	editor_log( d, "Modified password" );
	break;
	...
	medit_disp_saving_menu(d);
	editor_log( d, "Changed save_poison_death to %d", victim->saving_poison_death );
	return;
	...
	medit_disp_saving_menu(d);
	editor_log( d, "Changed save_wand to %d", victim->saving_wand );
	return;
	...
	medit_disp_saving_menu(d);
	editor_log( d, "Changed save_paralysis_petrification to %d", victim->saving_para_petri );
	return;
	...
	medit_disp_saving_menu(d);
	editor_log( d, "Changed save_breath to %d", victim->saving_breath );
	return;
	...
	medit_disp_saving_menu(d);
	editor_log( d, "Changed save_spell_staff to %d", victim->saving_spell_staff );
	return;
	...
	editor_log( d, "Changed strength to %d", victim->perm_str );
	break;
	...
	editor_log( d, "Changed intelligence to %d", victim->perm_int );
        break;
	...
	editor_log( d, "Changed victim wisdom to %d", victim->perm_wis );
        break;
	...
	editor_log( d, "Changed dexterity to %d", victim->perm_dex );
        break;
	...
	editor_log( d, "Changed constitution to %d", victim->perm_con );
        break;
	...
	editor_log( d, "Changed charisma to %d", victim->perm_cha );
        break;
	...
	editor_log( d, "Changed luck to %d", victim->perm_lck );
        break;
	...
	editor_log( d, "Changed sex to %s", victim->sex == 1 ? "Male" : victim->sex == 2 ? "Female" : "Neutral" );
	break;
	...
	editor_log( d, "Changed hitroll to %d", victim->hitroll );
	break;
	...
	editor_log( d, "Changed damroll to %d", victim->damroll );
	break;

    case MEDIT_DAMNUMDIE:
        if ( IS_NPC(victim) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
	    victim->pIndexData->damnodice = URANGE( 0, atoi(arg), 100 );
	editor_log( d, "Changed damnumdie to %d", victim->pIndexData->damnodice );
	break;

    case MEDIT_DAMSIZEDIE:
        if ( IS_NPC(victim) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
	    victim->pIndexData->damsizedice = URANGE( 0, atoi(arg), 100 );
	editor_log( d, "Changed damsizedie to %d", victim->pIndexData->damsizedice );
	break;

    case MEDIT_DAMPLUS:
        if ( IS_NPC(victim) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
	    victim->pIndexData->damplus = URANGE( 0, atoi(arg), 1000 );
	editor_log( d, "Changed damplus to %d", victim->pIndexData->damplus );
	break;

    case MEDIT_HITNUMDIE:
	if ( IS_NPC(victim) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
	    victim->pIndexData->hitnodice = URANGE( 0, atoi(arg), 32767 );
	editor_log( d, "Changed hitnumdie to %d", victim->pIndexData->hitnodice );
	break;

    case MEDIT_HITSIZEDIE:
	if ( IS_NPC(victim) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
	    victim->pIndexData->hitsizedice = URANGE( 0, atoi(arg), 30000 );
	editor_log( d, "Changed hitsizedie to %d", victim->pIndexData->hitsizedice );
	break;

    case MEDIT_HITPLUS:
	if ( IS_NPC(victim) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
	    victim->pIndexData->hitplus = URANGE( 0, atoi(arg), 30000 );
	editor_log( d, "Changed hitplus to %d", victim->pIndexData->hitplus );
	break;

    case MEDIT_AC:
	victim->armor = URANGE( -300, atoi(arg), 300 );
	editor_log( d, "Changed armor to %d", victim->armor );
	break;

    case MEDIT_GOLD:
	victim->gold = UMAX( 0, atoi(arg) );
	editor_log( d, "Changed gold to %d", victim->gold );
	break;

    case MEDIT_POS:
	victim->position = URANGE( 0, atoi(arg), POS_STANDING );
	editor_log( d, "Changed position to %d", victim->position );
	break;

    case MEDIT_DEFAULT_POS:
	victim->defposition = URANGE( 0, atoi(arg), POS_STANDING );
	editor_log( d, "Changed default position to %d", victim->defposition );
	break;

    case MEDIT_MENTALSTATE:
	victim->mental_state = URANGE( -100, atoi(arg), 100 );
	editor_log( d, "Changed mental state to %d", victim->mental_state );
	break;

    case MEDIT_EMOTIONAL:
	victim->emotional_state = URANGE( -100, atoi(arg), 100 );
	editor_log( d, "Changed emotional state to %d", victim->emotional_state );
	break;

    case MEDIT_THIRST:
	victim->pcdata->condition[COND_THIRST] = URANGE( 0, atoi(arg), 100 );
	editor_log( d, "Changed thirst to %d", victim->pcdata->condition[COND_THIRST] );
	break;

    case MEDIT_FULL:
	victim->pcdata->condition[COND_FULL] = URANGE( 0, atoi(arg), 100 );
	editor_log( d, "Changed hunger to %d", victim->pcdata->condition[COND_FULL] );
	break;

    case MEDIT_DRUNK:
	victim->pcdata->condition[COND_DRUNK] = URANGE( 0, atoi(arg), 100 );
	editor_log( d, "Changed drunkness to %d", victim->pcdata->condition[COND_DRUNK] );
	break;

    case MEDIT_FAVOR:
	victim->pcdata->favor = URANGE( -2500, atoi(arg), 2500 );
	editor_log( d, "Changed favor to %d", victim->pcdata->favor );
	break;
	...
	case 0:
	    break;
	case 1:
	    EDITOR_MODE(d) = MEDIT_SAV1;
	    send_to_char( "\n\rEnter throw (-30 to 30): ", d->character );
	    return;
	case 2:
	    EDITOR_MODE(d) = MEDIT_SAV2;
	    send_to_char( "\n\rEnter throw (-30 to 30): ", d->character );
	    return;
	case 3:
	    EDITOR_MODE(d) = MEDIT_SAV3;
	    send_to_char( "\n\rEnter throw (-30 to 30): ", d->character );
	    return;
	case 4:
	    EDITOR_MODE(d) = MEDIT_SAV4;
	    send_to_char( "\n\rEnter throw (-30 to 30): ", d->character );
	    return;
	case 5:
	    EDITOR_MODE(d) = MEDIT_SAV5;
	    send_to_char( "\n\rEnter throw (-30 to 30): ", d->character );
	    return;
	}
	...
	editor_log( d, "Changed class to %s", npc_class[victim->class] );
	break;
	...
	editor_log( d, "Changed race to %s", npc_race[victim->race] );
	break;
	...
	editor_log( d, "%s the body part %s", IS_SET( victim->xflags, 1 << (number - 1) ) ? "Added" : "Removed",
	    part_flags[number] );
	medit_disp_parts(d);
	return;
	...
	if ( IS_NPC(victim) && xIS_SET( victim->act, ACT_PROTOTYPE ) )
        ...
        bug( "Soy una rumberaaaa",0);
	medit_disp_attack_menu(d);
	editor_log( d, "%s the attack %s", xIS_SET(victim->attacks,number)? "Added" : "Removed", attack_flags[number] );
 	return;
	...
	medit_disp_defense_menu(d);
	editor_log( d, "%s the attack %s", xIS_SET(victim->defenses,number) ? "Added" : "Removed", defense_flags[number] );
	return;

    case MEDIT_LEVEL:
	victim->level = URANGE( 1, atoi(arg), 64 );
	editor_log( d, "Changed level to %d", victim->level );
	break;

    case MEDIT_ALIGNMENT:
	victim->alignment = URANGE( -1000, atoi(arg), 1000 );
	editor_log( d, "Changed alignment to %d", victim->alignment );
	break;
	...
	medit_disp_ris(d);
	editor_log( d, "%s the resistant %s", IS_SET( victim->resistant, 1 << number ) ? "Added" : "Removed", ris_flags[number] );
	return;
	...
	medit_disp_ris(d);
	editor_log( d, "%s the immune %s", IS_SET( victim->immune, 1 << number ) ? "Added" : "Removed", ris_flags[number] );
	return;
	...
	medit_disp_ris(d);
	editor_log( d, "%s the suscept %s", IS_SET( victim->susceptible, 1 << number ) ? "Added" : "Removed", ris_flags[number] );
	return;
	...
	editor_log( d, "Changes spec_func to %s", lookup_spec( victim->spec_fun ) );
	break;

    case MEDIT_DEITY:
	number = atoi(arg);
	if ( number < 0 || number > editor_top_deity+1 )
	{
	...
	number -= 1;
	deity = get_deity( editor_deity_list[number] );
	if ( !deity )
	{
	    bug( "Unknown deity linked into editor_deity_list.", 0 );
	    break;
	}
	STRFREE( victim->pcdata->deity_name );
	victim->pcdata->deity_name = QUICKLINK( deity->name );
	victim->pcdata->deity = deity;
	editor_log( d, "Deity changed to %s", deity->name );
	break;
	...
	if ( get_trust(d->character) < LEVEL_GOD )
	    break;
	number = atoi(arg);
	if ( number < 0 || number > editor_top_order+1 )
	{
	...
	clan = get_clan( editor_clan_list[number-1] );
	if ( !clan )
	{
	    bug( "Non-existant clan linked into editor_clan_list.", 0 );
	    break;
	}
	...
	editor_log( d, "Clan changed to %s", clan->name );
	break;
	...
	if ( get_trust(d->character) < 58 )
	    break;
	number = atoi(arg);
	if ( number < 0 || number > editor_top_council )
	{
	...
	council = get_council( editor_council_list[number] );
	if ( !council )
	{
	    bug( "Unknown council linked into editor_council_list.", 0 );
	    break;
	}
	STRFREE( victim->pcdata->council_name );
	victim->pcdata->council_name = QUICKLINK( council->name );
	victim->pcdata->council = council;
	editor_log( d, "Council changed to %s", council->name );
	break;

/*-------------------------------------------------------------------*/
    default:
	/*. We should never get here .*/
	bug( "EDITOR: medit_parse(): Reached default case!", 0 );
	limpiar_editor(d);
	return;;
    }
/*-------------------------------------------------------------------*/
/*. END OF CASE
    ...
    return to main menu.  Use EDITOR_CHANGE as a 'has changed' flag .*/

    EDITOR_CHANGE(d) = TRUE;
    medit_disp_menu(d);
}
/*. End of medit_parse() .*/

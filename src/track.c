...
if (!IS_IMMORTAL( ch ))
    ...
    {
	case BFS_ERROR:
	    send_to_char("Hmm... algo debe estar mal.\n\r", ch);
	    break;
	case BFS_ALREADY_THERE:
	    send_to_char("Estas en su misma room!\n\r", ch);
	    break;
	case BFS_NO_PATH:
	    send_to_char("No logras encontrar el camino desde aqui.\n\r", ch);
	    learn_from_failure(ch, gsn_track);
	    break;
	default:
	    ch_printf(ch, "Sientes que esta hacia %s de aqui...\n\r", dir_name[dir]);
...
void found_prey( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	{
	case 0: sprintf( buf, "No me hagas buscarte, %s!", victname );
		do_say( ch, buf );
	        break;
	case 1: act( AT_ACTION, "$n huele por la habitacion buscando a $N.", ch, NULL, victim, TO_NOTVICT );
		act( AT_ACTION, "Hueles por la habitacion buscando a $N.", ch, NULL, victim, TO_CHAR );
		act( AT_ACTION, "$n huele por la habitacion buscandote.", ch, NULL, victim, TO_VICT );
		sprintf( buf, "Puedo oler tu sangre!" );
		do_say( ch, buf );
		break;
	case 2: sprintf( buf, "Te voy a cortar a cachitos, %s!", victname );
		do_yell( ch, buf );
		break;
	case 3: do_say( ch, "Solo espera que te encuentre...");
		break;
	}
	return;
    }
	...
	switch( number_bits( 2 ) )
	{
	case 0:	do_say( ch, "Vamos! Cobarde!" );
		sprintf( buf, "%s es un PUTO cobarde!", victname );
		do_yell( ch, buf );
		break;
	case 1: sprintf( buf, "Acabemos con esto de una vez, %s", victname );
		do_say( ch, buf );
		break;
	case 2: sprintf( buf, "%s es un gallina!", victname );
		do_yell( ch, buf );
		break;
    ...
    switch( number_bits( 2 ) )
    {
     case 0: sprintf( buf, "Tu sangre es mia, %s!", victname );
	     do_yell( ch, buf);
	     break;
     case 1: sprintf( buf, "Nos encontramos otra vez, %s!", victname );
     	     do_say( ch, buf );
     	     break;
     case 2: sprintf( buf, "Que quieres que grabe en tu tumba, %s?", victname );
     	     do_say( ch, buf );
     	     break;
...
void hunt_victim( CHAR_DATA *ch )
{
    ...
    if (!found)
    {
	do_say(ch, "Mierda! Mi presa se ha ido!!" );
	stop_hunting( ch );
	return;
    }

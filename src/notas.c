...
{ "Bugs",	  	"Bugs, fallos, errores",		     0,     2,     "inmortal", DEF_NORMAL, 60, NULL, FALSE },
{ "Personal", 	"Mensajes personales",			 0,     2,     "todos", DEF_EXCLUDE,28, NULL, FALSE },
...
*/
		sprintf( ch->pcdata->in_progress->date, "N/A" );
	}
...
void do_nread (CHAR_DATA *ch, char *argument)
{
	...
	else /* just next one */
	{
		...
		{	
			...
			{	
				sprintf (buf, "&x&wNota en el apartado &g%s&w:\n\r\n\r",ch->pcdata->board->short_name);
				send_to_char (buf,ch);
				show_note_to_char (ch,p,count); /* Advance if new note is newer than the currently newest for that char */
				*last_note =  UMAX (*last_note, p->date_stamp);
				return;
			}
		}
	...
	CHAR_DATA *ch = d->character;
		...
		switch (tolower(argument[0]))
		{
			...
			case 'e': /* post note */
				...
				sprintf(buf, "Tienes una nueva nota de %s en el buzon %s", ch->name, ch->pcdata->board->short_name);
				mensa_nota(ch, "nota", buf);
				break;
			...
			default: /* invalid response */
				send_to_char( "La respuesta no es correcta:\n\r\n\r",ch);
				send_to_char( szFinishPrompt, ch);
				send_to_char("\n\r", ch );

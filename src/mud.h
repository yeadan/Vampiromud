 ...
 * Adaptado mejorado y modificado por COLIKOTRON Code Team(No Copyright)    *
 * Implementadores SiGo Kayser SaNgUiNaRi 199x-200x(QPR)Qpr License         *
 * Bajo derechos de QPR(Que Pague Rita). Proyecto GNU.                      *
 ****************************************************************************/
 ...
 * 1-. Minimo
 * 2-. Normalillo
 * 3-. Brutal, demasiada informacion para ser usada remotamente :D
 * 4-. Ni te kuento.
 */
#define DEPURANDO 		3
...
typedef struct  conquistas_data         CON_DATA; /* Estructuira de conquistas */
typedef struct	affect_data		AFFECT_DATA;
typedef struct  esfera_table            ESFERA; /* Estructura de esferas y sus niveles */
typedef struct  disci_table             DISCI; /* Estructura de niveles de disciplinas */
typedef struct  disci_nivel             NIVEL; /* Estructura de niveles de las disciplinas */
typedef struct  disci_affect            DISCI_AFFECT; /* Affects de las disciplinas */
typedef struct  esfera_affect           ESFERA_AFFECT; /* Afects de las esferas */
typedef struct	area_data		AREA_DATA;
typedef struct  quest_data              QUEST_DATA; /* data de los quests */
...
typedef struct  veditor_data            VEDITOR_DATA;
 ...
 */
#define MAX_QDATA            10   /* maximo de datas de quest */
...
#define MAX_NIVEL_DISCIPLINA                 10           /* Maximo nivel de Disciplina */
#define MAX_PODERES        330                /* Maximo numero de poderes */
#define MAX_DISCIPLINAS                            35           /* Maximo numero de disciplinas */
#define MAX_ESFERAS                 9 /* Maximo numero de esferas */
#define MAX_FAMILIAS                                    5       /* Maximo de Familias */
#define MAX_COMANDOS                           500      /* Maximo numero de Comandos */
#define EDAD_REMORT                                255     /* Edad pa hacer el remort */

#define MAX_REXITS		   20	/* Maximum exits allowed in 1 room */
#define MAX_SKILL		  500
#define MAX_TIPOS                  18   /* Maximo de tipos de disciplina */
#define SPELL_SILENT_MARKER   "silent"	/* No OK. or Failed. */
#define MAX_CLASS                40
#define MAX_NPC_CLASS		   26

#define MAX_RACE                   25
#define MAX_NIVEL_TABLA            15 /*Maximo tabla_nivel? :P */
#define MAX_NPC_RACE		   91
#define MAX_PC_DISCIPLINAS                      10

extern int MAX_PC_RACE;
extern int MAX_PC_CLASS;
extern int total_clases;
/* -----------------08/12/00 2:59--------------------
 *   Definiciones de las Generaciones - Rangos del
 *   personaje. SiGo
 * --------------------------------------------------*/
#define MAX_GENERACION        13
#define GENERACION_CAIN       (MAX_GENERACION - 12)
#define GENERACION_SEGUNDA    (MAX_GENERACION - 11)
#define GENERACION_ANTEDILUVIANO       (MAX_GENERACION - 10)
#define GENERACION_MATUSALEN  (MAX_GENERACION - 9)
#define GENERACION_QUINTA     (MAX_GENERACION - 8)
#define GENERACION_SEXTA     (MAX_GENERACION - 7)
#define GENERACION_SEPTIMA     (MAX_GENERACION - 6)
#define GENERACION_OCTAVA     (MAX_GENERACION - 5)
#define GENERACION_NOVENA     (MAX_GENERACION - 4)
#define GENERACION_DECIMA     (MAX_GENERACION - 3)
#define GENERACION_DECIMOPRIMERA     (MAX_GENERACION - 2)
#define GENERACION_DECIMOSEGUNDA     (MAX_GENERACION - 1)
#define GENERACION_NODEFINIDA       0

/* ----------------31/06/01 7:00----------------------- *
 * Definicion de los chiquillos sires y demas partes fundamentales del abrazar           *
 *        SiGo y SaNgUi vampiromud@hotmail.com                                                   *
 * ------------------------------------------------ */

#define MAX_CHIQUILLOS                 25
#define IS_ROLLPLAYER(ch)                                   IS_VAMPIRE(ch)
#define ES_CHIQUILLO(ch)                                      (xIS_SET((ch)->act, PLR_ABRAZADO))


/*
 * DECLARE_DO_FUN cambiado a ddf (mira ke son lelos)
 */

#define ddf DECLARE_DO_FUN

ddf( do_critico                 );  /* Critico comun por el momento */
ddf( do_potencia                );  /* Potencia */
ddf( do_fascinacion             );  /* Presencia */
ddf( do_mirada		        );  /* Presencia */
ddf( do_celeridad               );  /* Celeridad */
ddf( do_fortaleza               );  /* Fortaleza */
ddf( do_vaff                    );  /* Utilidad */
ddf( do_haff                    );  /* Utilidad */
ddf( do_capasombria             );  /* Ofuscacion */
ddf( do_pasion                  );  /* Dementacion */
ddf( do_embrujar                );  /* Dementacion */
ddf( do_coma                    );  /* Dementacion */
ddf( do_dominio                 );  /* Dominacion */
ddf( do_mesmerismo              );  /* Dominacion */
ddf( do_memoria_del_juerguista  );  /* Dominacion */
ddf( do_oleadademente           );  /* Dementacion */
ddf( do_cancionserena           );  /* Dementacion */
ddf( do_escudobaali             );  /* Daimoinon */
ddf( do_llamassubmundo          );  /* Daimoinon */
ddf( do_llamadasalvaje          );  /* Animalismo */
ddf( do_susurrosamables         );  /* Animalismo */
ddf( do_sentidosaguzados        );  /* Auspex */
ddf( do_visiondelalma           );  /* Auspex */
ddf( do_toqueespiritual         );  /* Auspex */
ddf( do_proyeccionpsiquica      );  /* Auspex */
ddf( do_petrificarse            );  /* Taumaturgia */
ddf( do_silenciodelamuerte      );  /* Extincion */
ddf( do_debilidad               );  /* Extincion */
ddf( do_enfermedad              );  /* Extincion */
ddf( do_tormento	        );  /* Extincion */
ddf( do_testigodelastinieblas   );  /* Protean */
ddf( do_garrasdelabestia        );  /* Protean */
ddf( do_enterramiento           );  /* Protean */
ddf( do_mascara		        );  /* Mortis  */
ddf( do_aprender                );  /* Utilidad para aprender esferas */
ddf( do_esferas                 );  /* Para ver las esferas que se poseen */
ddf( do_dset                    );  /* Para editar las disciplinas on line */
ddf( do_dver                    );  /* Para ver datos de las disciplinas */
ddf( do_creardisciplina         );  /* Para crear disciplinas on line */
ddf( do_crearnivel              );  /* Crea un nivel de disciplina */
ddf( do_nset                    );  /* Modifica los valores de los niveles */
ddf( do_nver                    );  /* Para ver los datos de los diefrentes niveles */
ddf( do_concentrar              );  /* Para lanzar niveles de disciplinas */
ddf( do_conjurar                );  /* Para lanzar niveles de esferas */
ddf( do_renacer                 );  /* Para hacer el remort */
ddf( do_infoarea                );  /* Para ver cuantos puntos tiene cada familia en un area */
ddf( do_tiempo                  );  /* Esfera Tiempo */
ddf( do_crearesfera             );  /* Crea un nivel de disciplina */
ddf( do_eset                    );  /* Modifica los valores de los niveles */
ddf( do_ever                    );  /* Para ver los datos de las diferentes esferas */
ddf( do_reclamar                );  /* Para reclamar los objetos quest */
...
#define QUEST_ITEM_SIMBOLO       236
#define QUEST_ITEM_OJOS          237
#define QUEST_ITEM_ESPADA        238
#define QUEST_ITEM_DAGA          239
#define QUEST_ITEM_IRA           235
#define QUEST_ITEM_CAIN          234
#define QUEST_ITEM_MUERTE        233
/*
 * Esperemos que Funcione xD SiGo y SaNgUi
 */

/*
 *  disciplinas Vampiricas y Sendas Taumaturgas  SiGo y SaNgUi  2001,modificado por Nkari en el 2004
 */
...
#define DISCIPLINA_VICISITUD     31
#define DISCIPLINA_VISCERATIKA   32
#define DISCIPLINA_VALEREN       33
#define DISCIPLINA_COMUN         34
...
/*
 * Util para el sistema Vampire Roll
 */
#define MAX_DROGAS 5
#define MAX_INFLUENCIAS 3
...
#define LEVEL_HERO		   (MAX_LEVEL - 17)
...
#define LEVEL_ENGENDRO             (MAX_LEVEL - 15)
#define LEVEL_SUPERAVATAR          (MAX_LEVEL  - 16)
#define LEVEL_AVATAR		   (MAX_LEVEL - 17)

#define LEVEL_LOG		    LEVEL_LESSER
#define LEVEL_HIGOD		    LEVEL_GOD

#include "bank.h"               /* Banko */
#include "rare.h"
#include "board.h" /* Notas */
#include "finger.h"
...
#define SECONDS_PER_PLAYER_TICK		  2
#define PULSE_PER_SECOND			  4
#define PULSE_VIOLENCE				 (3 * PULSE_PER_SECOND)
#define PULSE_MOBILE				 (4 * PULSE_PER_SECOND)
#define PULSE_TICK					 (SECONDS_PER_TICK * PULSE_PER_SECOND)
#define PULSE_PLAYER_TICK			 (SECONDS_PER_PLAYER_TICK * PULSE_PER_SECOND)
 ...
 * SMAUG Version -- Scryn
 */
#define VAMPIRO_VERSION_MAJOR "2"
#define VAMPIRO_VERSION_MINOR "0"
...
typedef enum
{
  ...
  CON_COPYOVER_RECOVER, CON_NOTE_TO,            CON_NOTE_EXPIRED,
  CON_NOTE_SUBJECT,     CON_NOTE_EXPIRE,        CON_NOTE_TEXT,
  CON_NOTE_FINISH,      CON_MEDIT,              CON_OEDIT,
  CON_REDIT
} connection_types;
...
// Yo anyadiria para la destreza un sh_int q aumente el hit (Sherlock).
struct	dex_app_type
{
    sh_int	defensive;
};
...
#define CLASS_MAGE       0  /*SiGo y SaNgUi*/
#define CLASS_CLERIC     1  /*SiGo y SaNgUi*/
#define CLASS_THIEF      2  /*SiGo y SaNgUi*/
#define CLASS_WARRIOR       3  /*SiGo y SaNgUi*/
#define CLASS_VAMPIRE       4  /*SiGo y SaNgUi*/
#define CLASS_DRUID      5     /*SiGo y SaNgUi*/
#define CLASS_RANGER     6     /*SiGo y SaNgUi*/
#define CLASS_AUGURER       7 /* 5-12-2000 SiGo y SaNgUi*/
#define CLASS_PALADIN       8 /* 5-12-2000 SiGo y SaNgUi*/
#define CLASS_MALKAVIAN      9          /*SiGo y SaNgUi*/
#define CLASS_TREMERE    10   /* 5-12-2000 SiGo y SaNgUi*/
#define CLASS_NOSFERATU      12  /*SiGo y Sangui*/
#define CLASS_BAALI       11  /*SiGo y SaNgUi*/ 
#define CLASS_GANGREL     13  /*SiGo y SaNgUi*/
#define CLASS_TOREADOR    14            /* SiGo y SaNgUi */
#define CLASS_BRUJAH      15            /* SiGo y SaNgUi */
#define CLASS_RAVNOS      16            /* SiGo Y SaNgUi */
#define CLASS_SALUBRI     17  /* SiGo y SaNgUi */
#define CLASS_VENTRUE     18            /* SiGo y SaNgUi */
#define CLASS_TZIMISCE   19 /* SiGo y SaNgUi */
/*#define CLASS_CAPADOCIO    20  */ /*ES LA CINCO!!*/
#define CLASS_BLOODWALKER     24        /* SiGo y SaNgUi */
/*#define CLASS_ASSAMITA     22*//* ES LA CUATRO!!*/
#define CLASS_SETITA       20   /*Nkari*/
#define CLASS_LASOMBRA     21   /*Nkari*/
#define CLASS_GIOVANNI     23   /*Nkari*/
/*
 * Languages -- Altrag
 */
 ...
 * TO types for act.
 */
typedef enum { TO_ROOM, TO_NOTVICT, TO_VICT, TO_MUD, TO_CHAR, TO_FAMILIA, TO_CANSEE } to_types;
/*
...
#define AT_HIT		   AT_GREY
#define AT_HITME	   AT_DGREY
#define AT_IMMORT	   AT_DGREY
...
struct	class_type
{
    ...
    int                         disci_class1; /* disciplinas de clase */
    int                         disci_class2; /* disciplinas de clase */
    int                         disci_class3; /* disciplinas de clase */
    sh_int      njugadores;             /* Numero de jugadores de la clase conectados */
...
struct	clan_data
{
    ...
    int         puntos;         /* Puntos de familia */
    sh_int         posicion;       /* Posicion de la Familia */
    char      *aliados[10]; /* Aliados de la familia */
    char      *enemigos[10]; /* Enemigos de la familia */
    char      *tregua; /* Treguas de la familia */
    sh_int      odio[10]; /* Odio hacia la familia */
    sh_int      alianza[10]; /* Nivel de alianza de las familias */
    char        *proalianza[10]; /* Propuestas de alianza */
    char        *deguerra[10]; /* Propuesta de guerra */
    sh_int      nv_alianza[10];     /* Nivel de la propuesta de alianza */
    sh_int      nv_guerra[10];     /* Nivel de la declaracion de guerra */
    int      oro; /* Oro acumulado por la familia */
    int      experiencia; /* Experiencia acumulada por la familia */
    sh_int      tributo[5];     /* Tributos que han de pagarse */
    sh_int      castigos[5];    /* Castigos a los que han de acojerse */
    int           impuestos[1];    /*  Impuestos a las demas familias */ /* Impuetso de oro e impuesto de px */
    sh_int           numero;
    int                 conquistas; /* Conquistas del clan */
    sh_int    puntos_conquista[35]; /* Puntos de la familia en el area */ /* A eliminar */
    char         *propietario[35]; /* Nombre del propietario de las area */
    sh_int             agresor[10];   /* ¿Es agresor? */
    int       muerte_guerra[10]; /* Para ver cuantos miembros han muerto en una guerra */
    int       victorias_guerra[10]; /* Lo kontrario ke lo de arriba xD */
    int         tierras; /* Tierras en propiedad de la familia */
    int         victorias;
    int         derrotas;
    int         madera;
    int         mineral;
    ...
    int         agua;
    int         combustibles;
    int         drogas[MAX_DROGAS]; /* Ole ole XD */
    int         calidad[MAX_DROGAS]; /* Calidad de las reservas de droga */
    int         seguidores;
    char        *estatus_social;
    int         deudas;
    int         morgue; /* Vnum de la morgue del clan */
    sh_int      influencia[MAX_INFLUENCIAS]; /* Influencia en las decisiones a tomar por los EMPLEMENTADORES
    y los DIOSES (sin excederse ni enturbiar), influencia social, influencia economica, etc, etc */
...
struct disci_affect
{
    ...
    EXT_BV              bitvector;
    int                 bitvectores;
    DISCI_AFFECT *      proximo;   //Este no es igual q el primer campo? (Sherlock);
    char *              modifier;
    char *              dura;
    sh_int              ritual; /* Es un ritual? */
    sh_int              senda;  /* Es una senda? */
};

/*
 * Affects para las esferas
 */
struct esfera_affect
{
    ESFERA_AFFECT *     next;
    ESFERA_AFFECT *     prev;
    sh_int              tipo;
    ...
    EXT_BV              bitvector;
    int                 bitvectores;
    ESFERA_AFFECT *     proximo;
    char *              modifier;
    char *              dura;
    sh_int              combinada; /* Es producto de una combinacion? */
    sh_int              pura; /* Es una esfera pura? */
};
...
#define MOB_VNUM_HERALDO_INFERNAL  4
...
#define ACT_BANKER                3	        /* banquero y tal       */
#define ACT_HEALER                4             /* Curanderos           */
...
#define ACT_QUESTMASTER		 31             /* Questmaster */
#define ACT_NOQUEST              32             /* No es un mob Quest */
#define ACT_AUTOMOB              33             /* Es un mob automatico (como el a4 exclusive) */
#define ACT_UALIZADO             34             /* Actualizacion de utomobs */
#define ACT_DOMINADO             35             /* Mob dominado? */
#define ACT_VACIO                36
...
typedef enum
{
  ...
  AFF_CONTAGIOUS, AFF_ACIDMIST,  AFF_VENOMSHIELD, AFF_HABILIDAD,
  AFF_PROTECCION_VAMPIRICA, MAX_AFFECTED_BY
} affected_by_types;
 ...
 * Resistencias etc
 */
#define SAV_CREO_IGNEM                  BV00
#define SAV_CALDERO_SANGUINEO           BV01
#define SAV_DEBILIDAD                   BV02
#define SAV_OLEADA_DEMENTE              BV03
#define SAV_AGRAVADAS                   BV04
#define SAV_COMA                        BV05
#define SAV_PASION                      BV06
#define SAV_BACKSTAB                    BV07
#define SAV_TANGANA                     BV08
#define SAV_FUEGO                       BV09
#define SAV_HIELO                       BV10
#define SAV_ENERGIA                     BV11
#define SAV_ACIDO                       BV12
#define SAV_VENENO                      BV13
#define SAV_LUZ_SOLAR                   BV14
#define SAV_SAGRADO                     BV15
#define SAV_MAGIA                       BV16
#define SAV_DAGAS                       BV17
#define SAV_ESTACAS                     BV18
#define SAV_ESPADAS                     BV19
#define SAV_DOMINACION                  BV20
#define SAV_SANGRE_DE_HADA              BV21
#define SAV_GOLPES                      BV22
#define SAV_ESFERA_DIVINA               BV23
#define SAV_OSCURA                      BV24
/* 24 Saves */
...
typedef enum
{
  ...
  ATCK_WEAKEN, ATCK_SPIRALBLAST, ATCK_DRAINBREATH, MAX_ATTACK_TYPE
} attack_types;
...
typedef enum
{
  ...
  ITEM_PROTOTYPE, ITEM_NOLOCATE, ITEM_GROUNDROT, ITEM_LOOTABLE, ITEM_QUEST,
  ITEM_RARE, ITEM_UNIQUE, ITEM_TAQUILLA, ITEM_CASUAL, ITEM_AUTOITEM, MAX_ITEM_FLAG
} item_extra_flags;
...
#define ITEM_WEAR_SATELITE      BV22
...
#define ROOM_VNUM_MORGUE          21199
#define ROOM_VNUM_DONATION        21193
#define ROOM_VNUM_DONATIONTWO     21192
...
typedef enum
{
  ...
  POS_EVASIVE, POS_STANDING, POS_MOUNTED, POS_SHOVE, POS_DRAG, POS_PREDECAP
} positions;
...
typedef enum
{
  PLR_IS_NPC,PLR_QUESTOR,PLR_BOUGHT_PET, PLR_SHOVEDRAG, PLR_AUTOEXIT, PLR_AUTOLOOT,
  ...
  PLR_AUTOGOLD, PLR_AUTOMAP, PLR_AFK, PLR_INVISPROMPT, PLR_AUTODANYO,
  PLR_ABRAZADO, PLR_SIRE, PLR_CHIQUILLO,   PLR_WIZINCOG,  PLR_RENACIDO, /* SiGo y SaNgUi */
  PLR_CAMUFLAJE, PLR_PRESENCIA,PLR_PKTOTAL,  
  PLR_TRAUMATIZADO, PLR_CORAZON_PETREO, PLR_SENTIDOS_AGUZADOS, PLR_SILENCIO_MORTAL, 
  PLR_DEBILIDAD, PLR_DOMINADO, PLR_INCORPOREO, PLR_TEMPORIS,
  PLR_ALTERADO
 } player_flags;

/*
 * Affects de las disciplinas
 */
typedef enum
{
  DAF_POTENCIA, DAF_FORTALEZA, DAF_CELERIDAD,  
  DAF_TRAUMATIZADO, DAF_CORAZON_PETREO, DAF_CAPA_SOMBRIA, DAF_SENTIDOS_AGUZADOS,
  DAF_SILENCIO_MORTAL, DAF_DEBILIDAD, DAF_TESTIGO, DAF_GARRAS_BESTIA, DAF_DOMINADO,
  DAF_INCORPOREO, DAF_KOMA, DAF_MASCARA, DAF_FASCINACION, DAF_MIRADA, DAF_EMBRUJADO,
  DAF_ENFERMEDAD
} disci_flags;

/*
 * Esferas para jugadores
 */
typedef enum
{
  ESF_CARDINAL, ESF_CORRESPONDENCIA, ESF_ENTROPIA, ESF_ESPIRITU, ESF_FUERZAS,
  ESF_MATERIA, ESF_MENTE, ESF_TIEMPO, ESF_VIDA
} player_esferas;

/*
 * Disciplina para jugadores
 */
typedef enum
{
  DIS_ANIMALISMO, DIS_AUSPEX, DIS_BARDO, DIS_CELERIDAD, DIS_CONOCIMIENTO, DIS_DAIMOINON,
  DIS_DEMENTACION, DIS_DOMINACION, DIS_EXTICION, DIS_KINETISMO, DIS_FORTALEZA, DIS_MELOPOMENE,
  DIS_MORTIS, DIS_MYTHECERIA, DIS_NIGOMANCIA, DIS_NIHILISTICA, DIS_OBEAH, DIS_OBTENEBRACION,
  DIS_OFUSCACION, DIS_PRESENCIA, DIS_POTENCIA, DIS_PROTEAN, DIS_QUIMERISMO, DIS_SANGUINUS,
  DIS_SERPENTIS, DIS_SPIRITUS, DIS_TANATOSIS, DIS_TEMPORIS, DIS_TAUMATURGIA, DIS_DARKTARMA,
  DIS_VISCISITUD, DIS_VISCERATIKA, DIS_COMUN,DIS_VALEREN
}dis_table;
...
typedef enum
{
  TIPO_NINGUNO, TIPO_DANYO, TIPO_CURATIVA, TIPO_CELERIDAD, TIPO_USOPROPIO,
  TIPO_PROTECCION, TIPO_SUBTERFUGIO, TIPO_SENDA, TIPO_RITUAL, TIPO_TAUMATURGIA,
  TIPO_MENTAL, TIPO_ALMA, TIPO_INVOCACION, TIPO_AGRAVADAS, TIPO_FRENESI,
  TIPO_BERSERCK, TIPO_ROTHSERKER
}disci_tipos;
...
typedef enum
{
  AP_NADA, AP_FUE, AP_DES, AP_INT, AP_SAB, AP_CON,
  AP_SEX, AP_CLASE, AP_NIVEL, AP_EDAD, AP_ALTURA, AP_PESO,
  AP_MANA, AP_VIDA, AP_MOVE, AP_ORO, AP_EXP, AP_AC,
  AP_HITROLL, AP_DAMROLL, AP_SAVING_POISON, AP_SAVING_ROD,
  AP_SAVING_PARA, AP_SAVING_BREATH, AP_SAVING_SPELL, AP_CAR,
  AP_AFFECT, AP_RESISTENTE, AP_IMMUNE, AP_SUSCEPTIBLE,
  AP_FVOL, AP_SUE, AP_MAN, AP_INTI, AP_CAMINO,
  AP_FRENESI, AP_OCULTISMO, AP_GENERACION, AP_SANGRE, AP_AGRAVADAS, AP_DRUNK,
  AP_MENTAL, AP_POSICION, AP_ASTUCIA, AP_ALERTA, AP_REMOVESPELL,
  AP_REMOVE, MAX_AP_TIPO
}ap_tipo;

#define REVERSE_AP                 1000
...
#define PCFLAG_PRIVACY             BV18 /* Privacidad del finger */
#define PCFLAG_DECAPITADO          BV19 /* Para los decapitados */
...
struct	char_data
{
    ...
    CHAR_DATA *		master;
    CHAR_DATA *		leader;
    CHAR_DATA *      challenged;        /* Empezar torneos entre players esctructuracion
                                         * SiGo y SanGui */
    CHAR_DATA *      betted_on;
    int              bet_amt;
    FIGHT_DATA *	fighting;
    ...
    DISCI_AFFECT *      primer_affect;
    DISCI_AFFECT *      ultimo_affect;
    ESFERA_AFFECT *     primer_eaffect;
    ESFERA_AFFECT *     ultimo_eaffect;
    NOTE_DATA *		pnote;
    NOTE_DATA *		comments;
    ...
    int        gen;
    int			exp;
    int        exp_acumulada;           /* Experiencia de avatares */
    EXT_BV		act;
    EXT_BV		affected_by;
    EXT_BV              afectado_por; /* Affects de las dsciplinas */
    EXT_BV		no_affected_by;
    EXT_BV              esferas; /* Esferas para magos y Clerigos */
    int			carry_weight;
    int			carry_number;
    int			xflags;
    int			no_immune;
    ...
    int                             puntos;
    char*                         chiquillos[MAX_CHIQUILLOS];
    sh_int              num_chiquillos;
    char*                         sire_ch;
    int                             edad;
    int                             reputacion;
    int                             sire_class;
   sh_int                     cnt_abrazo; /* Contador de PLR_ABRAZADO para el abrazo de jugadores por Vampiros */
   sh_int                     cnt_adiestro; /* Contador de PLR_ADIESTRO para el adiestramiento de jugadores por NoVampiros */
   sh_int                     contador_adquirir; /* Cuenta atras en minutos del tiempo a transcurrir entre la adquisiscion de disciplinas/habilidades */
   sh_int                     disci_tope;     /* Tope de disciplinas aprendidas segun generaciones */
   sh_int                     hab_tope;    /* Tope de habilidades aprendidas segun rangos */
  int                       revision; /* Revision de ficha y actualizacion de personajes ole mi cuerpo serrano xD eR SaNgUi */
   sh_int		tmplevel;  /* Para poder canear a unos rufianes mortales y kedar como un senyorito xD */
    sh_int		tmptrust;  /* SiGo y SanGuI FuMaNDo Y TeCLeaNDo LeLoS NoS VaMoS KeDaNDo */
    int                         conquistas; /* Conquistas del jugador SiGo SaNgUi */
    int                 tira_dado; /* Para las tiradas de dados */
    sh_int              f_voluntad; /* Fuerza de voluntad */
    int                 tiempo_fv; /* Tiempo para volver a entrenar la fuerza de voluntad */
    sh_int              pcamino; /* Puntuacion de camino */
    char*                camino; /* El camino que sigue */
    int                 frenesi; /* Puntuacion de frenesi */
    int                 agravadas; /* Heridas agravadas */
    char*                 posicion;  /* Posicion social del Personaje */
    int                 manipulacion; /* Nivel de manipulacion */
    char*                cazas[5]; /* Las cazas de sangre declaradas contra el */
    char*                recompensa[5];  /* Las clases de recompensa que hay contra el */
    char*                cazador[5];     /* Quien declara la caza */
    char*                camuflaje; /* Camuflaje */
    sh_int               modo_ficha; /* Modo de la ficha del jugador OJO No soportado por SMAUG1.4*/
    sh_int               astucia; /* Astucia del jugador 0 - 10 */
    sh_int               alerta; /* Alerta del personaje 0 - 10 */
    sh_int               intimidacion; /* Intimidacion del personaje 0 - 10 */
    sh_int               ocultismo; /* Nivel de ocultismo del personaje 0 - 10 */
    sh_int               cnt_pktotal; /* Tiempo ke dura el flag PkTotal */
    sh_int               dificultad; /* Esto solo se aplica a mobs para el automob */
    sh_int               nivel_real; /* Nivel original del automob solo en mobs */
};

struct          tabla_generaciones
{
        char*           nombre;
};
...
struct	pc_data
{
    ...
    DEITY_DATA *	deity;
    NOTA_DATA *         board;
    time_t              last_note[MAX_BOARD]; /* Ultima nota del apartado */
    char *		homepage;
    char *     betted_on;
    char *		clan_name;
    char * 		council_name;
    ...
    int         balance; /* balance bankario */
    int        pkills;     /*(Numero de veces que se ha matado en pk 'clan') Number of pkills on behalf of clan */
    int        genepkills; /* Numero de muertos para mejorar la generacion del personaje*//* SiGo*//* muesca */
    int        genepdeaths; /* Numero de veces muerto para generacion del personaje *//*SiGo*/
    int        pdeaths; /* (Numero de veces matado legalmente) Number of times pkilled (legally)  */
    int			habilidades[34]; /* Disciplinas o habilidades */
    int			disci_puntos;  /* Puntos de disciplina */
    int			disci_adquirir; /* Para adquirir una disciplina */
    ...
    sh_int              nextquest; /* Questmaster */
    sh_int              countdown; /* Questmaster */
    sh_int              questobj; /* Questmaster */
    sh_int              questmob; /* Questmaster */
    /* Fin del sisteam quest */
    ...
    sh_int                     renacido; /* SiGo y SanGuI, esto como ya se ve es pal remort */
  int         mana_ant; /* Mana anterior del remort */
  int         vida_ant; /* Vida anterios del remort */
  int         mov_ant;  /* Movimiento anterior del remort */
  int         max_level; /* Nivel maximo del jugador */
  sh_int        descanso; /* Descanso de los vampiros mientras duermen */
  sh_int        cnt_pasion;    /* Contador */
  sh_int        modificador_mental; /* Contador */
  char*         email;
  int           icq;
  int           coeficiente; /* Coeficiente de batalla */
  sh_int        cnt_critico;  /* Contador para golpes criticos */
  sh_int        cnt_dominacion; /* Contador */
  sh_int        horas_nivel; /* Contador de horas en el nivel para que no puedan entrenar mas de 1000 qp por nivel */
  sh_int        alterado; /* Estatus de alteracion de jugador */
};


/*
 * Quest                                 Snippet original de LrdElder adaptado totalmente para Mundo de Tinieblas
 *                                       por: COLIKOTRON Code Team vampiromud@hotmail.com
 */
struct  quest_data
{
    int 	rango_nivel;
    int	 	num_prac;
    int		coste_prac;
    int		premio_vnum[MAX_QDATA+1];
    int		premio_valor[MAX_QDATA+1];
    int		premio_oro;
    int		coste_oro;
};
...
struct	obj_index_data
{
    ...
    sh_int		layers;
    int			rent;			/* Unused */
    //EXT_BV		  vflags; // Flags de VampiroMuD
};
...
struct	obj_data
{
    ...
    char *              creator; /* Creador del objeto */
    char *              propietario; /* Propietario del objeto */
};
...
struct	area_data
{
    ...
    sh_int                                      numero;       /* Numero de areas */    /* A eliminar */
    sh_int                                      conquistable; /* Se puede conquistar */
    char *              propietario; /* propietario del area */
    sh_int                                      revisada;       /* Esta revisada */
    int                                         room_total;     /* Kuantas rooms tiene el area (no se guarda en ningun sitio) */
    sh_int              puntos[MAX_FAMILIAS]; /* Puntos conseguidos por las familias */
};
...
struct	skill_type
{
    ...
    sh_int  type;       /* Spell/Skill/Weapon/Tongue/Disciplina  */
    ...
    int     disciplina;                 /* Disciplina SiGo y SaNgUi */
    int     nv_disciplina;              /* Nivel disciplina SiGo y SaNgUi */
    sh_int      para_vampiros; /* Es una habilidad que puedan usar losvampiros? */
    sh_int	remort; 
...
/* Anyadimos empalar y tangana en la generacion de numeros de skill */
extern  sh_int  gsn_empalar;
extern  sh_int gsn_tangana;
extern  sh_int  gsn_cook;
extern	sh_int	gsn_dodge;
extern   sh_int   gsn_study; /* SiGo Y SaNgUi */
...
extern   sh_int   gsn_llamas_submundo;     /* para fireshield  *//* SiGo */
 ...
 * Conquistas
 */
#define ES_PROPIETARIO( ch )            ((ch)->pcdata->clan->propietario[ch->in_room->area->numero] == ch->pcdata->clan->name)
#define ES_CONQUISTABLE(area)           ((area)->conquistable == 0 )
#define ESTA_REVISADA(area)             ((area)->revisada != 0 )
...
#define IS_IMMORTAL(ch)    (get_trust((ch)) >= LEVEL_IMMORTAL+2)
/*#define IS_HERO(ch)		(get_trust((ch)) >= LEVEL_HERO)*/
#define IS_HERO(ch)		(get_trust((ch)) >= LEVEL_AVATAR)
#define IS_AFFECTED(ch, sn)	(xIS_SET((ch)->affected_by, (sn)))
#define HAS_BODYPART(ch, part)	((ch)->xflags == 0 || IS_SET((ch)->xflags, (part)))

#define ES_LICANTROPO(ch)   ((ch)->class == 6)
#define ES_HADA(ch)         ((ch)->class == 8)
#define ES_MAGO(ch)         ((ch)->class == 0)
#define ES_LADRON(ch)       ((ch)->class == 2)
#define ES_CRUZADO(ch)      ((ch)->class == 7)
#define ES_REMORT(ch)          (xIS_SET((ch)->act, PLR_RENACIDO ))  /* Es remort el jugador? */
#define ES_ANCIANO(ch)          (get_age((ch)) >= EDAD_REMORT) /* Edad minima pal remort */


#define IS_VAMPIRE(ch)    ((!IS_NPC(ch)                 \
            &&  ((ch)->class==CLASS_VAMPIRE             \
            ||  (ch)->class==CLASS_BAALI                \
            ||  (ch)->class==CLASS_TREMERE              \
            ||  (ch)->class==CLASS_MALKAVIAN            \
            ||  (ch)->class==CLASS_LASOMBRA             \
            ||  (ch)->class==CLASS_SETITA             \
	    ||  (ch)->class==CLASS_GANGREL              \
            ||  (ch)->class==CLASS_GIOVANNI             \
            ||  (ch)->class==CLASS_TZIMISCE            \
            ||  (ch)->class==CLASS_BLOODWALKER          \
            ||  (ch)->class==CLASS_RAVNOS               \
            ||  (ch)->class==CLASS_SALUBRI              \
            ||  (ch)->class==CLASS_TOREADOR             \
            ||  (ch)->class==CLASS_DRUID		\
	    ||  (ch)->class==CLASS_VENTRUE              \
            ||  (ch)->class==CLASS_BRUJAH               \
            ||  (ch)->class==CLASS_NOSFERATU)))

#define ES_AFECTADO(ch)      (( xIS_SET(ch->afectado_por, DAF_FORTALEZA)         \
                             || xIS_SET(ch->afectado_por, DAF_MASCARA) \
			     || xIS_SET(ch->afectado_por, DAF_CAPA_SOMBRIA) \
			     || xIS_SET(ch->afectado_por, DAF_POTENCIA) \
                             || xIS_SET(ch->afectado_por, DAF_SILENCIO_MORTAL)         \
			     || xIS_SET(ch->afectado_por, DAF_CELERIDAD)         \
                             || xIS_SET(ch->afectado_por, DAF_TESTIGO) \
                             || xIS_SET(ch->afectado_por, DAF_CORAZON_PETREO)         \
                             || xIS_SET(ch->afectado_por, DAF_DEBILIDAD)         \
			     || xIS_SET(ch->afectado_por, DAF_ENFERMEDAD)         \
                             || xIS_SET(ch->afectado_por, DAF_KOMA)         \
                             || xIS_SET(ch->afectado_por, DAF_SENTIDOS_AGUZADOS) \
			     || xIS_SET(ch->afectado_por, DAF_INCORPOREO) \
	                     || xIS_SET(ch->afectado_por, DAF_FASCINACION) \
			     || xIS_SET(ch->afectado_por, DAF_MIRADA)       \
		   	     || xIS_SET(ch->afectado_por, DAF_EMBRUJADO)    \
		             || xIS_SET(ch->afectado_por, DAF_TRAUMATIZADO)      \
                             || xIS_SET(ch->afectado_por, DAF_GARRAS_BESTIA)))

#define IS_GOOD(ch)     ((ch)->alignment >= 350)
#define WAS_IMMORTAL(ch)	(!IS_NPC((ch)) && (ch)->tmplevel > LEVEL_HERO) /* Pa canear mortales lo dicho ke ciego voy yujuuuuuuuuuuuuu!!! */
...
struct	cmd_type
{
    ...
    int                          estado; /* Avtivado o desactivado SiGo y SaNgUi */
    int                          disciplina; /* Disciplina a la que corresponde 0 ninguna */
    int                          disci_level;   /* Nivel de la disciplina */
};

/*
 * Estructura de esferas magicas
 */
struct esfera_table
{
   ESFERA *             next; /* Siguiente esfera en la lista */
   ESFERA *             prev; /* Esfera previa en la lista */
   char *               fichero; /* Nombre del fichero de la esfera */
   char *               nombre; /* Nombre de la esfera */
   char *               descripcion; /* Descrpcion de la esfera */
   sh_int               esfera; /* Que numero de esfera le corresponde */
   sh_int               wait; /* Wait que produce su uso */
   sh_int               tipo; /* Tipo de esfera */
   sh_int               enivel; /* Nivel al que correponde */
   DO_FUN *             do_fun; /* Funcion */
   char *               cabala; /* A que cabala pertenece */
   char *               niveles[10]; /* Niveles qye incluye */
   sh_int               mana;  /* Mana que requiere */
   char *               dammes; /* Mensaje de danyo */
   char *               sevames; /* Mensaje de cuando se va el afect si lo da */
   char *               exitoch;        /* Mensaje de exito para el jugador */
   char *               exitovict;      /* Mensaje de exito para la victima */
   char *               exitoroom;      /* Mensaje de exito para la room */
   char *               exitomud;       /* Mensaje de extito para todo el MUD */
   char *               falloch;        /* Mensaje de fallo para el jugador */
   char *               fallovict;      /* Mensaje de fallo para la victima */
   char *               falloroom;      /* Mensaje de fallo para la room */
   char *               fallomud;       /* Mensaje de fallo para todo el MUD */
   char *               muertech;        /* Mensaje de muerte para el jugador */
   char *               muertevict;      /* Mensaje de muerte para la victima */
   char *               muerteroom;      /* Mensaje de muerte para la room */
   char *               muertemud;       /* Mensaje de muerte para todo el MUD */
   char *               inmunech;        /* Mensaje de inmune para el jugador */
   char *               inmunevict;      /* Mensaje de inmune para la victima */
   char *               inmuneroom;      /* Mensaje de inmune para la room */
   char *               inmunemud;       /* Mensaje de inmune para todo el MUD */
};

/*
 * Estructura de niveles de disciplina
 */
struct disci_table
{
   DISCI *              next; /* Siguiente disciplina en la lista */
   DISCI *              prev; /* Previa disciplina en la list */
   char *               fichero; /* Nombre del fichero de la disciplina */
   char *               nombre; /* Nombre de la disciplina */
   char *               descripcion; /* Descripcion de la disciplina */
   sh_int               numero; /* Que numero de disciplina es */
   char *               clan; /* Es propia de que clan */
   char *               niveles[10]; /* Niveles que incluyen */
   NIVEL *              nivel_data; /* Paso a la tabla de niveles de la disciplina */

   NIVEL *              primer_nivel; /* Primer nivel de la lista */
   NIVEL *              ultimo_nivel; /* Ultimo nivel de la lista */
};

/*
 * Estructura de los niveles de las disciplinas
 */
struct disci_nivel
{
   NIVEL *              next; /* Siguiente nivel en la lista */
   NIVEL *              prev; /* Previo nivel en la lista */
   char *               fichero; /* Nombre del fichero */
   DO_FUN *             do_fun; /* Enlace a las funciones as eliminamos numero de comandos */
   char *               nombre; /* Nombre del nivel */
   char *               descripcion; /* Descripcion del nivel */
   sh_int               nivel; /* Numero del nivel */
   int               dnivel; /* Numero del affect del nivel */
   sh_int               disciplina; /*  A que disciplina corresponde */
   sh_int               tipo; /* Tipos de nivel */
   sh_int               sangre; /* Sangre que requiere */
   int                  wait; /* Wait que produce su uso */
   char *               clan; /* De que clan es propia */
   char *               dammes; /* Mensaje de danyo */
   char *               sevames; /* Mensaje de cuando se va el afect si lo da */
   DISCI_AFFECT *       affect; /* Para el nombre el affect etc etc etc */
   char *               exitoch;        /* Mensaje de exito para el jugador */
   char *               exitovict;      /* Mensaje de exito para la victima */
   char *               exitoroom;      /* Mensaje de exito para la room */
   char *               exitomud;       /* Mensaje de extito para todo el MUD */
   ...
   DISCI_AFFECT *       primer_affect; /* Primer affect de la lista */
   DISCI_AFFECT *       ultimo_affect; /* Ultimo affect de la lista */
};
...
extern const char *         disciplina[MAX_DISCIPLINAS]; /* Extructura de disciplinas */
...
extern  char *  const   dis_affect      [];
extern  char *  const   esf_affect      [];
extern  char *  const   disci_tnombre   [MAX_TIPOS];
extern  char *  const   esferas         [MAX_ESFERAS];
extern  char *  const   dis             [MAX_DISCIPLINAS];
...
extern    char *  const   container_flags []; /* OLC VampiroAreaEditor */
...
extern		CMDTYPE		  *	command_hash	[MAX_COMANDOS];
...
extern          DISCI             *     primera_disciplina;
extern          DISCI             *     segunda_disciplina;
extern          DISCI             *     disci;
extern          NIVEL             *     primer_nivel;
extern          NIVEL             *     ultimo_nivel;
...
extern          QUEST_DATA      *     questmaster; /* SiGo y SaNgUi */
...
DECLARE_DO_FUN( do_miembros );
DECLARE_DO_FUN( do_mejorar ); /* Sustituye al comando do_train ke era un Snipett de ROM */
DECLARE_DO_FUN( do_mensaje );
DECLARE_DO_FUN( do_vficha );
/*DECLARE_DO_FUN( do_diablerizar );*/
DECLARE_DO_FUN( do_decapitar);
DECLARE_DO_FUN( do_topten   );
/*DECLARE_DO_FUN( do_ascender );
DECLARE_DO_FUN( do_degradar );
DECLARE_DO_FUN( do_abdicar );*/
/*DECLARE_DO_FUN( do_crear_taquilla );*/
DECLARE_DO_FUN( do_camuflaje );
DECLARE_DO_FUN( do_ocopy	); /* Editor de areas */
DECLARE_DO_FUN( do_mcopy     );
DECLARE_DO_FUN( do_rcopy     );
DECLARE_DO_FUN( do_guardatodo );
DECLARE_DO_FUN( do_omedit   );
DECLARE_DO_FUN( do_ooedit    );
DECLARE_DO_FUN( do_oredit    );  /* Fin Editor de areas */
DECLARE_DO_FUN( do_setquest );
DECLARE_DO_FUN( do_queststat );
DECLARE_DO_FUN( do_heal         );
DECLARE_DO_FUN( do_corazon_de_piedra ); /* Taumaturgia nivel 4 */
DECLARE_DO_FUN( skill_notfound	);
DECLARE_DO_FUN( do_aassign	);
DECLARE_DO_FUN( do_add_imm_host );
DECLARE_DO_FUN( do_adminlist	);
DECLARE_DO_FUN(	do_advance	);
DECLARE_DO_FUN(   do_generacionup );    /* SiGo */
...
DECLARE_DO_FUN( do_saldo );
DECLARE_DO_FUN( do_reintegro );
DECLARE_DO_FUN( do_ingreso );
...
DECLARE_DO_FUN(                 do_empalar                      ); /* empalar */
DECLARE_DO_FUN(                 do_tangana                      ); /* tangana */
DECLARE_DO_FUN(	do_balzhur	);
DECLARE_DO_FUN(	do_bamfin	);
DECLARE_DO_FUN(	do_bamfout	);
DECLARE_DO_FUN(	do_watch	);
DECLARE_DO_FUN(	do_ban		);
DECLARE_DO_FUN(	do_banco	                        );   /* banco xD */
...
DECLARE_DO_FUN( do_disciplinas    ); /* para ver las disciplinas */
...
DECLARE_DO_FUN(   do_misericordia  );
...
DECLARE_DO_FUN(	do_adquirir           );  /* Para adquirir un nuevo nivel de disciplina */
...
DECLARE_DO_FUN( do_regreso       );  /* SiGo */
...
/*DECLARE_DO_FUN(  do_toma_polla );*/
DECLARE_DO_FUN( do_track	);
DECLARE_DO_FUN( do_train    );
DECLARE_DO_FUN( do_asimilar ); /* Asimilar un nivel de una disciplina */
DECLARE_DO_FUN( do_traffic	);
DECLARE_DO_FUN(	do_transfer	);
DECLARE_DO_FUN(	do_trucos_del_almendruco        );
...
DECLARE_SPELL_FUN(      spell_draining_breath   );
...
DECLARE_SPELL_FUN(      spell_habilidad		);
...
/* declarados en skills_disciplinas.c */
/* animalismo (1) */
DECLARE_SPELL_FUN(          spell_comunion_espiritual  );
DECLARE_SPELL_FUN(          spell_bestia_dominada       );
/* nivel 2 vision del alma, spell samug */
DECLARE_SPELL_FUN(          spell_toque_espiritual      ); /* nivel tres */
DECLARE_SPELL_FUN(          spell_aura_percibida            ); /* nivel cuatro */
DECLARE_SPELL_FUN(          spell_percepcion_asesina          ); /* nivel cinco */
DECLARE_SPELL_FUN(          spell_forma_espiritual          ); /* nivel */
/* dementacion (7) */
DECLARE_SPELL_FUN(          spell_confusion                     ); /* durante el tiempo que dura la victima no acierta la mayoria de golpes */
DECLARE_SPELL_FUN(          spell_locura_masiva              ); /* causa locura a todos en la habitacion */
DECLARE_SPELL_FUN(          spell_coma_masivo              ); /* coma a todos en la habitacion */
 ...
 */
#define ESFERA_DIR              "../esfera/" /* Esferas */
#define DISCIPLINAS_DIR         "../disciplinas/"  /* disciplinas */
#define DISCIPLINAS_NIVELES_DIR         "../disciplinas/niveles/"  /* Para los niveles de las disciplinas */
#define TAQUILLAS_DIR   "../taquillas/" /* Pa las takillas */
#define CONQUISTAS_DIR  "../conquistas/" /* Para als conquistas */
...
#define QUEST_FILE   SYSTEM_DIR "quest.dat" /* Archivo de datos del Quest más ter xD*/ /* SiGo FuMo Y No Me aBuRRo...*/
#define WATCH_LIST      "watch.lst"     /* List of watches              */
#define BAN_LIST        "ban.lst"       /* List of bans                 */
#define RESERVED_LIST	"reserved.lst"	/* List of reserved names	*/
#define DISCI_LIST      "disciplinas.lst" /* Lista de disciplinas       */
#define NIVEL_LIST      "nivel.lst"     /* Lista de niveles de disciplina */
#define ESFERA_LIST     "esfera.lst"    /* Lista de esferas */
...
#define GNLEVEL_FILE SYSTEM_DIR "generacion.txt"  /* SiGo */
...
#define DISCIPLINAS_FILE DISCIPLINAS_DIR "disciplinas.dat" /* Disciplinas */
...
#define FALLOS_FILE     SYSTEM_DIR "fallos.dat"
#define EXE_FILE       "../debug/src/vampiromud"            /* ejecutable path     */
#define LAST_COMMAND    SYSTEM_DIR "lastcommand.dat" /* Ultimo comando introducido antes de petar el MUD */
...
/*
 * Arena Pk torneos legales sin perdida de stats ni Genepk
 */
...
/* Mejorado por SiGo Vampiromud2.0b 2002 */
void   generar_topten  args( ( void ) );
void   guardar_topten  args( ( void ) );
void   cargar_topten   args( ( void ) );
...
void    do_guardatodo   args( ( CHAR_DATA *ch, char *argument ) );
...
void do_abdicar        args(( CHAR_DATA *pj, char *argument, CLAN_DATA *familia  ));
void do_degradar       args(( CHAR_DATA *pj, char *argument, CLAN_DATA *familia  ));
void do_ascender       args(( CHAR_DATA *pj, char *argument, CLAN_DATA *familia  ));
...
void    do_trucos_del_almendruco         args( ( CHAR_DATA *ch, char *argument ) );
void    do_mortality    args( ( CHAR_DATA *ch, char *argument ) ); /* Pa canear */

/* boards.c */
void	load_boards	args( ( void ) );
BD *  get_board   args( ( CHAR_DATA *ch ) );
...
// Familias
FAMILIA_DATOS * buscaFamilia args(( char *nombre ));

void	load_clans	args( ( void ) );
void	save_clan	args( ( CLAN_DATA *clan ) );
void    do_pertenecer  args( ( CHAR_DATA *ch, char *argument, RACE_TYPE *raza ) );
...
/*void  send_to_char_color   args( ( DESCRIPTOR_DATA *d, const char *txt,
            int length ) );*/
...
/* disciplinas2.c */
int   disc_points_needed args( (CHAR_DATA *ch) );                /* puntos necesarios para adkirir un nivel de disciplina */
void  ganar_disci_puntos  args( (CHAR_DATA *ch, int points ) );  /* ganancia de puntos de disciplina */
void    cent_to_char  args( (char *txt, CHAR_DATA *ch) );  /* texto en pantalla centrado */
void    do_creo   args( ( CHAR_DATA *ch, char *argument ) ); /* Creo ignem Discipina Taumaturgia */
void    do_caldero  args( ( CHAR_DATA *ch, char *argument ) ); /* Caldero Sanguineo Disciplina Taumaturgia */
void    do_forma_incorporea  args( ( CHAR_DATA *ch, char *argument ) ); /* Niebla Disciplina Taumaturgia */
void do_diablerizar  args(( CHAR_DATA *ch, char *argument, CHAR_DATA *victim ));
void perdida_total   args(( CHAR_DATA *ch, CHAR_DATA *victim ));

/* declarar.c */
void do_declarar        args( ( CHAR_DATA *ch, char *argument ) );
/* vampireroll.c */
void do_abrazar  args( ( CHAR_DATA *ch, char *argument ) );
void do_decision_hereje  args( ( CHAR_DATA *ch ) );
void do_adiestrar  args( ( CHAR_DATA *ch, char *argument ) );
void do_decision_mortales  args( ( CHAR_DATA *ch ) );
void tira_dados       args( ( CHAR_DATA *ch, int ndados, int ncaras, int bonus, int dificultad ) );
/* vampiros.c */
void disciplina_to_char  args( ( CHAR_DATA *ch, DISCI_AFFECT *daf ) );
void disciplina_remove  args( ( CHAR_DATA *ch, DISCI_AFFECT *daf ) );
void disci_affect_modificar  args( ( CHAR_DATA *ch, DISCI_AFFECT *daf, bool dAff ) );
void save_disciplina         args( ( DISCI *disciplina ) );
void save_nivel              args( ( NIVEL *nivel ) );
void load_disciplinas        args( ( void ) );
void load_nivel              args( ( void ) );
void interprete              args( ( CHAR_DATA *ch, NIVEL *nivel, char *argument ) );
/*void lanzar_disciplina       args( ( CHAR_DATA *ch, CHAR_DATA *victim, NIVEL *nivel ) );*/
/*bool lanzar_disciplina       args( ( CHAR_DATA *ch, CHAR_DATA *victim, NIVEL *nivel ) );*/

/* esferas.c */
void esfera_to_char     args( ( CHAR_DATA *ch, ESFERA_AFFECT *eaf ) );
void esfera_remove      args( ( CHAR_DATA *ch, ESFERA_AFFECT *eaf ) );
void esfera_affect_modificar    args( ( CHAR_DATA *ch, ESFERA_AFFECT *eaf ) );

/* automob.c */
void crear_automob      args( ( CHAR_DATA *ch, CHAR_DATA *victim ) );

/* autoobj.c */
void crear_autoobjeto   args( ( CHAR_DATA *ch, CHAR_DATA *victim, OBJ_DATA *obj ) );

/* renacimiento.c */
void renacimiento       args( ( CHAR_DATA *ch ) );
...
sh_int	ris_damage	args( ( CHAR_DATA *ch, int dam, int ris ) );
...
void  ganancia_pk   args( ( CHAR_DATA *ch ) );
void  perdida_pk   args( ( CHAR_DATA *victim ) );
...
void do_crear_taquilla       args(( CHAR_DATA *ch, char *argument, OBJ_DATA *taquilla ));
...
void    calculo_coeficiente args((CHAR_DATA *ch));
...
/* Kayser para el sset delete */
void    free_skill     args( ( SKILLTYPE *skill ) );
/* Fin kayser para el sset delete. Kayser 2004*/
...
/*int   get_gen     args( ( CHAR_DATA *ch ) );*/
int	get_exp_worth	args( ( CHAR_DATA *ch ) );
/*int   get_gen_worth  args( ( CHAR_DATA *ch ) );*/
/*int   gen_level   args( ( CHAR_DATA *ch, sh_int generacion ) );*/
...
char *	daffect_loc_name	args( ( int localizacion ) );
...
void	update_userec	args( ( struct timeval *time_used,
				struct timerset *userec ) );
void           asignar_disciplinas      args( ( CHAR_DATA *ch ) ); /* Asignacion de disciplinas al 0 al crear una nueva ficha */
...
/* nota.c */
NOTE_DATA *note_free;
void free_nota          args( (NOTE_DATA *note) );
...
void    write_taquillas   args( ( CHAR_DATA *ch, char *name, OBJ_DATA *objrem ) );
...
/* taquillas.c */
void fwrite_locker        args( ( CHAR_DATA *ch, OBJ_DATA *locker ) );

/* conquistas.c */
void save_con_file    args(( CON_DATA *Conquista));
CON_DATA *get_conquista  args ( (char *name) );
AREA_DATA *get_area_fp   args ( ( char *filename ) );
...
/* update.c */
void quest_reclamar     args( ( CHAR_DATA *ch, OBJ_DATA *obj ) );
void bajar_nivel args( ( CHAR_DATA *ch ) );
void  advance_level  args( ( CHAR_DATA *ch ) );
void atrasar_nivel  args( ( CHAR_DATA *ch ) );
void	gain_exp	args( ( CHAR_DATA *ch, int gain ) );
void	gain_conquista	args( ( CHAR_DATA *ch, int ganancia ) );
void	gain_condition	args( ( CHAR_DATA *ch, int iCond, int value ) );
void  advance_generacion args ( ( CHAR_DATA *ch ) );  /* SiGo */
void  degradacion_generacion_pk args ( ( CHAR_DATA *ch ) );
...
void mensa_todos        args( ( CHAR_DATA *ch, char *envio, char *mensaje) ); /* Sustituye send_to_char en mensajes del sistema */
void mensa              args( ( CHAR_DATA *ch, char *envio, char *mensaje) ); /* Sustituye echo_to_all */
void mensa_familia      args( ( CHAR_DATA *ch, char *mensaje) ); /* Mensajes para la familia */
void mensa_nota			args( ( CHAR_DATA *ch, char *envio, char *mensaje) ); /* Para mensajes de notas */
void espera_menos       args( ( CHAR_DATA *ch, int nivel, int espera) ); /* Wait a reducir para jugadores afectados por celeridad y temporis */
...
#define GA_RIS          BV30
/*
 * Esto es para que se vea en color el texto usando el caracter especial "&"
 * SiGo email to sigo@vampiromud.com
 * COLIKOTRON Code Team 1999
 */
#define send_to_char send_to_char_color
#define send_to_pager send_to_pager_color
#define ch_printf ch_printf_color
#define pager_printf pager_printf_color
